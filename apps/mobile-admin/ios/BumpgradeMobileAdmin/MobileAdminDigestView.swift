...
struct MobileAdminDigestView: View {
    ...
    private var directorReviewPanel: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("Director review API")
                .font(.caption.weight(.black))
                .textCase(.uppercase)
                .foregroundStyle(Color(red: 0.46, green: 0.38, blue: 0.09))
            Text(contract.directorReviewApi.route)
                .font(.title3.weight(.bold))
                .foregroundStyle(Color(red: 0.05, green: 0.07, blue: 0.06))
            Text(contract.directorReviewApi.purpose)
                .font(.subheadline)
                .lineSpacing(3)
                .foregroundStyle(Color(red: 0.31, green: 0.36, blue: 0.33))
            Detail(label: "Status", value: "\(contract.directorReviewApi.status) · issue #\(contract.directorReviewApi.issue)")
            Detail(label: "Auth", value: contract.directorReviewApi.authBoundary)
            Detail(label: "Boundary", value: contract.directorReviewApi.reviewBoundary)
            Detail(label: "Inputs", value: contract.directorReviewApi.requiredInputs.joined(separator: ", "))
        }
        .padding(18)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(Color.white)
    ...
    private var commerceReviewPanel: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("Commerce review API")
                .font(.caption.weight(.black))
                .textCase(.uppercase)
                .foregroundStyle(Color(red: 0.46, green: 0.38, blue: 0.09))
            Text(contract.commerceReviewApi.route)
                .font(.title3.weight(.bold))
                .foregroundStyle(Color(red: 0.05, green: 0.07, blue: 0.06))
            Text(contract.commerceReviewApi.purpose)
                .font(.subheadline)
                .lineSpacing(3)
                .foregroundStyle(Color(red: 0.31, green: 0.36, blue: 0.33))
            Detail(label: "Status", value: "\(contract.commerceReviewApi.status) · issue #\(contract.commerceReviewApi.issue)")
            Detail(label: "Auth", value: contract.commerceReviewApi.authBoundary)
            Detail(label: "Boundary", value: contract.commerceReviewApi.reviewBoundary)
            Detail(label: "Inputs", value: contract.commerceReviewApi.requiredInputs.joined(separator: ", "))
        }
        .padding(18)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(Color.white)
