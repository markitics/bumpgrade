...
} from "../src/lib/audience-sequence-dispatch-preflights";
import {
...
} from "../src/lib/audience-sequence-dispatch-attempts";
import {
...
} from "../src/lib/audience-sequence-queue-producer-readiness";
import {
...
} from "../src/lib/audience-sequence-queue-consumer-readiness";
import {
...
} from "../src/lib/audience-sequence-provider-call-readiness";
import {
...
} from "../src/lib/audience-sequence-delivery-attempt-readiness";
import {
...
test.describe("Bumpgrade scaffold", () => {
  ...
  test("audience automation source data exposes opt-in, tags, sequences, and automation rules", async ({ request, page }) => {
    ...
    expect(payload.audienceSequenceDispatchAttempts).toEqual(
      expect.objectContaining({
        id: "audience-sequence-dispatch-attempt-contract",
        status: audienceSequenceDispatchAttemptStatus,
        issue: audienceSequenceDispatchAttemptIssue,
        parentIssue: 17,
        apiRoute: audienceSequenceDispatchAttemptApiRoute,
        ownerRoute: "/admin/audience",
        confirmation: expect.objectContaining({
          required: true,
          text: audienceSequenceDispatchAttemptConfirmationText,
        }),
        counts: expect.objectContaining({
    ...
    expect(payload.audienceSequenceQueueProducerReadiness).toEqual(
      expect.objectContaining({
    ...
    expect(payload.audienceSequenceQueueConsumerReadiness).toEqual(
      expect.objectContaining({
        ...
        dependency: expect.objectContaining({
          ...
          queueProducerReadinessIssue: audienceSequenceQueueProducerReadinessIssue,
    ...
    expect(payload.audienceSequenceProviderCallReadiness).toEqual(
      expect.objectContaining({
        ...
        dependency: expect.objectContaining({
          ...
          queueConsumerReadinessIssue: audienceSequenceQueueConsumerReadinessIssue,
          queueConsumerReadinessRequired: true,
        }),
        counts: expect.objectContaining({
          providerCallReadinessRecords: expect.any(Number),
          dryRunProviderCallContracts: expect.any(Number),
          dryRunMessagesSnapshotted: expect.any(Number),
          heldEnrollmentsSnapshotted: expect.any(Number),
          activeSuppressionsSnapshotted: expect.any(Number),
    ...
    expect(payload.audienceSequenceDeliveryAttemptReadiness).toEqual(
      expect.objectContaining({
        ...
        dependency: expect.objectContaining({
          ...
          providerCallReadinessIssue: audienceSequenceProviderCallReadinessIssue,
          providerCallReadinessRequired: true,
        }),
        counts: expect.objectContaining({
          deliveryAttemptReadinessRecords: expect.any(Number),
          dryRunDeliveryAttemptContracts: expect.any(Number),
          dryRunMessagesSnapshotted: expect.any(Number),
          heldEnrollmentsSnapshotted: expect.any(Number),
          activeSuppressionsSnapshotted: expect.any(Number),
    ...
    expect(payload.audienceExportReadiness).toEqual(
      expect.objectContaining({
        ...
        counts: expect.objectContaining({
          subscribers: expect.any(Number),
          subscribedSubscribers: expect.any(Number),
          consentedSubscribers: expect.any(Number),
          exportEligibleSubscribers: expect.any(Number),
          suppressedSubscribers: expect.any(Number),
          activeSuppressionEntries: expect.any(Number),
          unsubscribedSubscribers: expect.any(Number),
          pausedSequenceSubscribers: expect.any(Number),
    ...
    expect(payload.audienceSequenceScheduleIntents).toEqual(
      expect.objectContaining({
        ...
        counts: expect.objectContaining({
          scheduleIntents: expect.any(Number),
          activeDryRunIntents: expect.any(Number),
          readyEnrollmentsReserved: expect.any(Number),
          heldEnrollmentsSnapshotted: expect.any(Number),
          deliveryQueueRowsCreatedRecords: 0,
          recipientPayloadsCreatedRecords: 0,
          personalizedBodiesCreatedRecords: 0,
    ...
    expect(payload.audienceSequenceDeliveryBatches).toEqual(
      expect.objectContaining({
        ...
        counts: expect.objectContaining({
          deliveryBatches: expect.any(Number),
          dryRunBatches: expect.any(Number),
          readyEnrollmentsBatched: expect.any(Number),
          heldEnrollmentsSnapshotted: expect.any(Number),
          activeSuppressionsSnapshotted: expect.any(Number),
          providerSendEnabledBatches: 0,
    ...
    expect(payload.audienceSequenceDispatchPreflights).toEqual(
      expect.objectContaining({
    ...
    expect(payload.workspaces).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          sequenceDispatchPreflightApiRoute: audienceSequenceDispatchPreflightApiRoute,
          sequenceDispatchAttemptApiRoute: audienceSequenceDispatchAttemptApiRoute,
          sequenceProviderCallReadinessApiRoute: audienceSequenceProviderCallReadinessApiRoute,
    ...
    expect(payload.writeBoundary).toContain("Issue #364 can record owner-confirmed sequence dispatch attempt receipts");
    expect(payload.writeBoundary).toContain("Issue #366 can record owner-reviewed sequence Queue producer readiness");
    expect(payload.writeBoundary).toContain("Issue #368 can record owner-reviewed sequence Queue consumer readiness");
    expect(payload.writeBoundary).toContain("Issue #370 can record owner-reviewed sequence provider-call readiness");
    ...
    expect(payload.caveat).toContain("dry-run sequence dispatch-attempt receipt evidence from issue #364");
    expect(payload.caveat).toContain("sequence Queue producer readiness evidence from issue #366");
    expect(payload.caveat).toContain("sequence Queue consumer readiness evidence from issue #368");
    expect(payload.caveat).toContain("sequence provider-call readiness evidence from issue #370");
    ...
    await expect(page.getByRole("heading", { name: /Unsubscribe evidence is captured before any sends exist/i })).toBeVisible();
    ...
    expect(afterPayload.audienceExportReadiness.counts.unsubscribedSubscribers).toBeGreaterThanOrEqual(1);
    expect(afterPayload.audienceExportReadiness.counts.pausedSequenceSubscribers).toBeGreaterThanOrEqual(1);
    expect(afterPayload.audienceExportReadiness.counts.exportFilesCreatedRecords).toBe(0);
    expect(afterPayload.audienceExportReadiness.redaction).toEqual(
      expect.objectContaining({
        rawEmailsIncluded: false,
        subscriberIdsIncluded: false,
        exportFileUrlsIncluded: false,
  ...
  test("audience unsubscribe API records suppression evidence without revealing membership", async ({ request }) => {
    ...
    const unauthorizedSequenceScheduleIntentResponse = await request.post(audienceSequenceScheduleIntentApiRoute, {
      data: {
        sequenceId: "sequence-indie-launch-nurture",
        expectedWorkspaceRevisionId: audienceAutomationWorkspace.revisionId,
        ...
        idempotencyKey: `${idempotencyKey}-unauthorized-sequence-schedule-intent`,
      },
    });
    expect(unauthorizedSequenceScheduleIntentResponse.status()).toBe(401);
    await expect(unauthorizedSequenceScheduleIntentResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "owner_session_required" }),
    );

    const unauthorizedSequenceDeliveryBatchResponse = await request.post(audienceSequenceDeliveryBatchApiRoute, {
      data: {
        scheduleIntentId: "sequence-schedule-intent-not-public",
        sequenceId: "sequence-indie-launch-nurture",
        expectedWorkspaceRevisionId: audienceAutomationWorkspace.revisionId,
        expectedSequenceStatus: "draft",
        expectedReadyEnrollmentCount: 0,
        confirmationText: audienceSequenceDeliveryBatchConfirmationText,
        idempotencyKey: `${idempotencyKey}-unauthorized-sequence-delivery-batch`,
      },
    });
    ...
    const unauthorizedSequenceDispatchPreflightResponse = await request.post(audienceSequenceDispatchPreflightApiRoute, {
      data: {
        deliveryQueueMessageId: "sequence-delivery-queue-message-not-public",
        sequenceId: "sequence-indie-launch-nurture",
        expectedWorkspaceRevisionId: audienceAutomationWorkspace.revisionId,
        expectedSequenceStatus: "draft",
        expectedReadyEnrollmentCount: 0,
        confirmationText: audienceSequenceDispatchPreflightConfirmationText,
        idempotencyKey: `${idempotencyKey}-unauthorized-sequence-dispatch-preflight`,
      },
    });
    expect(unauthorizedSequenceDispatchPreflightResponse.status()).toBe(401);
    await expect(unauthorizedSequenceDispatchPreflightResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "owner_session_required" }),
    );

    const unauthorizedSequenceDispatchAttemptResponse = await request.post(audienceSequenceDispatchAttemptApiRoute, {
      data: {
        dispatchPreflightId: "sequence-dispatch-preflight-not-public",
        sequenceId: "sequence-indie-launch-nurture",
        expectedWorkspaceRevisionId: audienceAutomationWorkspace.revisionId,
        expectedSequenceStatus: "draft",
        expectedReadyEnrollmentCount: 0,
        confirmationText: audienceSequenceDispatchAttemptConfirmationText,
        idempotencyKey: `${idempotencyKey}-unauthorized-sequence-dispatch-attempt`,
      },
    });
    expect(unauthorizedSequenceDispatchAttemptResponse.status()).toBe(401);
    await expect(unauthorizedSequenceDispatchAttemptResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "owner_session_required" }),
    );

    const unauthorizedSequenceQueueProducerReadinessResponse = await request.post(
      audienceSequenceQueueProducerReadinessApiRoute,
      {
        data: {
          dispatchAttemptId: "sequence-dispatch-attempt-not-public",
          sequenceId: "sequence-indie-launch-nurture",
          expectedWorkspaceRevisionId: audienceAutomationWorkspace.revisionId,
          expectedSequenceStatus: "draft",
          expectedReadyEnrollmentCount: 0,
          confirmationText: audienceSequenceQueueProducerReadinessConfirmationText,
          idempotencyKey: `${idempotencyKey}-unauthorized-sequence-queue-producer-readiness`,
        },
      },
    );
    expect(unauthorizedSequenceQueueProducerReadinessResponse.status()).toBe(401);
    await expect(unauthorizedSequenceQueueProducerReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "owner_session_required" }),
    );

    const unauthorizedSequenceQueueConsumerReadinessResponse = await request.post(
      audienceSequenceQueueConsumerReadinessApiRoute,
      {
        data: {
          queueProducerReadinessId: "sequence-queue-producer-readiness-not-public",
          sequenceId: "sequence-indie-launch-nurture",
          expectedWorkspaceRevisionId: audienceAutomationWorkspace.revisionId,
          expectedSequenceStatus: "draft",
          expectedReadyEnrollmentCount: 0,
          confirmationText: audienceSequenceQueueConsumerReadinessConfirmationText,
          idempotencyKey: `${idempotencyKey}-unauthorized-sequence-queue-consumer-readiness`,
        },
      },
    );
    expect(unauthorizedSequenceQueueConsumerReadinessResponse.status()).toBe(401);
    await expect(unauthorizedSequenceQueueConsumerReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "owner_session_required" }),
    );

    const unauthorizedSequenceProviderCallReadinessResponse = await request.post(
      audienceSequenceProviderCallReadinessApiRoute,
      {
        data: {
          queueConsumerReadinessId: "sequence-queue-consumer-readiness-not-public",
          sequenceId: "sequence-indie-launch-nurture",
          expectedWorkspaceRevisionId: audienceAutomationWorkspace.revisionId,
          expectedSequenceStatus: "draft",
          expectedReadyEnrollmentCount: 0,
          confirmationText: audienceSequenceProviderCallReadinessConfirmationText,
          idempotencyKey: `${idempotencyKey}-unauthorized-sequence-provider-call-readiness`,
        },
      },
    );
    expect(unauthorizedSequenceProviderCallReadinessResponse.status()).toBe(401);
    await expect(unauthorizedSequenceProviderCallReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "owner_session_required" }),
    );

    const unauthorizedSequenceDeliveryAttemptReadinessResponse = await request.post(
      audienceSequenceDeliveryAttemptReadinessApiRoute,
      {
        data: {
          providerCallReadinessId: "sequence-provider-call-readiness-not-public",
          sequenceId: "sequence-indie-launch-nurture",
          expectedWorkspaceRevisionId: audienceAutomationWorkspace.revisionId,
          expectedSequenceStatus: "draft",
          expectedReadyEnrollmentCount: 0,
          confirmationText: audienceSequenceDeliveryAttemptReadinessConfirmationText,
          idempotencyKey: `${idempotencyKey}-unauthorized-sequence-delivery-attempt-readiness`,
        },
      },
    );
    expect(unauthorizedSequenceDeliveryAttemptReadinessResponse.status()).toBe(401);
    await expect(unauthorizedSequenceDeliveryAttemptReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "owner_session_required" }),
    );
      ...
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          issueNumbers: [
            ...
            211, 253, 259, 347, 351, 354, 358, 360, 362, 364, 366, 368, 370, 372,
          ],
        }),
        expect.objectContaining({
    ...
    expect(payload.readContracts).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          auth: "owner-session",
          stableIds: expect.arrayContaining(["sequenceScheduleIntentId", "emailSequenceId", "idempotencyKey"]),
        }),
        expect.objectContaining({
  ...
  test("allowlisted owner can sign in and open protected admin surfaces", async ({ page }, testInfo) => {
    ...
    const sequenceReadiness = audienceSourceAfterNotePayload.audienceSequenceDeliveryReadiness;
    const sequenceRecord = sequenceReadiness.records.find(
      (record: { sequenceId: string }) => record.sequenceId === "sequence-indie-launch-nurture",
    );
    if (!sequenceRecord) throw new Error("Expected seeded sequence readiness record.");
    expect(sequenceRecord).toEqual(
      expect.objectContaining({
        sequenceId: "sequence-indie-launch-nurture",
        status: "draft",
        deliveryEnabled: false,
    ...
    const sequenceScheduleIntentResponse = await page.request.post(audienceSequenceScheduleIntentApiRoute, {
      data: {
        ...
        requestedStartAt: "2026-05-21T16:30",
        confirmationText: audienceSequenceScheduleIntentConfirmationText,
        idempotencyKey: sequenceScheduleIntentIdempotencyKey,
      },
    });
    expect(sequenceScheduleIntentResponse.ok(), await sequenceScheduleIntentResponse.text()).toBeTruthy();
    const sequenceScheduleIntentPayload = await sequenceScheduleIntentResponse.json();
    expect(sequenceScheduleIntentPayload).toEqual(
      expect.objectContaining({
        ok: true,
        ...
        intent: expect.objectContaining({
          sequenceId: sequenceRecord.sequenceId,
          expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
          readyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
    ...
    const replaySequenceScheduleIntentResponse = await page.request.post(audienceSequenceScheduleIntentApiRoute, {
      data: {
        sequenceId: sequenceRecord.sequenceId,
        expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
        expectedSequenceStatus: sequenceRecord.status,
    ...
    expect(replaySequenceScheduleIntentResponse.ok(), await replaySequenceScheduleIntentResponse.text()).toBeTruthy();
    await expect(replaySequenceScheduleIntentResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: true,
        duplicate: true,
        intent: expect.objectContaining({ id: sequenceScheduleIntentPayload.intent.id }),
      }),
    );
    const staleSequenceScheduleIntentResponse = await page.request.post(audienceSequenceScheduleIntentApiRoute, {
      data: {
        sequenceId: sequenceRecord.sequenceId,
        expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
        expectedSequenceStatus: sequenceRecord.status,
        ...
        confirmationText: audienceSequenceScheduleIntentConfirmationText,
        idempotencyKey: `playwright-sequence-schedule-intent-stale-${Date.now()}`,
      },
    });
    expect(staleSequenceScheduleIntentResponse.status()).toBe(409);
    await expect(staleSequenceScheduleIntentResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: false,
    ...
    const sequenceDeliveryBatchIdempotencyKey = `playwright-sequence-delivery-batch-${Date.now()}`;
    const sequenceDeliveryBatchResponse = await page.request.post(audienceSequenceDeliveryBatchApiRoute, {
      data: {
        ...
        expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
        confirmationText: audienceSequenceDeliveryBatchConfirmationText,
        idempotencyKey: sequenceDeliveryBatchIdempotencyKey,
      },
    });
    expect(sequenceDeliveryBatchResponse.ok(), await sequenceDeliveryBatchResponse.text()).toBeTruthy();
    const sequenceDeliveryBatchPayload = await sequenceDeliveryBatchResponse.json();
    expect(sequenceDeliveryBatchPayload).toEqual(
      expect.objectContaining({
        ok: true,
    ...
    const replaySequenceDeliveryBatchResponse = await page.request.post(audienceSequenceDeliveryBatchApiRoute, {
      data: {
        scheduleIntentId: sequenceScheduleIntentPayload.intent.id,
        sequenceId: sequenceRecord.sequenceId,
        expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
    ...
    expect(replaySequenceDeliveryBatchResponse.ok(), await replaySequenceDeliveryBatchResponse.text()).toBeTruthy();
    await expect(replaySequenceDeliveryBatchResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: true,
        duplicate: true,
        batch: expect.objectContaining({ id: sequenceDeliveryBatchPayload.batch.id }),
      }),
    );
    const staleSequenceDeliveryBatchResponse = await page.request.post(audienceSequenceDeliveryBatchApiRoute, {
      data: {
        scheduleIntentId: sequenceScheduleIntentPayload.intent.id,
        sequenceId: sequenceRecord.sequenceId,
        expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
        ...
        expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments + 1,
        confirmationText: audienceSequenceDeliveryBatchConfirmationText,
        idempotencyKey: `playwright-sequence-delivery-batch-stale-${Date.now()}`,
      },
    });
    expect(staleSequenceDeliveryBatchResponse.status()).toBe(409);
    await expect(staleSequenceDeliveryBatchResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: false,
        code: "stale_readiness_count",
        currentReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
    ...
    const sequenceDispatchPreflightIdempotencyKey = `playwright-sequence-dispatch-preflight-${Date.now()}`;
    const sequenceDispatchPreflightResponse = await page.request.post(audienceSequenceDispatchPreflightApiRoute, {
      data: {
        ...
        expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
        confirmationText: audienceSequenceDispatchPreflightConfirmationText,
        idempotencyKey: sequenceDispatchPreflightIdempotencyKey,
      },
    });
    expect(sequenceDispatchPreflightResponse.ok(), await sequenceDispatchPreflightResponse.text()).toBeTruthy();
    const sequenceDispatchPreflightPayload = await sequenceDispatchPreflightResponse.json();
    expect(sequenceDispatchPreflightPayload).toEqual(
      expect.objectContaining({
        ok: true,
    ...
    const replaySequenceDispatchPreflightResponse = await page.request.post(audienceSequenceDispatchPreflightApiRoute, {
      data: {
        deliveryQueueMessageId: sequenceQueueMessagesPayload.messages.id,
        sequenceId: sequenceRecord.sequenceId,
        expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
    ...
    expect(replaySequenceDispatchPreflightResponse.ok(), await replaySequenceDispatchPreflightResponse.text()).toBeTruthy();
    await expect(replaySequenceDispatchPreflightResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: true,
        duplicate: true,
        preflight: expect.objectContaining({ id: sequenceDispatchPreflightPayload.preflight.id }),
      }),
    );
    const staleSequenceDispatchPreflightResponse = await page.request.post(audienceSequenceDispatchPreflightApiRoute, {
      data: {
        deliveryQueueMessageId: sequenceQueueMessagesPayload.messages.id,
        sequenceId: sequenceRecord.sequenceId,
        expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
        ...
        expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments + 1,
        confirmationText: audienceSequenceDispatchPreflightConfirmationText,
        idempotencyKey: `playwright-sequence-dispatch-preflight-stale-${Date.now()}`,
      },
    });
    expect(staleSequenceDispatchPreflightResponse.status()).toBe(409);
    await expect(staleSequenceDispatchPreflightResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: false,
        code: "stale_readiness_count",
        currentReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
    ...
    const sequenceDispatchAttemptIdempotencyKey = `playwright-sequence-dispatch-attempt-${Date.now()}`;
    const sequenceDispatchAttemptResponse = await page.request.post(audienceSequenceDispatchAttemptApiRoute, {
      data: {
        ...
        expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
        confirmationText: audienceSequenceDispatchAttemptConfirmationText,
        idempotencyKey: sequenceDispatchAttemptIdempotencyKey,
      },
    });
    expect(sequenceDispatchAttemptResponse.ok(), await sequenceDispatchAttemptResponse.text()).toBeTruthy();
    const sequenceDispatchAttemptPayload = await sequenceDispatchAttemptResponse.json();
    expect(sequenceDispatchAttemptPayload).toEqual(
      expect.objectContaining({
        ok: true,
        status: "sequence_dispatch_attempt_recorded",
        duplicate: false,
    ...
    const replaySequenceDispatchAttemptResponse = await page.request.post(audienceSequenceDispatchAttemptApiRoute, {
      data: {
        dispatchPreflightId: sequenceDispatchPreflightPayload.preflight.id,
        sequenceId: sequenceRecord.sequenceId,
        expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
    ...
    expect(replaySequenceDispatchAttemptResponse.ok(), await replaySequenceDispatchAttemptResponse.text()).toBeTruthy();
    await expect(replaySequenceDispatchAttemptResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: true,
        duplicate: true,
        attempt: expect.objectContaining({ id: sequenceDispatchAttemptPayload.attempt.id }),
      }),
    );
    const staleSequenceDispatchAttemptResponse = await page.request.post(audienceSequenceDispatchAttemptApiRoute, {
      data: {
        dispatchPreflightId: sequenceDispatchPreflightPayload.preflight.id,
        sequenceId: sequenceRecord.sequenceId,
        expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
        ...
        expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments + 1,
        confirmationText: audienceSequenceDispatchAttemptConfirmationText,
        idempotencyKey: `playwright-sequence-dispatch-attempt-stale-${Date.now()}`,
      },
    });
    expect(staleSequenceDispatchAttemptResponse.status()).toBe(409);
    await expect(staleSequenceDispatchAttemptResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: false,
        code: "stale_readiness_count",
        currentReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
    ...
    const sequenceQueueProducerReadinessIdempotencyKey = `playwright-sequence-queue-producer-readiness-${Date.now()}`;
    const sequenceQueueProducerReadinessResponse = await page.request.post(
      audienceSequenceQueueProducerReadinessApiRoute,
      {
        data: {
          ...
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          confirmationText: audienceSequenceQueueProducerReadinessConfirmationText,
          idempotencyKey: sequenceQueueProducerReadinessIdempotencyKey,
        },
      },
    );
    expect(sequenceQueueProducerReadinessResponse.ok(), await sequenceQueueProducerReadinessResponse.text()).toBeTruthy();
    const sequenceQueueProducerReadinessPayload = await sequenceQueueProducerReadinessResponse.json();
    expect(sequenceQueueProducerReadinessPayload).toEqual(
      expect.objectContaining({
        ok: true,
        ...
        record: expect.objectContaining({
          ...
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          dryRunMessageCount: sequenceReadiness.counts.readyEnrollments,
          producerBinding: "AUDIENCE_SEQUENCE_DELIVERY_QUEUE",
          producerMode: "dry_run_contract_only_no_cloudflare_queue_producer",
    ...
    const replaySequenceQueueProducerReadinessResponse = await page.request.post(
      audienceSequenceQueueProducerReadinessApiRoute,
      {
        data: {
          dispatchAttemptId: sequenceDispatchAttemptPayload.attempt.id,
    ...
    expect(
      replaySequenceQueueProducerReadinessResponse.ok(),
      await replaySequenceQueueProducerReadinessResponse.text(),
    ).toBeTruthy();
    await expect(replaySequenceQueueProducerReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: true,
        duplicate: true,
        record: expect.objectContaining({ id: sequenceQueueProducerReadinessPayload.record.id }),
      }),
    );
    const staleSequenceQueueProducerReadinessResponse = await page.request.post(
      audienceSequenceQueueProducerReadinessApiRoute,
      {
        data: {
          dispatchAttemptId: sequenceDispatchAttemptPayload.attempt.id,
          ...
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments + 1,
          confirmationText: audienceSequenceQueueProducerReadinessConfirmationText,
          idempotencyKey: `playwright-sequence-queue-producer-readiness-stale-${Date.now()}`,
        },
      },
    );
    expect(staleSequenceQueueProducerReadinessResponse.status()).toBe(409);
    await expect(staleSequenceQueueProducerReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: false,
        code: "stale_readiness_count",
        currentReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
    ...
    const sequenceQueueConsumerReadinessIdempotencyKey = `playwright-sequence-queue-consumer-readiness-${Date.now()}`;
    const missingSequenceQueueConsumerConfirmationResponse = await page.request.post(
      audienceSequenceQueueConsumerReadinessApiRoute,
      {
        data: {
          ...
          expectedSequenceStatus: sequenceRecord.status,
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          confirmationText: "Consume sequence Queue messages now",
          idempotencyKey: `${sequenceQueueConsumerReadinessIdempotencyKey}-missing-confirmation`,
        },
      },
    );
    expect(missingSequenceQueueConsumerConfirmationResponse.status()).toBe(400);
    await expect(missingSequenceQueueConsumerConfirmationResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "confirmation_required" }),
    );

    const missingSequenceQueueConsumerProducerResponse = await page.request.post(
      audienceSequenceQueueConsumerReadinessApiRoute,
      {
        data: {
          queueProducerReadinessId: "sequence-queue-producer-readiness-not-current",
          sequenceId: sequenceRecord.sequenceId,
          expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
          expectedSequenceStatus: sequenceRecord.status,
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          confirmationText: audienceSequenceQueueConsumerReadinessConfirmationText,
          idempotencyKey: `${sequenceQueueConsumerReadinessIdempotencyKey}-missing-producer`,
        },
      },
    );
    expect(missingSequenceQueueConsumerProducerResponse.status()).toBe(404);
    await expect(missingSequenceQueueConsumerProducerResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "queue_producer_readiness_not_found" }),
    );

    const sequenceQueueConsumerReadinessResponse = await page.request.post(
      audienceSequenceQueueConsumerReadinessApiRoute,
      {
        data: {
          queueProducerReadinessId: sequenceQueueProducerReadinessPayload.record.id,
          ...
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          confirmationText: audienceSequenceQueueConsumerReadinessConfirmationText,
          idempotencyKey: sequenceQueueConsumerReadinessIdempotencyKey,
        },
      },
    );
    expect(sequenceQueueConsumerReadinessResponse.ok(), await sequenceQueueConsumerReadinessResponse.text()).toBeTruthy();
    const sequenceQueueConsumerReadinessPayload = await sequenceQueueConsumerReadinessResponse.json();
    expect(sequenceQueueConsumerReadinessPayload).toEqual(
      expect.objectContaining({
        ok: true,
        ...
        record: expect.objectContaining({
          ...
          expectedSequenceStatus: sequenceRecord.status,
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          expectedQueueProducerReadinessStatus: audienceSequenceQueueProducerReadinessStatus,
          dryRunMessageCount: sequenceReadiness.counts.readyEnrollments,
          consumerBinding: "AUDIENCE_SEQUENCE_DELIVERY_QUEUE",
          consumerMode: "dry_run_contract_only_no_cloudflare_queue_consumer",
    ...
    const replaySequenceQueueConsumerReadinessResponse = await page.request.post(
      audienceSequenceQueueConsumerReadinessApiRoute,
      {
        data: {
          queueProducerReadinessId: sequenceQueueProducerReadinessPayload.record.id,
    ...
    expect(
      replaySequenceQueueConsumerReadinessResponse.ok(),
      await replaySequenceQueueConsumerReadinessResponse.text(),
    ).toBeTruthy();
    await expect(replaySequenceQueueConsumerReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: true,
        duplicate: true,
        record: expect.objectContaining({ id: sequenceQueueConsumerReadinessPayload.record.id }),
      }),
    );
    const staleSequenceQueueConsumerReadinessResponse = await page.request.post(
      audienceSequenceQueueConsumerReadinessApiRoute,
      {
        data: {
          queueProducerReadinessId: sequenceQueueProducerReadinessPayload.record.id,
          ...
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments + 1,
          confirmationText: audienceSequenceQueueConsumerReadinessConfirmationText,
          idempotencyKey: `playwright-sequence-queue-consumer-readiness-stale-${Date.now()}`,
        },
      },
    );
    expect(staleSequenceQueueConsumerReadinessResponse.status()).toBe(409);
    await expect(staleSequenceQueueConsumerReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: false,
        code: "stale_readiness_count",
        currentReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
    ...
    const sequenceProviderCallReadinessIdempotencyKey = `playwright-sequence-provider-call-readiness-${Date.now()}`;
    const missingSequenceProviderCallConfirmationResponse = await page.request.post(
      audienceSequenceProviderCallReadinessApiRoute,
      {
        data: {
          ...
          expectedSequenceStatus: sequenceRecord.status,
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          confirmationText: "Call the sequence provider now",
          idempotencyKey: `${sequenceProviderCallReadinessIdempotencyKey}-missing-confirmation`,
        },
      },
    );
    expect(missingSequenceProviderCallConfirmationResponse.status()).toBe(400);
    await expect(missingSequenceProviderCallConfirmationResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "confirmation_required" }),
    );

    const missingSequenceProviderCallConsumerResponse = await page.request.post(
      audienceSequenceProviderCallReadinessApiRoute,
      {
        data: {
          queueConsumerReadinessId: "sequence-queue-consumer-readiness-not-current",
          sequenceId: sequenceRecord.sequenceId,
          expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
          expectedSequenceStatus: sequenceRecord.status,
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          confirmationText: audienceSequenceProviderCallReadinessConfirmationText,
          idempotencyKey: `${sequenceProviderCallReadinessIdempotencyKey}-missing-consumer`,
        },
      },
    );
    expect(missingSequenceProviderCallConsumerResponse.status()).toBe(404);
    await expect(missingSequenceProviderCallConsumerResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "queue_consumer_readiness_not_found" }),
    );

    const sequenceProviderCallReadinessResponse = await page.request.post(
      audienceSequenceProviderCallReadinessApiRoute,
      {
        data: {
          queueConsumerReadinessId: sequenceQueueConsumerReadinessPayload.record.id,
          ...
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          confirmationText: audienceSequenceProviderCallReadinessConfirmationText,
          idempotencyKey: sequenceProviderCallReadinessIdempotencyKey,
        },
      },
    );
    expect(sequenceProviderCallReadinessResponse.ok(), await sequenceProviderCallReadinessResponse.text()).toBeTruthy();
    const sequenceProviderCallReadinessPayload = await sequenceProviderCallReadinessResponse.json();
    expect(sequenceProviderCallReadinessPayload).toEqual(
      expect.objectContaining({
        ok: true,
        ...
        record: expect.objectContaining({
          ...
          expectedSequenceStatus: sequenceRecord.status,
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          expectedQueueConsumerReadinessStatus: audienceSequenceQueueConsumerReadinessStatus,
          dryRunMessageCount: sequenceReadiness.counts.readyEnrollments,
          providerName: "resend_or_cloudflare_email_provider_pending",
          providerMode: "dry_run_contract_only_no_provider_call",
          providerCallEnabled: false,
          deliveryAttemptsCreated: false,
          deliveryResultsCreated: false,
          deliveryStatusWebhooksProcessed: false,
    ...
    const replaySequenceProviderCallReadinessResponse = await page.request.post(
      audienceSequenceProviderCallReadinessApiRoute,
      {
        data: {
          queueConsumerReadinessId: sequenceQueueConsumerReadinessPayload.record.id,
    ...
    expect(
      replaySequenceProviderCallReadinessResponse.ok(),
      await replaySequenceProviderCallReadinessResponse.text(),
    ).toBeTruthy();
    await expect(replaySequenceProviderCallReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: true,
        duplicate: true,
        record: expect.objectContaining({ id: sequenceProviderCallReadinessPayload.record.id }),
      }),
    );
    const staleSequenceProviderCallReadinessResponse = await page.request.post(
      audienceSequenceProviderCallReadinessApiRoute,
      {
        data: {
          queueConsumerReadinessId: sequenceQueueConsumerReadinessPayload.record.id,
          ...
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments + 1,
          confirmationText: audienceSequenceProviderCallReadinessConfirmationText,
          idempotencyKey: `playwright-sequence-provider-call-readiness-stale-${Date.now()}`,
        },
      },
    );
    expect(staleSequenceProviderCallReadinessResponse.status()).toBe(409);
    await expect(staleSequenceProviderCallReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: false,
        code: "stale_readiness_count",
        currentReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
    ...
    const sequenceDeliveryAttemptReadinessIdempotencyKey = `playwright-sequence-delivery-attempt-readiness-${Date.now()}`;
    const missingSequenceDeliveryAttemptConfirmationResponse = await page.request.post(
      audienceSequenceDeliveryAttemptReadinessApiRoute,
      {
        data: {
          ...
          expectedSequenceStatus: sequenceRecord.status,
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          confirmationText: "Attempt sequence delivery now",
          idempotencyKey: `${sequenceDeliveryAttemptReadinessIdempotencyKey}-missing-confirmation`,
        },
      },
    );
    expect(missingSequenceDeliveryAttemptConfirmationResponse.status()).toBe(400);
    await expect(missingSequenceDeliveryAttemptConfirmationResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "confirmation_required" }),
    );

    const missingSequenceDeliveryAttemptProviderCallResponse = await page.request.post(
      audienceSequenceDeliveryAttemptReadinessApiRoute,
      {
        data: {
          providerCallReadinessId: "sequence-provider-call-readiness-not-current",
          sequenceId: sequenceRecord.sequenceId,
          expectedWorkspaceRevisionId: sequenceReadiness.workspace.revisionId,
          expectedSequenceStatus: sequenceRecord.status,
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          confirmationText: audienceSequenceDeliveryAttemptReadinessConfirmationText,
          idempotencyKey: `${sequenceDeliveryAttemptReadinessIdempotencyKey}-missing-provider-call`,
        },
      },
    );
    expect(missingSequenceDeliveryAttemptProviderCallResponse.status()).toBe(404);
    await expect(missingSequenceDeliveryAttemptProviderCallResponse.json()).resolves.toEqual(
      expect.objectContaining({ ok: false, code: "provider_call_readiness_not_found" }),
    );

    const sequenceDeliveryAttemptReadinessResponse = await page.request.post(
      audienceSequenceDeliveryAttemptReadinessApiRoute,
      {
        data: {
          providerCallReadinessId: sequenceProviderCallReadinessPayload.record.id,
          ...
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          confirmationText: audienceSequenceDeliveryAttemptReadinessConfirmationText,
          idempotencyKey: sequenceDeliveryAttemptReadinessIdempotencyKey,
        },
      },
    );
    expect(
      sequenceDeliveryAttemptReadinessResponse.ok(),
      await sequenceDeliveryAttemptReadinessResponse.text(),
    ).toBeTruthy();
    const sequenceDeliveryAttemptReadinessPayload = await sequenceDeliveryAttemptReadinessResponse.json();
    expect(sequenceDeliveryAttemptReadinessPayload).toEqual(
      expect.objectContaining({
        ok: true,
        ...
        record: expect.objectContaining({
          ...
          expectedSequenceStatus: sequenceRecord.status,
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
          expectedProviderCallReadinessStatus: audienceSequenceProviderCallReadinessStatus,
          dryRunMessageCount: sequenceReadiness.counts.readyEnrollments,
          providerName: "resend_or_cloudflare_email_provider_pending",
          providerMode: "dry_run_contract_only_no_delivery_attempt",
          deliveryAttemptEnabled: false,
          deliveryAttemptsCreated: false,
          deliveryResultsCreated: false,
          deliveryStatusWebhooksProcessed: false,
    ...
    const replaySequenceDeliveryAttemptReadinessResponse = await page.request.post(
      audienceSequenceDeliveryAttemptReadinessApiRoute,
      {
        data: {
          providerCallReadinessId: sequenceProviderCallReadinessPayload.record.id,
    ...
    expect(
      replaySequenceDeliveryAttemptReadinessResponse.ok(),
      await replaySequenceDeliveryAttemptReadinessResponse.text(),
    ).toBeTruthy();
    await expect(replaySequenceDeliveryAttemptReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: true,
        duplicate: true,
        record: expect.objectContaining({ id: sequenceDeliveryAttemptReadinessPayload.record.id }),
      }),
    );
    const staleSequenceDeliveryAttemptReadinessResponse = await page.request.post(
      audienceSequenceDeliveryAttemptReadinessApiRoute,
      {
        data: {
          providerCallReadinessId: sequenceProviderCallReadinessPayload.record.id,
          ...
          expectedReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments + 1,
          confirmationText: audienceSequenceDeliveryAttemptReadinessConfirmationText,
          idempotencyKey: `playwright-sequence-delivery-attempt-readiness-stale-${Date.now()}`,
        },
      },
    );
    expect(staleSequenceDeliveryAttemptReadinessResponse.status()).toBe(409);
    await expect(staleSequenceDeliveryAttemptReadinessResponse.json()).resolves.toEqual(
      expect.objectContaining({
        ok: false,
        code: "stale_readiness_count",
        currentReadyEnrollmentCount: sequenceReadiness.counts.readyEnrollments,
    ...
    expect(audienceSourceAfterSchedulePayload.audienceSequenceScheduleIntents.counts.scheduleIntents).toBeGreaterThanOrEqual(1);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceScheduleIntents.counts.deliveryQueueRowsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceScheduleIntents.counts.recipientPayloadsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceScheduleIntents.counts.personalizedBodiesCreatedRecords,
    ).toBe(0);
    expect(audienceSourceAfterSchedulePayload.audienceSequenceScheduleIntents.counts.unsubscribeUrlsCreatedRecords).toBe(
      0,
    );
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceScheduleIntents.counts.providerMessageIdsCreatedRecords,
    ).toBe(0);
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceScheduleIntents)).not.toContain(
      "m@rkmoriarty.com",
    );
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceScheduleIntents)).not.toContain(
      audienceEmail,
    );
    expect(audienceSourceAfterSchedulePayload.audienceSequenceDeliveryBatches.counts.deliveryBatches).toBeGreaterThanOrEqual(1);
    expect(audienceSourceAfterSchedulePayload.audienceSequenceDeliveryBatches.counts.providerSendEnabledBatches).toBe(0);
    expect(audienceSourceAfterSchedulePayload.audienceSequenceDeliveryBatches.counts.deliveryQueueRowsCreatedBatches).toBe(0);
    expect(audienceSourceAfterSchedulePayload.audienceSequenceDeliveryBatches.counts.recipientPayloadsCreatedBatches).toBe(0);
    expect(audienceSourceAfterSchedulePayload.audienceSequenceDeliveryBatches.counts.personalizedBodiesCreatedBatches).toBe(0);
    expect(audienceSourceAfterSchedulePayload.audienceSequenceDeliveryBatches.counts.unsubscribeUrlsCreatedBatches).toBe(0);
    expect(audienceSourceAfterSchedulePayload.audienceSequenceDeliveryBatches.counts.providerMessageIdsCreatedBatches).toBe(0);
    ...
    expect(audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights.counts.dryRunPreflights).toBeGreaterThanOrEqual(1);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights.counts.providerSendEnabledRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights.counts.deliveryQueueRowsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights.counts.cloudflareQueueMessagesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights.counts.queuePayloadBodiesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights.counts.recipientPayloadsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights.counts.personalizedBodiesCreatedRecords,
    ).toBe(0);
    expect(audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights.counts.unsubscribeUrlsCreatedRecords).toBe(
      0,
    );
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights.counts.providerResponsesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights.counts.providerMessageIdsCreatedRecords,
    ).toBe(0);
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights)).not.toContain(
      "m@rkmoriarty.com",
    );
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceDispatchPreflights)).not.toContain(
      audienceEmail,
    );
    expect(audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts.counts.dryRunAttempts).toBeGreaterThanOrEqual(1);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts.counts.providerSendEnabledRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts.counts.deliveryQueueRowsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts.counts.cloudflareQueueMessagesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts.counts.queuePayloadBodiesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts.counts.recipientPayloadsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts.counts.personalizedBodiesCreatedRecords,
    ).toBe(0);
    expect(audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts.counts.unsubscribeUrlsCreatedRecords).toBe(
      0,
    );
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts.counts.providerResponsesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts.counts.providerMessageIdsCreatedRecords,
    ).toBe(0);
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts)).not.toContain(
      "m@rkmoriarty.com",
    );
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceDispatchAttempts)).not.toContain(
      audienceEmail,
    ...
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness.counts.queueProducerReadinessRecords,
    ).toBeGreaterThanOrEqual(1);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness.counts
        .cloudflareQueueProducerEnabledRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness.counts
        .cloudflareQueueMessagesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness.counts.queuePayloadBodiesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness.counts.recipientPayloadsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness.counts.personalizedBodiesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness.counts.unsubscribeUrlsCreatedRecords,
    ).toBe(0);
    expect(audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness.counts.providerSendEnabledRecords).toBe(
      0,
    );
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness.counts.providerResponsesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness.counts.providerMessageIdsCreatedRecords,
    ).toBe(0);
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness)).not.toContain(
      "m@rkmoriarty.com",
    );
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceQueueProducerReadiness)).not.toContain(
      audienceEmail,
    ...
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts.queueConsumerReadinessRecords,
    ).toBeGreaterThanOrEqual(1);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts
    ...
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts
        .queueRetryRecordsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts
        .queueDeadLetterRecordsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts.queuePayloadBodiesReadRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts.queuePayloadBodiesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts.recipientPayloadsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts.personalizedBodiesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts.unsubscribeUrlsCreatedRecords,
    ).toBe(0);
    expect(audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts.providerSendEnabledRecords).toBe(
      0,
    );
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts.providerResponsesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness.counts.providerMessageIdsCreatedRecords,
    ).toBe(0);
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness)).not.toContain(
      "m@rkmoriarty.com",
    );
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceQueueConsumerReadiness)).not.toContain(
      audienceEmail,
    ...
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness.counts.providerCallReadinessRecords,
    ).toBeGreaterThanOrEqual(1);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness.counts.providerCallEnabledRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness.counts.deliveryAttemptsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness.counts.deliveryResultsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness.counts
        .deliveryStatusWebhooksProcessedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness.counts.deliveryReceiptsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness.counts
        .cloudflareQueueConsumerEnabledRecords,
    ...
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness.counts.queuePayloadBodiesReadRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness.counts.providerSendEnabledRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness.counts.providerResponsesCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness.counts.providerMessageIdsCreatedRecords,
    ).toBe(0);
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness)).not.toContain(
      "m@rkmoriarty.com",
    );
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceProviderCallReadiness)).not.toContain(
      audienceEmail,
    ...
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDeliveryAttemptReadiness.counts
        .deliveryAttemptReadinessRecords,
    ).toBeGreaterThanOrEqual(1);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDeliveryAttemptReadiness.counts
        .deliveryAttemptEnabledRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDeliveryAttemptReadiness.counts.deliveryAttemptsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDeliveryAttemptReadiness.counts.deliveryResultsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDeliveryAttemptReadiness.counts
        .deliveryStatusWebhooksProcessedRecords,
    ...
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDeliveryAttemptReadiness.counts.deliveryReceiptsCreatedRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDeliveryAttemptReadiness.counts
        .cloudflareQueueConsumerEnabledRecords,
    ...
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDeliveryAttemptReadiness.counts.providerSendEnabledRecords,
    ).toBe(0);
    expect(
      audienceSourceAfterSchedulePayload.audienceSequenceDeliveryAttemptReadiness.counts
    ...
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceDeliveryAttemptReadiness)).not.toContain(
      "m@rkmoriarty.com",
    );
    expect(JSON.stringify(audienceSourceAfterSchedulePayload.audienceSequenceDeliveryAttemptReadiness)).not.toContain(
      audienceEmail,
    ...
    await expect(page.getByRole("heading", { name: "Sequence batches", exact: true })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: "Sequence intents" })).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Owner sequence intent is recorded before any queue payload exists" }),
    ).toBeVisible();
    ...
    await expect(page.getByText("Sequence dispatch preflight dry runs", { exact: true })).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Sequence dispatch gates are checked before Queue handoff" }),
    ).toBeVisible();
    await expect(page.getByRole("button", { name: /Record sequence dispatch preflight/i }).first()).toBeVisible();
    await expect(page.getByText("Sequence dispatch attempt receipts", { exact: true })).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Sequence handoff stays receipt-only before Queue producers exist" }),
    ).toBeVisible();
    await expect(page.getByRole("button", { name: /Record sequence dispatch attempt/i }).first()).toBeVisible();
    await expect(page.getByText("Sequence Queue producer readiness", { exact: true })).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Sequence producers stay disabled until payload and consumer gates pass" }),
    ).toBeVisible();
    await expect(page.getByRole("button", { name: /Record sequence Queue producer readiness/i }).first()).toBeVisible();
    await expect(page.getByText("Sequence Queue consumer readiness", { exact: true })).toBeVisible();
    await expect(
      page.getByRole("heading", {
        name: "Sequence consumers stay disabled until ack and provider-handoff gates pass",
      }),
    ).toBeVisible();
    await expect(page.getByRole("button", { name: /Record sequence Queue consumer readiness/i }).first()).toBeVisible();
    await expect(page.getByRole("heading", { name: "Sequence consumer handoff is readiness-only" }).first()).toBeVisible();
    await expect(page.getByRole("heading", { name: "Sequence producer handoff is readiness-only" }).first()).toBeVisible();
    await expect(page.getByText("Sequence provider-call readiness", { exact: true })).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Provider calls stay disabled until response and receipt gates pass" }),
    ).toBeVisible();
    await expect(page.getByRole("button", { name: /Record sequence provider-call readiness/i }).first()).toBeVisible();
    await expect(page.getByRole("heading", { name: "Sequence provider handoff is readiness-only" }).first()).toBeVisible();
    await expect(page.getByText("Sequence delivery-attempt readiness", { exact: true })).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Delivery attempts stay disabled until result and receipt gates pass" }),
    ).toBeVisible();
    await expect(page.getByRole("button", { name: /Record sequence delivery-attempt readiness/i }).first()).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Sequence delivery attempt is readiness-only" }).first(),
    ).toBeVisible();
    await expect(page.getByRole("heading", { name: "Preflights prove import safety before contact writes" })).toBeVisible();
    await expect(page.getByRole("button", { name: /Record import preflight/i })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: "Consumer handoff is readiness-only", exact: true })).toBeVisible();
