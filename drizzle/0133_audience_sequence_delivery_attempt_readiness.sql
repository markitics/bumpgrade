CREATE TABLE IF NOT EXISTS audience_sequence_delivery_attempt_readiness (
  id TEXT PRIMARY KEY,
  ...
  status TEXT NOT NULL DEFAULT 'sequence_delivery_attempt_readiness_recorded',
  queue_name TEXT NOT NULL,
  provider_name TEXT NOT NULL,
  provider_mode TEXT NOT NULL,
  delivery_attempt_gate_status TEXT NOT NULL,
  provider_call_dependency_status TEXT NOT NULL,
  ...
  expected_provider_call_readiness_status TEXT NOT NULL,
  ...
  delivery_attempt_enabled INTEGER NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX IF NOT EXISTS audience_sequence_delivery_attempt_readiness_idempotency_unique
  ON audience_sequence_delivery_attempt_readiness(idempotency_key);

CREATE INDEX IF NOT EXISTS audience_sequence_delivery_attempt_readiness_provider_call_idx
  ON audience_sequence_delivery_attempt_readiness(provider_call_readiness_id);

CREATE INDEX IF NOT EXISTS audience_sequence_delivery_attempt_readiness_attempt_idx
  ON audience_sequence_delivery_attempt_readiness(dispatch_attempt_id);

CREATE INDEX IF NOT EXISTS audience_sequence_delivery_attempt_readiness_sequence_status_idx
  ON audience_sequence_delivery_attempt_readiness(sequence_id, status);
...
SET
  ...
  public_evidence_json = json_insert(
    ...
    '$[#]',
    'Issue #372 adds owner-reviewed sequence delivery-attempt readiness gates from current provider-call readiness without enabling delivery attempts, sends, responses, message IDs, delivery results, webhooks, receipts, Cloudflare Queue consumption, Queue acknowledgements, retry/dead-letter rows, queue payload body reads, recipient payloads, personalized bodies, or unsubscribe URLs.'
  ),
  next_milestone = 'Add real contact imports, sequence scheduling, Cloudflare Queue producers/consumers, provider calls, and delivery attempts only after sender-domain, provider-event, provider rate-limit, provider response, send-payload, Queue producer, Queue consumer, provider-call, delivery-attempt, import redaction, unsubscribe footer, suppression, audit checks, sequence delivery-readiness, sequence delivery-batch gates, sequence queue-message gates, sequence dispatch-preflight gates, and sequence dispatch-attempt receipts stay explicit.',
  updated_at = unixepoch()
WHERE id = 'roadmap-email-automation';

UPDATE admin_user_journeys
SET
  issue_numbers_json = json_insert(COALESCE(issue_numbers_json, '[]'), '$[#]', 372),
  user_goal = REPLACE(
    user_goal,
    'owner-reviewed sequence provider-call readiness gates, broadcast readiness',
    'owner-reviewed sequence provider-call readiness gates, owner-reviewed sequence delivery-attempt readiness gates, broadcast readiness'
  ),
  ...
  happy_path_json = json_insert(
    ...
    '$[#]',
    'Record sequence delivery-attempt readiness from the current sequence provider-call readiness record after workspace revision, sequence status, readiness, provider-call, provider-response, message-ID, delivery-attempt, backpressure, idempotency, audit, and redaction gates are checked.'
  ),
  edge_cases_json = json_insert(
    COALESCE(edge_cases_json, '[]'),
    '$[#]',
    'Sequence delivery-attempt readiness records owner evidence only; it does not call providers, send messages, create provider responses, create provider message IDs, create delivery attempts or results, process webhooks, create receipts, consume or ack Queue messages, create retry/dead-letter rows, read queue payload bodies, create recipient payloads, personalized bodies, unsubscribe URLs, or public agent sequence writes.'
  ),
  agent_access = REPLACE(
    agent_access,
    'record sequence provider-call readiness gates, view broadcast readiness',
    'record sequence provider-call readiness gates, record sequence delivery-attempt readiness gates, view broadcast readiness'
  ),
  validation_json = json_insert(
    COALESCE(validation_json, '[]'),
    '$[#]',
    'Issue #372 records the owner-confirmed sequence delivery-attempt readiness path after current sequence provider-call readiness, workspace revision, sequence status, readiness, provider-response, message-ID, delivery-attempt, backpressure, idempotency, audit, and redaction checks.'
  ),
  updated_at = unixepoch()
