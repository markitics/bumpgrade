CREATE TABLE IF NOT EXISTS analytics_notification_send_payload_readiness_records (
  id TEXT PRIMARY KEY,
  ...
  content_consent_readiness_id TEXT NOT NULL,
  record_kind TEXT NOT NULL,
  notification_send_payload_readiness_disposition TEXT NOT NULL,
  ...
  expected_notification_content_consent_readiness_status TEXT NOT NULL,
  ...
  owner_send_payload_readiness_recorded INTEGER NOT NULL DEFAULT 0,
  payload_shape_reviewed INTEGER NOT NULL DEFAULT 0,
  unsubscribe_footer_reviewed INTEGER NOT NULL DEFAULT 0,
  consent_suppression_recheck_reviewed INTEGER NOT NULL DEFAULT 0,
  recipient_scope_reviewed INTEGER NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX IF NOT EXISTS analytics_notification_send_payload_readiness_idempotency_unique
  ON analytics_notification_send_payload_readiness_records(idempotency_key);
CREATE INDEX IF NOT EXISTS analytics_notification_send_payload_readiness_dashboard_time_idx
  ON analytics_notification_send_payload_readiness_records(dashboard_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_send_payload_readiness_readiness_channel_idx
  ON analytics_notification_send_payload_readiness_records(readiness_id, channel_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_send_payload_readiness_content_consent_idx
  ON analytics_notification_send_payload_readiness_records(content_consent_readiness_id, created_at);
