CREATE TABLE IF NOT EXISTS audience_broadcast_queue_consumer_readiness (
  id TEXT PRIMARY KEY,
  draft_id TEXT NOT NULL,
  status TEXT NOT NULL DEFAULT 'queue_consumer_readiness_ready',
  ...
  queue_producer_gate_status TEXT NOT NULL,
  ...
  cloudflare_queue_consumers_enabled INTEGER NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS audience_broadcast_queue_consumer_readiness_draft_status_idx
  ON audience_broadcast_queue_consumer_readiness(draft_id, status);

INSERT INTO audience_broadcast_queue_consumer_readiness (
  ...
  provider_message_ids_created, raw_payload_bodies_stored, metadata_json,
  created_at, updated_at
) VALUES (
  'broadcast-queue-consumer-readiness-launch-window',
  'broadcast-draft-launch-window',
  'queue_consumer_readiness_ready',
  'audience-broadcast-delivery',
  'bumpgrade-audience-broadcast-delivery-consumer',
  'dry_run_contract_only_no_worker_queue_consumer',
  'blocked_until_queue_payload_ack_retry_and_dead_letter_contracts_are_verified',
  'blocked_until_queue_producer_readiness_issue_209_stays_current',
  'blocked_until_send_payload_readiness_verified_without_raw_payload_body_exposure',
  'Future consumers must acknowledge only after idempotent provider handoff and audit correlation are recorded; no ack path exists in this contract.',
  'Retries must preserve the original idempotency basis and fail closed when provider rate limits, sender-domain gates, or suppression rechecks are stale.',
  ...
  'Future consumers must key idempotency by Queue message id, dispatch attempt id, send payload readiness id, and recipient scope before any provider send is attempted.',
  'Future consumers must carry redacted audit correlation from schedule intent through Queue producer readiness, consumer readiness, provider response readiness, and provider-event processing.',
  'Provider handoff stays blocked until queue consumer, sender-domain, provider-event, provider rate-limit, provider response, send-payload, suppression, and unsubscribe gates pass together.',
  'blocked_until_queue_producer_readiness_issue_209_stays_current',
  ...
  0,
  0,
  '{"issue":211,"cloudflareQueueConsumersEnabled":false,"cloudflareQueueMessagesConsumed":false,"cloudflareQueueMessagesAcked":false,"queueRetryRecordsCreated":false,"queueDeadLetterRecordsCreated":false,"queuePayloadBodiesRead":false,"recipientPayloadsCreated":false,"providerSendEnabled":false,"providerResponsesCreated":false,"providerMessageIdsCreated":false,"rawPayloadBodiesStored":false}',
  unixepoch(),
  unixepoch()
  ...
  consumer_gate_status=excluded.consumer_gate_status,
  producer_dependency_status=excluded.producer_dependency_status,
  ...
  provider_handoff_policy=excluded.provider_handoff_policy,
  queue_producer_gate_status=excluded.queue_producer_gate_status,
  ...
  cloudflare_queue_consumers_enabled=excluded.cloudflare_queue_consumers_enabled,
  cloudflare_queue_messages_consumed=excluded.cloudflare_queue_messages_consumed,
  cloudflare_queue_messages_acked=excluded.cloudflare_queue_messages_acked,
  queue_retry_records_created=excluded.queue_retry_records_created,
  queue_dead_letter_records_created=excluded.queue_dead_letter_records_created,
  queue_payload_bodies_read=excluded.queue_payload_bodies_read,
...
SET
  issue_numbers_json = '[17,85,103,137,167,169,171,173,175,177,183,189,191,197,199,201,203,205,207,209,211]',
