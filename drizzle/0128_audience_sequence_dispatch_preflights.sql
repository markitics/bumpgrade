CREATE TABLE IF NOT EXISTS audience_sequence_dispatch_preflights (
  id TEXT PRIMARY KEY,
...
SET
  ...
  next_milestone = 'Add real contact imports, sequence scheduling, Cloudflare Queue producers, and consumers only after sender-domain, provider-event, provider rate-limit, provider response, send-payload, Queue producer, Queue consumer, import redaction, unsubscribe footer, suppression, audit checks, sequence delivery-readiness, sequence delivery-batch gates, sequence queue-message gates, sequence dispatch-preflight gates, delivery-batch gates, queue-message gates, dispatch preflight boundaries, and dispatch-attempt receipts stay explicit.',
  updated_at = unixepoch()
WHERE id = 'roadmap-email-automation';

UPDATE admin_user_journeys
SET
  issue_numbers_json = json_insert(COALESCE(issue_numbers_json, '[]'), '$[#]', 362),
  user_goal = REPLACE(
    user_goal,
    'dry-run sequence queue-message evidence, broadcast readiness',
    'dry-run sequence queue-message evidence, dry-run sequence dispatch preflight evidence, broadcast readiness'
  ),
  ...
  edge_cases_json = json_insert(
    ...
    '$[#]',
    'Sequence dispatch preflights record aggregate owner evidence only; they do not dispatch Cloudflare Queue messages, create queue payload bodies, delivery queue rows, recipient payloads, personalized bodies, unsubscribe URLs, provider sends, provider responses, provider message IDs, or public agent sequence writes.'
  ),
  agent_access = REPLACE(
    agent_access,
    'record dry-run sequence queue-message evidence, view broadcast readiness',
    'record dry-run sequence queue-message evidence, record dry-run sequence dispatch preflight evidence, view broadcast readiness'
  ),
  validation_json = json_insert(
    COALESCE(validation_json, '[]'),
    '$[#]',
    'Issue #362 records the owner-confirmed dry-run sequence dispatch preflight path after sequence queue-message, workspace revision, sequence status, readiness, provider-limit, suppression, unsubscribe-footer, sender-domain, idempotency, audit, and redaction checks.'
  ),
  updated_at = unixepoch()
