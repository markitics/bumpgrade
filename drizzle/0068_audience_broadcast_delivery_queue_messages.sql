CREATE TABLE IF NOT EXISTS audience_broadcast_delivery_queue_messages (
  id TEXT PRIMARY KEY,
...
CREATE UNIQUE INDEX IF NOT EXISTS audience_broadcast_delivery_queue_messages_idempotency_unique
  ON audience_broadcast_delivery_queue_messages(idempotency_key);

CREATE INDEX IF NOT EXISTS audience_broadcast_delivery_queue_messages_batch_idx
  ON audience_broadcast_delivery_queue_messages(delivery_batch_id);

CREATE INDEX IF NOT EXISTS audience_broadcast_delivery_queue_messages_draft_status_idx
  ON audience_broadcast_delivery_queue_messages(draft_id, status);
...
SET
  ...
  next_milestone = 'Add real Cloudflare Queue producers only after sender-domain, unsubscribe footer, suppression, provider-limit, audit, delivery-batch, queue-message, and dispatch gates stay explicit.',
  updated_at = unixepoch()
WHERE id = 'roadmap-email-automation';

UPDATE admin_user_journeys
SET
  issue_numbers_json = '[17,85,103,137,167,169,171,173,175,177,183,189]',
  ...
  validation_json = '["Playwright covers /audience/source-data, aggregate subscriber/suppression/timeline/broadcast readiness/schedule intent/preview safety/queue readiness/delivery-batch/queue-message redaction, /audience/indie-launch-waitlist, valid opt-in, unsubscribe, unknown-email suppression, owner /admin/audience inspection, owner CRM note creation, owner dry-run broadcast schedule intent creation, owner delivery-batch dry-run creation, owner queue-message dry-run creation, unauthorized schedule/delivery/queue-message rejection, stale readiness rejection, validation failures, duplicate idempotency, sitemap discovery, and agent manifest discovery.", "Issues #85, #103, #137, #167, #169, #171, #173, #175, #177, #183, and #189 record the audience automation source-data contract, first live opt-in capture path, owner subscriber inspection path, unsubscribe/suppression path, owner CRM timeline note path, broadcast readiness path, dry-run broadcast schedule intent path, preview/footer safety path, queue readiness path, delivery-batch dry-run path, and queue-message dry-run path."]',
  updated_at = unixepoch()
