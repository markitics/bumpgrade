CREATE TABLE IF NOT EXISTS audience_sequence_delivery_status_webhook_readiness (
  id TEXT PRIMARY KEY,
  ...
  status TEXT NOT NULL DEFAULT 'sequence_delivery_status_webhook_readiness_recorded',
  queue_name TEXT NOT NULL,
  provider_name TEXT NOT NULL,
  provider_mode TEXT NOT NULL,
  delivery_status_webhook_gate_status TEXT NOT NULL,
  delivery_result_dependency_status TEXT NOT NULL,
  ...
  expected_delivery_result_readiness_status TEXT NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS audience_sequence_delivery_status_webhook_readiness_idempotency_unique
  ON audience_sequence_delivery_status_webhook_readiness(idempotency_key);

CREATE INDEX IF NOT EXISTS audience_sequence_delivery_status_webhook_readiness_result_readiness_idx
  ON audience_sequence_delivery_status_webhook_readiness(delivery_result_readiness_id);

CREATE INDEX IF NOT EXISTS audience_sequence_delivery_status_webhook_readiness_attempt_readiness_idx
  ON audience_sequence_delivery_status_webhook_readiness(delivery_attempt_readiness_id);

CREATE INDEX IF NOT EXISTS audience_sequence_delivery_status_webhook_readiness_provider_call_idx
  ON audience_sequence_delivery_status_webhook_readiness(provider_call_readiness_id);

CREATE INDEX IF NOT EXISTS audience_sequence_delivery_status_webhook_readiness_attempt_idx
  ON audience_sequence_delivery_status_webhook_readiness(dispatch_attempt_id);

CREATE INDEX IF NOT EXISTS audience_sequence_delivery_status_webhook_readiness_sequence_status_idx
  ON audience_sequence_delivery_status_webhook_readiness(sequence_id, status);

UPDATE admin_roadmap_items
SET
  summary = 'Subscriber segments, live consent-backed opt-in capture, unsubscribe/suppression evidence, owner CRM timeline notes, aggregate sequence delivery readiness, dry-run sequence schedule intents, dry-run sequence delivery batches, dry-run sequence queue-message evidence, dry-run sequence dispatch preflight evidence, dry-run sequence dispatch attempt receipts, owner-reviewed sequence Queue producer readiness gates, owner-reviewed sequence Queue consumer readiness gates, owner-reviewed sequence provider-call readiness gates, owner-reviewed sequence delivery-attempt readiness gates, owner-reviewed sequence delivery-result readiness gates, owner-reviewed sequence delivery-status webhook readiness gates, broadcast draft readiness, dry-run broadcast schedule intents, preview/footer safety records, queue readiness contracts, delivery-batch dry runs, dry-run queue-message evidence, dispatch preflight evidence, dispatch attempt receipts, sender-domain readiness gates, provider-event readiness gates, provider rate-limit readiness gates, provider response readiness gates, send-payload readiness gates, Queue producer readiness gates, Queue consumer readiness gates, provider-call readiness gates, delivery-attempt readiness gates, delivery-result readiness gates, delivery-status webhook readiness gates, owner-confirmed import intents, owner-confirmed import preflights, lead magnets, tags, draft sequence enrollment evidence, broadcasts, sequences, consent, and CRM-lite state.',
  public_evidence_json = json_insert(
    COALESCE(public_evidence_json, '[]'),
    '$[#]',
    'Issue #376 adds owner-reviewed sequence delivery-status webhook readiness gates from current delivery-result readiness without enabling provider calls, sends, responses, message IDs, delivery attempts, delivery results, status webhooks, webhook payload reads, provider polling, receipts, Cloudflare Queue consumption, Queue acknowledgements, retry/dead-letter rows, queue payload body reads, recipient payloads, personalized bodies, or unsubscribe URLs.'
  ),
  next_milestone = 'Add real contact imports, sequence scheduling, Cloudflare Queue producers/consumers, provider calls, delivery attempts, delivery results, and delivery-status webhooks only after sender-domain, provider-event, provider rate-limit, provider response, send-payload, Queue producer, Queue consumer, provider-call, delivery-attempt, delivery-result, delivery-status webhook, import redaction, unsubscribe footer, suppression, audit checks, sequence delivery-readiness, sequence delivery-batch gates, sequence queue-message gates, sequence dispatch-preflight gates, and sequence dispatch-attempt receipts stay explicit.',
  updated_at = unixepoch()
WHERE id = 'roadmap-email-automation';

UPDATE admin_user_journeys
SET
  issue_numbers_json = json_insert(COALESCE(issue_numbers_json, '[]'), '$[#]', 376),
  user_goal = REPLACE(
    user_goal,
    'owner-reviewed sequence delivery-result readiness gates, broadcast readiness',
    'owner-reviewed sequence delivery-result readiness gates, owner-reviewed sequence delivery-status webhook readiness gates, broadcast readiness'
  ),
  ...
  happy_path_json = json_insert(
    ...
    '$[#]',
    'Record sequence delivery-status webhook readiness from the current sequence delivery-result readiness record after workspace revision, sequence status, readiness, delivery-result, webhook, polling, receipt, backpressure, idempotency, audit, and redaction gates are checked.'
  ),
  edge_cases_json = json_insert(
    COALESCE(edge_cases_json, '[]'),
    '$[#]',
    'Sequence delivery-status webhook readiness records owner evidence only; it does not call providers, send messages, create provider responses, create provider message IDs, create delivery attempts or results, process webhooks, read or create webhook payloads, poll providers, create receipts, consume or ack Queue messages, create retry/dead-letter rows, read queue payload bodies, create recipient payloads, personalized bodies, unsubscribe URLs, or public agent sequence writes.'
  ),
  agent_access = REPLACE(
    agent_access,
    'record sequence delivery-result readiness gates, view broadcast readiness',
    'record sequence delivery-result readiness gates, record sequence delivery-status webhook readiness gates, view broadcast readiness'
  ),
  validation_json = json_insert(
    COALESCE(validation_json, '[]'),
    '$[#]',
    'Issue #376 records the owner-confirmed sequence delivery-status webhook readiness path after current sequence delivery-result readiness, workspace revision, sequence status, readiness, webhook, polling, receipt, backpressure, idempotency, audit, and redaction checks.'
  ),
  updated_at = unixepoch()
