CREATE TABLE IF NOT EXISTS audience_sequence_provider_call_readiness (
  id TEXT PRIMARY KEY,
  ...
  status TEXT NOT NULL DEFAULT 'sequence_provider_call_readiness_recorded',
  queue_name TEXT NOT NULL,
  provider_name TEXT NOT NULL,
  provider_mode TEXT NOT NULL,
  provider_call_gate_status TEXT NOT NULL,
  queue_consumer_dependency_status TEXT NOT NULL,
  ...
  expected_queue_consumer_readiness_status TEXT NOT NULL,
  ...
  provider_call_enabled INTEGER NOT NULL DEFAULT 0,
...
SET
  ...
  happy_path_json = json_insert(
    ...
    '$[#]',
    'Record sequence provider-call readiness from the current sequence Queue consumer readiness record after workspace revision, sequence status, readiness, Queue consumer, provider-response, message-ID, delivery-attempt, backpressure, idempotency, audit, and redaction gates are checked.'
  ),
  edge_cases_json = json_insert(
    COALESCE(edge_cases_json, '[]'),
    '$[#]',
    'Sequence provider-call readiness records owner evidence only; it does not call providers, send messages, create provider responses, create provider message IDs, create delivery attempts or results, process webhooks, create receipts, consume or ack Queue messages, create retry/dead-letter rows, read queue payload bodies, create recipient payloads, personalized bodies, unsubscribe URLs, or public agent sequence writes.'
  ),
  agent_access = REPLACE(
    agent_access,
    'record sequence Queue consumer readiness gates, view broadcast readiness',
    'record sequence Queue consumer readiness gates, record sequence provider-call readiness gates, view broadcast readiness'
  ),
  validation_json = json_insert(
    COALESCE(validation_json, '[]'),
    '$[#]',
    'Issue #370 records the owner-confirmed sequence provider-call readiness path after current sequence Queue consumer readiness, workspace revision, sequence status, readiness, provider-response, message-ID, delivery-attempt, backpressure, idempotency, audit, and redaction checks.'
  ),
  updated_at = unixepoch()
