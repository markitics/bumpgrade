CREATE TABLE IF NOT EXISTS product_offer_access_grant_intents (
  id TEXT PRIMARY KEY,
  ...
  status TEXT NOT NULL DEFAULT 'test_access_grant_recorded',
  confirmation_text_sha256 TEXT NOT NULL,
  offer_link_created INTEGER NOT NULL DEFAULT 1,
  test_checkout_intent_created INTEGER NOT NULL DEFAULT 1,
  entitlement_grant_created INTEGER NOT NULL DEFAULT 1,
...
CREATE UNIQUE INDEX IF NOT EXISTS product_offer_access_grant_intents_idempotency_unique
  ON product_offer_access_grant_intents(idempotency_key);
CREATE INDEX IF NOT EXISTS product_offer_access_grant_intents_product_status_idx
  ON product_offer_access_grant_intents(product_id, status);
CREATE INDEX IF NOT EXISTS product_offer_access_grant_intents_checkout_idx
  ON product_offer_access_grant_intents(checkout_intent_id);
CREATE INDEX IF NOT EXISTS product_offer_access_grant_intents_created_idx
  ON product_offer_access_grant_intents(created_at);

UPDATE admin_roadmap_items
SET
  summary = 'Digital product records, owner-confirmed draft product creation, owner-confirmed test offer/funnel linking, test purchase/access grant evidence, product/access source data, sandbox webhook-backed entitlement grants, owner entitlement inspection, customer entitlement lookup, private R2-backed fixture delivery with redemption revalidation, owner-confirmed private asset upload intents, owner-confirmed non-destructive revocation intents, protected content readiness, checkout-intent-scoped protected fixture delivery, subscription-backed membership access, fulfillment task evidence, access rules, and subscriptions.',
  public_evidence_json = json_array(
    ...
    'Issue #405 adds owner-confirmed test offer/funnel links and test access grants for owner-created products without Stripe Checkout Sessions, live charges, published offer copy, or public buyer exposure.'
  ),
  next_milestone = 'Use the owner-created product test grant evidence to wire real offer/funnel product selection and customer delivery gates before closing issue #16.',
  updated_at = unixepoch()
WHERE id = 'roadmap-products-access';

UPDATE admin_user_journeys
SET
  issue_numbers_json = json_array(16,83,101,139,141,143,146,147,151,179,181,185,187,251,403,405),
  ...
  user_goal = 'Inspect products, create a draft product record as an owner, link it to test offer/funnel IDs, create a synthetic paid test checkout/access grant, and inspect assets, access rules, entitlement templates, sandbox entitlement grant mappings, subscription-backed membership access, owner entitlement rows, owner-confirmed revocation intents, and protected fixture delivery checks before real private content storage is enabled.',
  happy_path_json = json_array(
    'Fetch /products/source-data.',
    'Find seeded product types, owner product creation contract, owner product offer/access test grant contract, asset IDs, access rules, entitlement templates, sandbox grant mappings, subscription membership access contract, aggregate entitlement inspection counts, customer lookup contract, private R2-backed delivery contract, owner-upload intent contract, owner-confirmed revocation intent contract, protected content readiness, protected fixture delivery contract, revision ID, and write boundary.',
    'Open /products/indie-launch-library to inspect the public preview.',
    'Open /admin/products as a verified owner to create a draft product record, link it to test offer/funnel IDs, create a synthetic paid test checkout/access grant, and inspect private buyer entitlement rows, checkout state, product and price context, queued fulfillment evidence, revocation intent records, and protected content readiness.',
  ...
  edge_cases_json = json_array(
    ...
    'Owner-created product offer/access grants create only test offer/funnel evidence, a synthetic paid checkout intent, an entitlement row, fulfillment task evidence, and audit evidence.',
    'Public /products/source-data exposes aggregate product creation and test grant counts plus redaction flags, not owner emails, actor user IDs, buyer emails, buyer hashes, checkout intent IDs, entitlement IDs, idempotency keys, metadata JSON, or raw Stripe IDs.',
  ...
  agent_access = 'Agents can read /products/source-data, aggregate product creation counts, aggregate owner-created product test grant counts, aggregate entitlement inspection counts, the preview route, the customer-safe checkout intent lookup contract, the short-lived private R2-backed download-token boundary with redemption revalidation, the owner-authenticated private asset upload intent boundary, owner-confirmed non-destructive revocation intent evidence, protected content readiness, protected fixture delivery checks, and subscription-backed membership access state. Owner sessions can create draft product records, link owner-created products to test offer/funnel IDs, create synthetic paid test checkout/access grants, inspect private buyer entitlement rows, revocation intent records, and protected content readiness in /admin/products, create small private asset upload records, and record non-destructive revocation intents only through exact-confirmed, idempotent, stale-state-checked writes. Trusted paid sandbox webhooks can grant seeded entitlement rows and trusted Stripe Billing webhooks can sync membership access; direct customer delivery of arbitrary uploads, destructive revocation, Stripe product or price creation, live offer/funnel publishing, real protected media delivery, Customer Portal actions, and subscription mutations require later APIs.',
  validation_json = json_array(
    'Playwright covers /products/source-data, product creation contract redaction, owner draft product creation, owner-created product offer/access grant contract redaction, idempotent replay, conflict rejection, aggregate product creation and test grant source-data exposure, aggregate entitlement inspection redaction, /products/indie-launch-library, customer /products/entitlements lookup, private R2-backed token delivery, current checkout-state revalidation, replay rejection, protected fixture delivery, protected fixture stale-state rejection, subscription-backed membership access activation/inactivation, owner private asset upload intent creation, owner-confirmed non-destructive revocation intent creation, idempotent replay, stale-state rejection, unauthorized rejection, owner /admin/products inspection, revocation intent readiness, protected content readiness, sitemap discovery, and agent manifest read-contract discovery.',
    'Issue #403 records the first owner-confirmed draft product creation path.',
    'Issue #405 records owner-created product test offer/funnel linking and test access grant evidence.'
  ),
  updated_at = unixepoch()
...
UPDATE admin_user_journeys
SET
  issue_numbers_json = json_array(16,83,99,101,139,141,143,146,147,151,179,181,185,187,251,403,405),
  source_evidence_json = json_array(
    'https://bumpgrade.com/products/source-data',
    'https://bumpgrade.com/products/entitlements',
    'https://bumpgrade.com/api/products/entitlements',
  ...
  user_goal = 'Confirm that paid sandbox checkout webhooks can grant product entitlements, trusted Stripe Billing state can sync membership access, owner-created draft products can produce test offer/funnel and access grant evidence, queued fulfillment evidence stays public-safe, owner-confirmed revocation intent records stay non-destructive, and protected fixtures return only for active eligible checkout-linked entitlements.',
  happy_path_json = json_array(
    ...
    'Open /admin/products as a verified owner to inspect the draft product, test offer/access grant, entitlement rows, queued fulfillment evidence, revocation intents, and protected content readiness.'
  ),
  edge_cases_json = json_array(
    ...
    'Duplicate owner-created product offer/access idempotency keys replay only identical requests.',
  ...
  agent_access = 'Agents can read /products/source-data, /api/products/entitlements, /api/products/download-tokens, /api/products/protected-content, and /commerce/source-data to understand the draft product creation contract, owner-created product test offer/access grant contract, entitlement grant contract, customer-safe lookup, short-lived private R2-backed fixture delivery with redemption revalidation, owner-upload intent boundary, owner-confirmed non-destructive revocation intent boundary, protected content readiness, protected fixture delivery checks, subscription-backed membership access, and aggregate inspection counts. Owner sessions can create draft product records, create owner-created product test offer/access grants, inspect private buyer entitlement rows, revocation intent records, and protected content readiness in /admin/products, create small private asset upload records, and record revocation intents via exact-confirmed, idempotent, stale-state-checked writes. Agents cannot grant live access, destructively revoke, expose private buyer data, deliver arbitrary protected bodies, deliver arbitrary uploads to customers, create Stripe products or prices, mutate live offers/funnels, mutate subscriptions, create Customer Portal sessions, or issue signed object URLs without future authenticated confirmed-write APIs.',
  validation_json = json_array(
    'Playwright covers source-data output, product creation contract redaction, owner draft product creation, owner-created product offer/access grant contract redaction, duplicate handling, aggregate redaction, a paid test checkout webhook that creates entitlement rows, customer-safe lookup, private R2-backed token delivery, current checkout-state revalidation, replay rejection, protected fixture delivery and stale-state rejection, subscription-backed membership access activation/inactivation, owner private asset upload intent creation, owner-confirmed non-destructive revocation intent creation, idempotent replay, stale-state rejection, unauthorized rejection, owner /admin/products inspection, revocation intent readiness, protected content readiness, and duplicate webhook idempotency.',
