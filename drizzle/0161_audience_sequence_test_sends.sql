CREATE TABLE IF NOT EXISTS audience_sequence_test_sends (
  id TEXT PRIMARY KEY,
  sequence_id TEXT NOT NULL,
  step_id TEXT NOT NULL,
  status TEXT NOT NULL DEFAULT 'owner_sequence_test_send_recorded',
  ...
  queue_messages_created INTEGER NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX IF NOT EXISTS audience_sequence_test_sends_idempotency_unique
  ON audience_sequence_test_sends(idempotency_key);

CREATE INDEX IF NOT EXISTS audience_sequence_test_sends_sequence_status_idx
  ON audience_sequence_test_sends(sequence_id, status);

CREATE INDEX IF NOT EXISTS audience_sequence_test_sends_created_idx
  ON audience_sequence_test_sends(created_at);

UPDATE admin_roadmap_items
SET
  summary = 'Subscriber segments, consent-backed opt-in capture, suppression evidence, owner CRM notes, sequence and broadcast dry-run gates, sender/provider/queue readiness, and owner-only broadcast plus sequence test-send execution paths.',
  public_evidence_json = '["Tracked by issue #17 and post-MVP execution issue #420.", "Issue #420 adds owner-confirmed broadcast and sequence test-send paths that send only to the verified owner-session email through the configured Cloudflare Email binding or test capture.", "The sequence test-send record stores recipient and actor hashes, confirmation hash, idempotency key, audit correlation, provider status, and redaction flags without raw recipient email, subscriber payloads, Queue messages, provider message IDs, or public agent sends."]',
...
SET
  user_goal = 'Inspect opt-ins, suppression, CRM notes, broadcast and sequence dry-run gates, readiness boundaries, import/export planning, and owner-only test-send paths before subscriber delivery exists.',
