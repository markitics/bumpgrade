CREATE TABLE IF NOT EXISTS analytics_notification_provider_polling_readiness_records (
  id TEXT PRIMARY KEY,
  ...
  notification_provider_polling_readiness_disposition TEXT NOT NULL,
  ...
  expected_notification_delivery_status_webhook_readiness_status TEXT NOT NULL,
  ...
  owner_provider_polling_readiness_recorded INTEGER NOT NULL DEFAULT 0,
  ...
  provider_polling_recorded INTEGER NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX IF NOT EXISTS analytics_notification_provider_polling_readiness_idempotency_unique
  ON analytics_notification_provider_polling_readiness_records(idempotency_key);
CREATE INDEX IF NOT EXISTS analytics_notification_provider_polling_readiness_dashboard_time_idx
  ON analytics_notification_provider_polling_readiness_records(dashboard_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_provider_polling_readiness_readiness_channel_idx
  ON analytics_notification_provider_polling_readiness_records(readiness_id, channel_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_provider_polling_readiness_send_payload_idx
  ON analytics_notification_provider_polling_readiness_records(send_payload_readiness_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_provider_polling_readiness_status_webhook_idx
  ON analytics_notification_provider_polling_readiness_records(delivery_status_webhook_readiness_id, created_at);
...
SET
  ...
  next_milestone = 'Add analytics notification receipt-payload readiness only after provider-polling readiness keeps provider sends/calls, provider responses, provider message IDs, delivery receipts, status webhooks, provider polling, provider secrets, sender credentials, recipients, email bodies, queues, customer alerts, and sends disabled.',
  updated_at = unixepoch()
WHERE id = 'roadmap-analytics-testing';

UPDATE admin_user_journeys
SET
  issue_numbers_json = json_array(18,87,105,107,119,121,123,125,127,129,261,263,265,267,269,271,284,286,288,290,292,294,297,299,301,303,305),
...
INSERT INTO admin_work_log_entries (
  ...
  'work-log-2026-05-21-analytics-notification-provider-polling-readiness',
  'Added analytics notification provider-polling readiness records',
  'Codex',
  'codex',
  'bumpgrade-bootstrap',
  'Continue the Bumpgrade parity build and add owner-reviewed analytics notification provider-polling readiness evidence for agents.',
  json_array(json_object('number',305,'url','https://github.com/markitics/bumpgrade/issues/305'), json_object('number',18,'url','https://github.com/markitics/bumpgrade/issues/18')),
  json_array(),
  ...
  json_array('src/lib/analytics-notification-provider-polling-readiness.ts','src/app/api/admin/analytics/notification-provider-polling-readiness/route.ts','src/app/analytics/source-data/route.ts','src/lib/agent-manifest.ts','docs/features/analytics-experiments.md','docs/agent/agent-ready.md','drizzle/0119_analytics_notification_provider_polling_readiness.sql'),
  json_array('npm run db:migrate:local','npm run typecheck','npm run cf:build','npm run lint','npm run test:runtime-secrets','npx playwright test tests/smoke.spec.ts --project=chromium -> 118 passed, 1 skipped','focused analytics provider-polling readiness, source-data, agent docs, admin source-data, and user-journey smoke tests','admin analytics provider-polling readiness screenshot captured under docs/pr-screenshots and public/pr-screenshots'),
  'Provider-polling readiness remains review-only: no provider send or call, delivery result, delivery-status webhook, status webhook, provider polling, provider configuration, provider response, provider message ID, delivery receipt, receipt payload, provider secret, sender credential, private DNS credential, Queue producer, queue message, queue acknowledgement, queue payload body, recipient payload, email body, customer alert, traffic routing, automated winner, or revenue claim was enabled.',
  unixepoch(),
  unixepoch(),
  json_array('https://bumpgrade.com/analytics/source-data','https://bumpgrade.com/api/admin/analytics/notification-provider-polling-readiness','https://bumpgrade.com/agent-docs/source-data','https://bumpgrade.com/pr-screenshots/issue-305-admin-analytics-provider-polling-readiness.png','https://github.com/markitics/bumpgrade/issues/305'),
  NULL,
  unixepoch()
)
