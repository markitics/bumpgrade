CREATE TABLE IF NOT EXISTS analytics_notification_provider_call_readiness_records (
  id TEXT PRIMARY KEY,
  ...
  notification_provider_call_readiness_disposition TEXT NOT NULL,
  ...
  expected_notification_queue_consumer_readiness_status TEXT NOT NULL,
  ...
  owner_provider_call_readiness_recorded INTEGER NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX IF NOT EXISTS analytics_notification_provider_call_readiness_idempotency_unique
  ON analytics_notification_provider_call_readiness_records(idempotency_key);
CREATE INDEX IF NOT EXISTS analytics_notification_provider_call_readiness_dashboard_time_idx
  ON analytics_notification_provider_call_readiness_records(dashboard_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_provider_call_readiness_readiness_channel_idx
  ON analytics_notification_provider_call_readiness_records(readiness_id, channel_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_provider_call_readiness_send_payload_idx
  ON analytics_notification_provider_call_readiness_records(send_payload_readiness_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_provider_call_readiness_queue_consumer_idx
  ON analytics_notification_provider_call_readiness_records(queue_consumer_readiness_id, created_at);
...
SET
  ...
  next_milestone = 'Add analytics notification delivery-attempt readiness only after provider-call readiness keeps provider sends/calls, provider configuration, provider responses, provider secrets, sender credentials, recipients, email bodies, queues, customer alerts, and sends disabled.',
  updated_at = unixepoch()
WHERE id = 'roadmap-analytics-testing';

UPDATE admin_user_journeys
SET
  issue_numbers_json = json_array(18,87,105,107,119,121,123,125,127,129,261,263,265,267,269,271,284,286,288,290,292,294,297),
...
INSERT INTO admin_work_log_entries (
  ...
  'work-log-2026-05-21-analytics-notification-provider-call-readiness',
  'Added analytics notification provider-call readiness records',
  'Codex',
  'codex',
  'bumpgrade-bootstrap',
  'Continue the Bumpgrade parity build and add owner-reviewed analytics notification provider-call readiness evidence for agents.',
  json_array(json_object('number',297,'url','https://github.com/markitics/bumpgrade/issues/297'), json_object('number',18,'url','https://github.com/markitics/bumpgrade/issues/18')),
  json_array(json_object('number',298,'url','https://github.com/markitics/bumpgrade/pull/298')),
  ...
  json_array('src/lib/analytics-notification-provider-call-readiness.ts','src/app/api/admin/analytics/notification-provider-call-readiness/route.ts','src/app/analytics/source-data/route.ts','src/lib/agent-manifest.ts','docs/features/analytics-experiments.md','docs/agent/agent-ready.md','drizzle/0115_analytics_notification_provider_call_readiness.sql'),
  json_array('npm run db:migrate:local','npm run typecheck','npm run cf:build','npm run lint','npm run test:runtime-secrets','npx playwright test tests/smoke.spec.ts --project=chromium -> 118 passed, 1 skipped','focused analytics provider-call readiness, source-data, agent docs, admin source-data, and user-journey smoke tests','admin analytics provider-call readiness screenshot captured under docs/pr-screenshots and public/pr-screenshots'),
  'Provider-call readiness remains review-only: no provider send or call, provider configuration, provider response, provider message ID, provider secret, sender credential, private DNS credential, Queue producer, Queue consumer, queue message, queue acknowledgement, queue payload body, recipient payload, email body, customer alert, traffic routing, automated winner, or revenue claim was enabled.',
  unixepoch(),
  unixepoch(),
  json_array('https://bumpgrade.com/analytics/source-data','https://bumpgrade.com/api/admin/analytics/notification-provider-call-readiness','https://bumpgrade.com/agent-docs/source-data','https://bumpgrade.com/pr-screenshots/issue-297-admin-analytics-provider-call-readiness.png','https://github.com/markitics/bumpgrade/issues/297','https://github.com/markitics/bumpgrade/pull/298'),
  NULL,
  unixepoch()
)
