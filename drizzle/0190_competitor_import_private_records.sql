CREATE TABLE IF NOT EXISTS competitor_import_private_records (
  id TEXT PRIMARY KEY,
...
CREATE UNIQUE INDEX IF NOT EXISTS competitor_import_private_records_draft_kind_unique
  ON competitor_import_private_records(draft_funnel_id, record_kind);

CREATE INDEX IF NOT EXISTS competitor_import_private_records_tenant_importer_kind_idx
  ON competitor_import_private_records(tenant_id, importer_platform_id, record_kind, updated_at);

CREATE INDEX IF NOT EXISTS competitor_import_private_records_draft_idx
  ON competitor_import_private_records(draft_funnel_id);
...
SET
  public_evidence_json = json_array(
    ...
    'Private importer writes persist safe importReview export analysis, platform export matches, and recognized match IDs on new private draft metadata.',
    ...
    '/imports/source-data exposes platform IDs, input kinds, platform-specific source checklists, export match templates, preflight signal labels, export-file parser fields, importReview persistence fields, private structured import-record fields, saved private plan parts, preflight review routes, private-draft API routes, safety gates, limitations, and source evidence IDs for agents.',
    ...
    'Private importer rollback routes archive private import plans without deleting saved plan content, steps, structured private import records, or audit history so the same source can be restarted.'
  ),
  next_milestone = 'Use structured private import records to deepen field extraction, then add subscriber import depth behind explicit confirmed-write gates.',
...
SET
  happy_path_json = json_array(
    ...
    'Use /imports/source-data when an agent needs stable importer IDs, input kinds, source checklist items, export match templates, preflight signal labels, export-file parser fields, importReview persistence fields, private structured import-record fields, saved private plan parts, safety gates, limitations, source IDs, private import API routes, and rollback routes.'
  ),
  ...
  agent_access = 'Agents can read /imports/source-data, /compare/source-data, and /features/source-data to answer importer questions with platform IDs, input kinds, platform-specific source checklists, source checklist items, export match templates, preflight signal labels, redacted sourceChecklistReview maps, exportFileAnalysis fields, platformExportMatches, private importReview metadata persistence, private structured import-record contracts, duplicate-review fields, saved private plan parts, rollback routes, safety gates, source IDs, limitations, and private import API routes. Creating or archiving a private import plan requires verified publisher auth, exact confirmation, idempotency, audit evidence, and redacted responses; public or billing-impacting import writes remain unavailable.',
  validation_json = json_array(
    'Issue #467 adds /imports, dedicated importer source guides, /imports/clickfunnels, /imports/source-data, public redacted preflight review, sourceChecklistReview maps, exportFileAnalysis parsing, platformExportMatches, private importReview metadata persistence, structured private import records, sitemap and llms discovery, feature and comparison source-data references, and importer smoke coverage.',
...
INSERT INTO admin_work_log_entries (
  ...
  'work-log-2026-05-25-importer-structured-private-records',
  'Created structured private importer records',
  'Codex',
  'codex',
  ...
  json_array('Focused private importer structured-record and redaction smoke coverage', 'typecheck', 'lint', 'runtime secrets', 'Cloudflare build'),
  NULL,
  unixepoch() - 1800,
  unixepoch(),
