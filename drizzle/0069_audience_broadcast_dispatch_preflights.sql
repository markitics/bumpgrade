CREATE TABLE IF NOT EXISTS audience_broadcast_dispatch_preflights (
  id TEXT PRIMARY KEY,
  draft_id TEXT NOT NULL,
  delivery_queue_message_id TEXT NOT NULL,
  delivery_batch_id TEXT NOT NULL,
  schedule_intent_id TEXT NOT NULL,
  status TEXT NOT NULL DEFAULT 'dispatch_preflight_dry_run_recorded',
...
SET
  ...
  validation_json = '["Playwright covers /audience/source-data, aggregate subscriber/suppression/timeline/broadcast readiness/schedule intent/preview safety/queue readiness/delivery-batch/queue-message/dispatch-preflight redaction, /audience/indie-launch-waitlist, valid opt-in, unsubscribe, unknown-email suppression, owner /admin/audience inspection, owner CRM note creation, owner dry-run broadcast schedule intent creation, owner delivery-batch dry-run creation, owner queue-message dry-run creation, owner dispatch-preflight dry-run creation, unauthorized schedule/delivery/queue-message/dispatch-preflight rejection, stale readiness rejection, validation failures, duplicate idempotency, sitemap discovery, and agent manifest discovery.", "Issues #85, #103, #137, #167, #169, #171, #173, #175, #177, #183, #189, and #191 record the audience automation source-data contract, first live opt-in capture path, owner subscriber inspection path, unsubscribe/suppression path, owner CRM timeline note path, broadcast readiness path, dry-run broadcast schedule intent path, preview/footer safety path, queue readiness path, delivery-batch dry-run path, queue-message dry-run path, and dispatch-preflight dry-run path."]',
  updated_at = unixepoch()
