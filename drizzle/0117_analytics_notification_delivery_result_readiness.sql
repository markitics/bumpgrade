CREATE TABLE IF NOT EXISTS analytics_notification_delivery_result_readiness_records (
  id TEXT PRIMARY KEY,
  ...
  notification_delivery_result_readiness_disposition TEXT NOT NULL,
  ...
  expected_notification_delivery_attempt_readiness_status TEXT NOT NULL,
  ...
  owner_delivery_result_readiness_recorded INTEGER NOT NULL DEFAULT 0,
  ...
  delivery_result_recorded INTEGER NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX IF NOT EXISTS analytics_notification_delivery_result_readiness_idempotency_unique
  ON analytics_notification_delivery_result_readiness_records(idempotency_key);
CREATE INDEX IF NOT EXISTS analytics_notification_delivery_result_readiness_dashboard_time_idx
  ON analytics_notification_delivery_result_readiness_records(dashboard_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_delivery_result_readiness_readiness_channel_idx
  ON analytics_notification_delivery_result_readiness_records(readiness_id, channel_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_delivery_result_readiness_send_payload_idx
  ON analytics_notification_delivery_result_readiness_records(send_payload_readiness_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_delivery_result_readiness_delivery_attempt_idx
  ON analytics_notification_delivery_result_readiness_records(delivery_attempt_readiness_id, created_at);
...
INSERT INTO admin_work_log_entries (
  ...
  'work-log-2026-05-21-analytics-notification-delivery-result-readiness',
  'Added analytics notification delivery-result readiness records',
  'Codex',
  'codex',
  'bumpgrade-bootstrap',
  'Continue the Bumpgrade parity build and add owner-reviewed analytics notification delivery-result readiness evidence for agents.',
  json_array(json_object('number',301,'url','https://github.com/markitics/bumpgrade/issues/301'), json_object('number',18,'url','https://github.com/markitics/bumpgrade/issues/18')),
  json_array(),
  ...
  json_array('src/lib/analytics-notification-delivery-result-readiness.ts','src/app/api/admin/analytics/notification-delivery-result-readiness/route.ts','src/app/analytics/source-data/route.ts','src/lib/agent-manifest.ts','docs/features/analytics-experiments.md','docs/agent/agent-ready.md','drizzle/0117_analytics_notification_delivery_result_readiness.sql'),
  json_array('npm run db:migrate:local','npm run typecheck','npm run cf:build','npm run lint','npm run test:runtime-secrets','npx playwright test tests/smoke.spec.ts --project=chromium -> 118 passed, 1 skipped','focused analytics delivery-result readiness, source-data, agent docs, admin source-data, and user-journey smoke tests','admin analytics delivery-result readiness screenshot captured under docs/pr-screenshots and public/pr-screenshots'),
  'Delivery-result readiness remains review-only: no provider send or call, delivery attempt, delivery result, status webhook, provider polling, provider configuration, provider response, provider message ID, delivery receipt, receipt payload, provider secret, sender credential, private DNS credential, Queue producer, queue message, queue acknowledgement, queue payload body, recipient payload, email body, customer alert, traffic routing, automated winner, or revenue claim was enabled.',
  unixepoch(),
  unixepoch(),
  json_array('https://bumpgrade.com/analytics/source-data','https://bumpgrade.com/api/admin/analytics/notification-delivery-result-readiness','https://bumpgrade.com/agent-docs/source-data','https://bumpgrade.com/pr-screenshots/issue-301-admin-analytics-delivery-result-readiness.png','https://github.com/markitics/bumpgrade/issues/301'),
  NULL,
  unixepoch()
)
