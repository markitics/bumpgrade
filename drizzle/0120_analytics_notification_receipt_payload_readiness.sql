CREATE TABLE IF NOT EXISTS analytics_notification_receipt_payload_readiness_records (
  id TEXT PRIMARY KEY,
  ...
  notification_receipt_payload_readiness_disposition TEXT NOT NULL,
  ...
  expected_notification_provider_polling_readiness_status TEXT NOT NULL,
  ...
  owner_receipt_payload_readiness_recorded INTEGER NOT NULL DEFAULT 0,
  ...
  receipt_payload_recorded INTEGER NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX IF NOT EXISTS analytics_notification_receipt_payload_readiness_idempotency_unique
  ON analytics_notification_receipt_payload_readiness_records(idempotency_key);
CREATE INDEX IF NOT EXISTS analytics_notification_receipt_payload_readiness_dashboard_time_idx
  ON analytics_notification_receipt_payload_readiness_records(dashboard_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_receipt_payload_readiness_readiness_channel_idx
  ON analytics_notification_receipt_payload_readiness_records(readiness_id, channel_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_receipt_payload_readiness_send_payload_idx
  ON analytics_notification_receipt_payload_readiness_records(send_payload_readiness_id, created_at);
CREATE INDEX IF NOT EXISTS analytics_notification_receipt_payload_readiness_provider_polling_idx
  ON analytics_notification_receipt_payload_readiness_records(provider_polling_readiness_id, created_at);
...
INSERT INTO admin_work_log_entries (
  ...
  'work-log-2026-05-21-analytics-notification-receipt-payload-readiness',
  'Added analytics notification receipt-payload readiness records',
  'Codex',
  'codex',
  'bumpgrade-bootstrap',
  'Continue the Bumpgrade parity build and add owner-reviewed analytics notification receipt-payload readiness evidence for agents.',
  json_array(json_object('number',307,'url','https://github.com/markitics/bumpgrade/issues/307'), json_object('number',18,'url','https://github.com/markitics/bumpgrade/issues/18')),
  json_array(),
  ...
  json_array('src/lib/analytics-notification-receipt-payload-readiness.ts','src/app/api/admin/analytics/notification-receipt-payload-readiness/route.ts','src/app/analytics/source-data/route.ts','src/lib/agent-manifest.ts','docs/features/analytics-experiments.md','docs/agent/agent-ready.md','drizzle/0120_analytics_notification_receipt_payload_readiness.sql'),
  json_array('npm run db:migrate:local','npm run typecheck','npm run cf:build','npm run lint','npm run test:runtime-secrets','npx playwright test tests/smoke.spec.ts --project=chromium -> 123 passed, 1 skipped','focused analytics receipt-payload readiness, source-data, agent docs, admin source-data, and user-journey smoke tests','admin analytics receipt-payload readiness screenshot captured under docs/pr-screenshots and public/pr-screenshots'),
  'Receipt-payload readiness remains review-only: no provider send or call, provider polling, delivery result, delivery-status webhook, status webhook, receipt payload, provider configuration, provider response, provider message ID, delivery receipt, provider secret, sender credential, private DNS credential, Queue producer, queue message, queue acknowledgement, queue payload body, recipient payload, email body, customer alert, traffic routing, automated winner, or revenue claim was enabled.',
  unixepoch(),
  unixepoch(),
  json_array('https://bumpgrade.com/analytics/source-data','https://bumpgrade.com/api/admin/analytics/notification-receipt-payload-readiness','https://bumpgrade.com/agent-docs/source-data','https://bumpgrade.com/pr-screenshots/issue-307-admin-analytics-receipt-payload-readiness.png','https://github.com/markitics/bumpgrade/issues/307'),
  NULL,
  unixepoch()
)
