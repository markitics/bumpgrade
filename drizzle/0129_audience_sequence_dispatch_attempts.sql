CREATE TABLE IF NOT EXISTS audience_sequence_dispatch_attempts (
  id TEXT PRIMARY KEY,
...
SET
  ...
  next_milestone = 'Add real contact imports, sequence scheduling, Cloudflare Queue producers, and consumers only after sender-domain, provider-event, provider rate-limit, provider response, send-payload, Queue producer, Queue consumer, import redaction, unsubscribe footer, suppression, audit checks, sequence delivery-readiness, sequence delivery-batch gates, sequence queue-message gates, sequence dispatch-preflight gates, sequence dispatch-attempt receipts, delivery-batch gates, queue-message gates, dispatch preflight boundaries, and dispatch-attempt receipts stay explicit.',
  updated_at = unixepoch()
WHERE id = 'roadmap-email-automation';

UPDATE admin_user_journeys
SET
  issue_numbers_json = json_insert(COALESCE(issue_numbers_json, '[]'), '$[#]', 364),
  user_goal = REPLACE(
    user_goal,
    'dry-run sequence dispatch preflight evidence, broadcast readiness',
    'dry-run sequence dispatch preflight evidence, dry-run sequence dispatch attempt receipts, broadcast readiness'
  ),
  ...
  happy_path_json = json_insert(
    ...
    '$[#]',
    'Record dry-run sequence dispatch attempt receipts from the current sequence dispatch preflight after workspace revision, sequence status, readiness, provider-limit, suppression, unsubscribe-footer, sender-domain, audit, Queue producer, Queue consumer, and idempotency gates are checked.'
  ),
  edge_cases_json = json_insert(
    COALESCE(edge_cases_json, '[]'),
    '$[#]',
    'Sequence dispatch attempt receipts record aggregate owner evidence only; they do not dispatch Cloudflare Queue messages, create queue payload bodies, delivery queue rows, recipient payloads, personalized bodies, unsubscribe URLs, provider sends, provider responses, provider message IDs, or public agent sequence writes.'
  ),
  agent_access = REPLACE(
    agent_access,
    'record dry-run sequence dispatch preflight evidence, view broadcast readiness',
    'record dry-run sequence dispatch preflight evidence, record dry-run sequence dispatch attempt receipts, view broadcast readiness'
  ),
  validation_json = json_insert(
    COALESCE(validation_json, '[]'),
    '$[#]',
    'Issue #364 records the owner-confirmed dry-run sequence dispatch attempt receipt path after sequence dispatch preflight, workspace revision, sequence status, readiness, provider-limit, suppression, unsubscribe-footer, sender-domain, Queue producer, Queue consumer, idempotency, audit, and redaction checks.'
  ),
  updated_at = unixepoch()
