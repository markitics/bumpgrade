...
import { AdminSequenceTestSendForm } from "@/components/admin-sequence-test-send-form";
  ...
  return (
    <main className="roadmap-page admin-roadmap-page admin-audience-page">
      <section className="roadmap-hero">
        <div>
          ...
          <p className="lede">
            ...
            disabled. Sequence provider-call readiness records the
            owner-reviewed provider handoff gate while provider calls, sends, responses, message IDs, delivery attempts,
            results, webhooks, receipts, Queue consumption, and payload reads remain disabled. Sequence delivery-attempt
            readiness records the owner-reviewed attempt gate from current provider-call readiness while delivery
          ...
          <div className="hero-actions">
            ...
            <Link
              href={`https://github.com/markitics/bumpgrade/issues/${audienceSequenceQueueConsumerReadinessIssue}`}
              className="secondary-action"
            >
            ...
            <Link
              href={`https://github.com/markitics/bumpgrade/issues/${audienceSequenceDeliveryAttemptReadinessIssue}`}
              className="secondary-action"
            >
            ...
            <Link
              href={`https://github.com/markitics/bumpgrade/issues/${audienceSequenceDeliveryResultReadinessIssue}`}
              className="secondary-action"
            >
      ...
      <section className="content-band alternate">
        <div className="feature-proof-grid">
          ...
          <div>
            ...
            <p>
              {audienceSequenceDeliveryAttemptReadiness.counts.deliveryAttemptReadinessRecords} attempt-readiness record
              {audienceSequenceDeliveryAttemptReadiness.counts.deliveryAttemptReadinessRecords === 1 ? "" : "s"};{" "}
              {audienceSequenceDeliveryAttemptReadiness.counts.deliveryAttemptEnabledRecords} attempts enabled.
            </p>
          </div>
          <div>
            <Send aria-hidden="true" />
          ...
          <div>
            <MailCheck aria-hidden="true" />
            <h3>Sequence test sends</h3>
            <p>
              {audienceSequenceTestSends.counts.testSends} owner-only sequence test
              {audienceSequenceTestSends.counts.testSends === 1 ? "" : "s"} recorded;{" "}
              {audienceSequenceTestSends.counts.cloudflareBindingSends} Cloudflare binding sends.
            </p>
          </div>
          <div>
            <ShieldCheck aria-hidden="true" />
          ...
          {audienceSequenceQueueProducerReadiness.latestRecords.length > 0 ? (
            audienceSequenceQueueProducerReadiness.latestRecords.map((record) => (
              <article key={record.id} className="roadmap-card active">
                ...
                <AdminSequenceQueueConsumerReadinessForm
                  queueProducerReadinessId={record.id}
      ...
      <section className="content-band alternate">
        <div className="roadmap-section-heading">
          <div>
            <p className="eyebrow">Sequence Queue consumer readiness</p>
            <h2>Sequence consumers stay disabled until ack and provider-handoff gates pass</h2>
          </div>
          <Link
            href={`https://github.com/markitics/bumpgrade/issues/${audienceSequenceQueueConsumerReadinessIssue}`}
            className="text-link compact-link"
        ...
        <div className="roadmap-grid admin-record-grid">
          {audienceSequenceQueueConsumerReadiness.latestRecords.length > 0 ? (
            audienceSequenceQueueConsumerReadiness.latestRecords.map((record) => (
              <article key={record.id} className="roadmap-card active">
                <div className="roadmap-card-top">
                  <span className="status-badge active">{record.status.replaceAll("_", " ")}</span>
                  <span className="admin-pill">{record.dryRunMessageCount} messages</span>
                ...
                <ShieldCheck aria-hidden="true" />
                <h3>Sequence consumer handoff is readiness-only</h3>
                <p>{record.consumerGateStatus.replaceAll("_", " ")}</p>
                <div className="roadmap-detail">
                  <strong>Producer readiness</strong>
                  <span>{record.queueProducerReadinessId}</span>
                </div>
                <div className="roadmap-detail">
                ...
                <p className="card-note">
                  Sequence Queue consumer readiness records owner-reviewed evidence only. Cloudflare Queue consumers,
                  Queue message consumption, acks, retry/dead-letter rows, payload reads, recipient payloads,
            ...
            <article className="roadmap-card">
              <div className="roadmap-card-top">
                <span className="status-badge pending">No sequence consumer readiness</span>
                <span className="admin-pill">Issue #{audienceSequenceQueueConsumerReadinessIssue}</span>
              </div>
              <h3>No sequence Queue consumer readiness records yet</h3>
              <p>
          ...
          {audienceSequenceProviderCallReadiness.latestRecords.length > 0 ? (
            audienceSequenceProviderCallReadiness.latestRecords.map((record) => (
              <article key={record.id} className="roadmap-card active">
                ...
                <AdminSequenceDeliveryAttemptReadinessForm
                  providerCallReadinessId={record.id}
                  sequenceId={record.sequenceId}
                  expectedWorkspaceRevisionId={record.expectedWorkspaceRevisionId}
                  expectedSequenceStatus={record.expectedSequenceStatus}
                  expectedReadyEnrollmentCount={record.expectedReadyEnrollmentCount}
      ...
      <section className="content-band alternate">
        <div className="roadmap-section-heading">
          <div>
            <p className="eyebrow">Sequence delivery-attempt readiness</p>
            <h2>Delivery attempts stay disabled until result and receipt gates pass</h2>
          </div>
          <Link
            href={`https://github.com/markitics/bumpgrade/issues/${audienceSequenceDeliveryAttemptReadinessIssue}`}
            className="text-link compact-link"
        ...
        <div className="roadmap-grid admin-record-grid">
          {audienceSequenceDeliveryAttemptReadiness.latestRecords.length > 0 ? (
            audienceSequenceDeliveryAttemptReadiness.latestRecords.map((record) => (
              <article key={record.id} className="roadmap-card active">
                <div className="roadmap-card-top">
                  <span className="status-badge active">{record.status.replaceAll("_", " ")}</span>
                  <span className="admin-pill">{record.dryRunMessageCount} messages</span>
                ...
                <Send aria-hidden="true" />
                <h3>Sequence delivery attempt is readiness-only</h3>
                <p>{record.deliveryAttemptGateStatus.replaceAll("_", " ")}</p>
                <div className="roadmap-detail">
                  <strong>Provider-call readiness</strong>
                  <span>{record.providerCallReadinessId}</span>
                </div>
                <div className="roadmap-detail">
                  <strong>Provider mode</strong>
                  <span>{record.providerMode.replaceAll("_", " ")}</span>
                ...
                <p className="card-note">
                  Sequence delivery-attempt readiness records owner-reviewed evidence only. Delivery attempts, provider
                  sends, provider responses, message IDs, delivery results, webhooks, receipts, Queue consumption, acks,
                  retry/dead-letter rows, payload reads, recipient payloads, personalized bodies, and unsubscribe URLs
                  remain disabled.
                </p>
                <AdminSequenceDeliveryResultReadinessForm
                  deliveryAttemptReadinessId={record.id}
                  sequenceId={record.sequenceId}
                  expectedWorkspaceRevisionId={record.expectedWorkspaceRevisionId}
                  expectedSequenceStatus={record.expectedSequenceStatus}
                  expectedReadyEnrollmentCount={record.expectedReadyEnrollmentCount}
            ...
            <article className="roadmap-card">
              <div className="roadmap-card-top">
                <span className="status-badge pending">No delivery-attempt readiness</span>
                <span className="admin-pill">Issue #{audienceSequenceDeliveryAttemptReadinessIssue}</span>
              </div>
              <h3>No sequence delivery-attempt readiness records yet</h3>
              <p>
                Record one from the latest sequence provider-call readiness record after provider-response, message-ID,
                result, webhook, receipt, backpressure, audit, and stale-state gates are checked.
              </p>
            </article>
          )}
        </div>
      ...
      <section className="content-band alternate">
        <div className="roadmap-section-heading">
          <div>
            <p className="eyebrow">Sequence delivery-result readiness</p>
            <h2>Delivery results stay disabled until webhook and receipt gates pass</h2>
          </div>
          <Link
            href={`https://github.com/markitics/bumpgrade/issues/${audienceSequenceDeliveryResultReadinessIssue}`}
            className="text-link compact-link"
        ...
        <div className="roadmap-grid admin-record-grid">
          {audienceSequenceDeliveryResultReadiness.latestRecords.length > 0 ? (
            audienceSequenceDeliveryResultReadiness.latestRecords.map((record) => (
              <article key={record.id} className="roadmap-card active">
                <div className="roadmap-card-top">
                  <span className="status-badge active">{record.status.replaceAll("_", " ")}</span>
                  <span className="admin-pill">{record.dryRunMessageCount} messages</span>
                ...
                <Send aria-hidden="true" />
                <h3>Sequence delivery result is readiness-only</h3>
                <p>{record.deliveryResultGateStatus.replaceAll("_", " ")}</p>
                <div className="roadmap-detail">
                  <strong>Delivery-attempt readiness</strong>
                  <span>{record.deliveryAttemptReadinessId}</span>
                </div>
                <div className="roadmap-detail">
                  <strong>Provider mode</strong>
                  <span>{record.providerMode.replaceAll("_", " ")}</span>
                ...
                <div className="roadmap-detail">
                  <strong>Delivery result policy</strong>
                  <span>{record.deliveryResultPolicy}</span>
                </div>
                <p className="card-note">
                  Sequence delivery-result readiness records owner-reviewed evidence only. Provider sends, provider
                  responses, message IDs, delivery attempts, delivery results, webhooks, receipts, Queue consumption,
                  acks, retry/dead-letter rows, payload reads, recipient payloads, personalized bodies, and unsubscribe
                  URLs remain disabled.
                </p>
            ...
            <article className="roadmap-card">
              <div className="roadmap-card-top">
                <span className="status-badge pending">No delivery-result readiness</span>
                <span className="admin-pill">Issue #{audienceSequenceDeliveryResultReadinessIssue}</span>
              </div>
              <h3>No sequence delivery-result readiness records yet</h3>
              <p>
      ...
      <section className="content-band">
        <div className="roadmap-section-heading">
          <div>
            <p className="eyebrow">Owner sequence test sends</p>
            <h2>Send one verified-owner sequence step before automation delivery exists</h2>
          </div>
          <Link href={`https://github.com/markitics/bumpgrade/issues/${audienceSequenceTestSendIssue}`} className="text-link compact-link">
        ...
        <div className="roadmap-grid admin-record-grid">
          <article className="roadmap-card active">
            <div className="roadmap-card-top">
              ...
              <span className="admin-pill">{audienceSequenceTestSends.counts.providerOkRows} provider OK</span>
            </div>
            <MailCheck aria-hidden="true" />
            <h3>Controlled sequence test path</h3>
            <p>
              Sends only to the verified owner session email after exact confirmation, idempotency, audit correlation,
              workspace revision checks, sequence status checks, and readiness count checks.
            </p>
            <div className="roadmap-detail">
              <strong>Cloudflare binding sends</strong>
              <span>{audienceSequenceTestSends.counts.cloudflareBindingSends}</span>
            </div>
            <div className="roadmap-detail">
              <strong>Test captures</strong>
              <span>{audienceSequenceTestSends.counts.testCaptures}</span>
            </div>
            <div className="roadmap-detail">
              <strong>Queue messages</strong>
              <span>{audienceSequenceTestSends.counts.queueMessagesCreatedRecords}</span>
            </div>
            <div className="roadmap-detail">
              <strong>Provider message IDs</strong>
              <span>{audienceSequenceTestSends.counts.providerMessageIdsCreatedRecords}</span>
            </div>
            <AdminSequenceTestSendForm
              sequences={audienceAutomationWorkspace.sequences}
              readiness={audienceSequenceDeliveryReadiness}
              latestTestSendId={audienceSequenceTestSends.latestTestSends[0]?.id ?? null}
            />
          </article>
          {audienceSequenceTestSends.latestTestSends.length > 0 ? (
            audienceSequenceTestSends.latestTestSends.map((testSend) => (
              <article key={testSend.id} className="roadmap-card active">
                ...
                <MailCheck aria-hidden="true" />
                <h3>Sequence test send {testSend.id.replace("sequence-owner-test-send-", "").slice(0, 8)}</h3>
                <p>{testSend.previewText}</p>
                <div className="roadmap-detail">
                  <strong>Step</strong>
                  <span>{testSend.stepId}</span>
                </div>
                <div className="roadmap-detail">
                  <strong>Audit correlation</strong>
                ...
                <p className="card-note">
                  Raw recipient email, raw email body, subscriber payloads, Queue messages, public agent sends, and
                  provider message IDs are excluded from the response and source-data.
                </p>
              </article>
            ))
          ) : (
            <article className="roadmap-card">
              <div className="roadmap-card-top">
                <span className="status-badge pending">No sequence test</span>
                <span className="admin-pill">Issue #{audienceSequenceTestSendIssue}</span>
              </div>
              <h3>No owner-only sequence test sends yet</h3>
              <p>
                Send one from the seeded sequence after current workspace revision, sequence status, and readiness
                counts are checked.
              </p>
            </article>
          )}
