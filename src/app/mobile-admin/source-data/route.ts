...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    actionIntentSummary,
    caveat:
      "This is the shared mobile admin contract and scaffold plan. The iOS and Android first slices now have simulator/emulator smoke evidence, /mobile-admin/dashboard/source-data is the live public-safe dashboard bridge with a redacted Director workstream digest, and issue #414 renders the owner-session, private-row API, Director review API, action-intent API, push-readiness boundary, distribution-readiness boundary, and confirmed-action requirements in the app scaffolds. /api/mobile-admin/private-rows is owner-session-only and read-only, /api/mobile-admin/private-rows/actions can mutate only low-risk private-row workflow state after owner confirmation, /api/mobile-admin/director-reviews can record owner-confirmed Director workstream acknowledgements, /api/mobile-admin/actions can record owner-gated audit-only action intents, and App Store/TestFlight or Play Store/internal-testing distribution, live push notifications, physical-device proof, and high-risk billing/publishing mobile mutations are not live yet.",
  });
}
