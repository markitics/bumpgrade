...
import {
  ...
  createAffiliatePartnerNotificationProviderReadinessRecord,
...
type NotificationProviderReadinessRecordRequestBody = {
  programId?: unknown;
...
async function parseBody(request: NextRequest): Promise<NotificationProviderReadinessRecordRequestBody> {
  try {
    return (await request.json()) as NotificationProviderReadinessRecordRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAffiliatePartnerNotificationProviderReadinessRecordSummary();
  ...
  return NextResponse.json({
    ...
    confirmation: { required: true, text: affiliatePartnerNotificationProviderReadinessRecordConfirmationText },
    contract: summary,
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAffiliatePartnerNotificationProviderReadinessRecord({
    ...
    notificationProviderReadinessDisposition: body.notificationProviderReadinessDisposition,
    ...
    expectedNotificationSendPreflightRecordStatus: body.expectedNotificationSendPreflightRecordStatus,
  ...
  if (!result.ok) {
    const status =
      ...
      result.status === "stale_notification_send_preflight_record_status" ||
    ...
    return jsonError(status, result.status, result.message, result.redaction, {
      ...
      currentNotificationSendPreflightRecordStatus: result.currentNotificationSendPreflightRecordStatus,
