...
import {
  ...
  createAudienceSequenceDeliveryQueueMessages,
...
type SequenceDeliveryQueueMessagesRequestBody = {
...
async function parseBody(request: NextRequest): Promise<SequenceDeliveryQueueMessagesRequestBody> {
  try {
    return (await request.json()) as SequenceDeliveryQueueMessagesRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAudienceSequenceDeliveryQueueMessageSummary();
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAudienceSequenceDeliveryQueueMessages({
  ...
  if (!result.ok) {
    const status =
      result.status === "delivery_batch_not_found" || result.status === "sequence_not_found"
        ? 404
