...
import {
  ...
  createAudienceSequenceQueueProducerReadiness,
...
type SequenceQueueProducerReadinessRequestBody = {
...
async function parseBody(request: NextRequest): Promise<SequenceQueueProducerReadinessRequestBody> {
  try {
    return (await request.json()) as SequenceQueueProducerReadinessRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAudienceSequenceQueueProducerReadinessSummary();
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAudienceSequenceQueueProducerReadiness({
  ...
  if (!result.ok) {
    const status =
      result.status === "dispatch_attempt_not_found" || result.status === "sequence_not_found"
        ? 404
