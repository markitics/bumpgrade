...
import {
  ...
  createAudienceSequenceDeliveryBatch,
...
type SequenceDeliveryBatchRequestBody = {
...
async function parseBody(request: NextRequest): Promise<SequenceDeliveryBatchRequestBody> {
  try {
    return (await request.json()) as SequenceDeliveryBatchRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAudienceSequenceDeliveryBatchSummary();
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAudienceSequenceDeliveryBatch({
  ...
  if (!result.ok) {
    const status =
      result.status === "schedule_intent_not_found" || result.status === "sequence_not_found"
        ? 404
