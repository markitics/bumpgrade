...
import {
  ...
  createAudienceSequenceDeliveryResultReadiness,
...
type SequenceDeliveryResultReadinessRequestBody = {
...
async function parseBody(request: NextRequest): Promise<SequenceDeliveryResultReadinessRequestBody> {
  try {
    return (await request.json()) as SequenceDeliveryResultReadinessRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAudienceSequenceDeliveryResultReadinessSummary();
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAudienceSequenceDeliveryResultReadiness({
  ...
  if (!result.ok) {
    const status =
      result.status === "delivery_attempt_readiness_not_found" || result.status === "sequence_not_found"
        ? 404
        : result.status === "readiness_unavailable"
          ...
          : result.status === "stale_workspace_revision" ||
              ...
              result.status === "stale_delivery_attempt_readiness_status" ||
              result.status === "stale_delivery_attempt_readiness_evidence" ||
              result.status === "delivery_result_gate_not_ready"
            ? 409
            : 400;
    return jsonError(status, result.status, result.message, result.redaction, {
      ...
      currentDeliveryAttemptReadinessStatus: result.currentDeliveryAttemptReadinessStatus,
    });
  }
