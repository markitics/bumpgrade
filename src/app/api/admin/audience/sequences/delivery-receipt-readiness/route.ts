...
import {
  ...
  createAudienceSequenceDeliveryReceiptReadiness,
...
type SequenceDeliveryReceiptReadinessRequestBody = {
...
async function parseBody(request: NextRequest): Promise<SequenceDeliveryReceiptReadinessRequestBody> {
  try {
    return (await request.json()) as SequenceDeliveryReceiptReadinessRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAudienceSequenceDeliveryReceiptReadinessSummary();
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAudienceSequenceDeliveryReceiptReadiness({
  ...
  if (!result.ok) {
    const status =
      result.status === "receipt_payload_readiness_not_found" || result.status === "sequence_not_found"
        ? 404
        : result.status === "readiness_unavailable"
          ...
          : result.status === "stale_workspace_revision" ||
              ...
              result.status === "stale_receipt_payload_readiness_status" ||
              result.status === "stale_receipt_payload_readiness_evidence" ||
              result.status === "delivery_receipt_gate_not_ready"
            ? 409
            : 400;
    return jsonError(status, result.status, result.message, result.redaction, {
      ...
      currentReceiptPayloadReadinessStatus: result.currentReceiptPayloadReadinessStatus,
    });
  }
