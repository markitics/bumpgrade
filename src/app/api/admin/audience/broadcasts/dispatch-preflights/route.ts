...
import {
  ...
  createAudienceBroadcastDispatchPreflight,
...
type DispatchPreflightRequestBody = {
...
async function parseBody(request: NextRequest): Promise<DispatchPreflightRequestBody> {
  try {
    return (await request.json()) as DispatchPreflightRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAudienceBroadcastDispatchPreflightSummary();
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAudienceBroadcastDispatchPreflight({
  ...
  if (!result.ok) {
    const status =
      result.status === "delivery_queue_message_not_found" || result.status === "broadcast_draft_not_found"
        ? 404
