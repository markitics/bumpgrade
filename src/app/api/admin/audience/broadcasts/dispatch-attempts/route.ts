...
import {
  ...
  createAudienceBroadcastDispatchAttempt,
...
type DispatchAttemptRequestBody = {
...
async function parseBody(request: NextRequest): Promise<DispatchAttemptRequestBody> {
  try {
    return (await request.json()) as DispatchAttemptRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAudienceBroadcastDispatchAttemptSummary();
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAudienceBroadcastDispatchAttempt({
  ...
  if (!result.ok) {
    const status =
      result.status === "dispatch_preflight_not_found" || result.status === "broadcast_draft_not_found"
        ? 404
