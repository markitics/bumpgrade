...
import {
  ...
  createAnalyticsNotificationReceiptPayloadReadiness,
...
type NotificationReceiptPayloadReadinessRequestBody = {
  dashboardId?: unknown;
  readinessId?: unknown;
  channelId?: unknown;
...
async function parseBody(request: NextRequest): Promise<NotificationReceiptPayloadReadinessRequestBody> {
  try {
    return (await request.json()) as NotificationReceiptPayloadReadinessRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAnalyticsNotificationReceiptPayloadReadinessSummary();
  ...
  return NextResponse.json({
    ...
    confirmation: { required: true, text: analyticsNotificationReceiptPayloadReadinessConfirmationText },
    contract: summary,
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAnalyticsNotificationReceiptPayloadReadiness({
    ...
    notificationReceiptPayloadReadinessDisposition: body.notificationReceiptPayloadReadinessDisposition,
    ...
    expectedNotificationProviderPollingReadinessStatus: body.expectedNotificationProviderPollingReadinessStatus,
  ...
  if (!result.ok) {
    const status =
      ...
      result.status === "stale_notification_provider_polling_readiness_status" ||
      ...
      result.status === "stale_notification_provider_polling_readiness_evidence" ||
    ...
    return jsonError(status, result.status, result.message, result.redaction, {
      ...
      currentNotificationProviderPollingReadinessStatus: result.currentNotificationProviderPollingReadinessStatus,
