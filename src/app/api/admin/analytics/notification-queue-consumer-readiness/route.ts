...
import {
  ...
  createAnalyticsNotificationQueueConsumerReadiness,
...
type NotificationQueueConsumerReadinessRequestBody = {
  dashboardId?: unknown;
  readinessId?: unknown;
  channelId?: unknown;
...
async function parseBody(request: NextRequest): Promise<NotificationQueueConsumerReadinessRequestBody> {
  try {
    return (await request.json()) as NotificationQueueConsumerReadinessRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAnalyticsNotificationQueueConsumerReadinessSummary();
  ...
  return NextResponse.json({
    ...
    confirmation: { required: true, text: analyticsNotificationQueueConsumerReadinessConfirmationText },
    contract: summary,
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAnalyticsNotificationQueueConsumerReadiness({
    ...
    notificationQueueConsumerReadinessDisposition: body.notificationQueueConsumerReadinessDisposition,
    ...
    expectedNotificationQueueProducerReadinessStatus: body.expectedNotificationQueueProducerReadinessStatus,
  ...
  if (!result.ok) {
    const status =
      ...
      result.status === "stale_notification_queue_producer_readiness_status" ||
      ...
      result.status === "stale_notification_queue_producer_readiness_evidence" ||
    ...
    return jsonError(status, result.status, result.message, result.redaction, {
      ...
      currentNotificationQueueProducerReadinessStatus: result.currentNotificationQueueProducerReadinessStatus,
