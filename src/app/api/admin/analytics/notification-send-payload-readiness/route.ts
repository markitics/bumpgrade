...
import {
  ...
  createAnalyticsNotificationSendPayloadReadiness,
...
type NotificationSendPayloadReadinessRequestBody = {
  dashboardId?: unknown;
  readinessId?: unknown;
  channelId?: unknown;
...
async function parseBody(request: NextRequest): Promise<NotificationSendPayloadReadinessRequestBody> {
  try {
    return (await request.json()) as NotificationSendPayloadReadinessRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAnalyticsNotificationSendPayloadReadinessSummary();
  ...
  return NextResponse.json({
    ...
    confirmation: { required: true, text: analyticsNotificationSendPayloadReadinessConfirmationText },
    contract: summary,
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAnalyticsNotificationSendPayloadReadiness({
    ...
    contentConsentReadinessId: body.contentConsentReadinessId,
    timeWindowKey: body.timeWindowKey,
    notificationSendPayloadReadinessDisposition: body.notificationSendPayloadReadinessDisposition,
    ...
    expectedNotificationContentConsentReadinessStatus: body.expectedNotificationContentConsentReadinessStatus,
  ...
  if (!result.ok) {
    const status =
      ...
      result.status === "stale_notification_content_consent_readiness_status" ||
      ...
      result.status === "stale_notification_content_consent_readiness_evidence" ||
    ...
    return jsonError(status, result.status, result.message, result.redaction, {
      ...
      currentNotificationContentConsentReadinessStatus: result.currentNotificationContentConsentReadinessStatus,
