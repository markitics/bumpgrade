...
import {
  ...
  createAnalyticsExperimentWinnerRollout,
...
async function parseBody(request: NextRequest): Promise<WinnerRolloutRequestBody> {
  try {
    return (await request.json()) as WinnerRolloutRequestBody;
  } catch {
    return {};
  }
}
...
export async function GET(request: NextRequest) {
  ...
  const summary = await getAnalyticsExperimentWinnerRolloutSummary();
...
export async function POST(request: NextRequest) {
  ...
  const result = await createAnalyticsExperimentWinnerRollout({
    action: body.action,
    ...
    expectedActiveRolloutRevision: body.expectedActiveRolloutRevision,
  ...
  if (!result.ok) {
    const status =
      result.status === "dashboard_not_found" ||
      result.status === "experiment_not_found" ||
      result.status === "rollout_not_found"
        ? 404
        : result.status === "stale_dashboard_revision" ||
            result.status === "stale_experiment_status" ||
            result.status === "stale_analytics_evidence" ||
            result.status === "active_rollout_exists" ||
            result.status === "stale_active_rollout" ||
    ...
    return jsonError(status, result.status, result.message, result.redaction, {
      ...
      activeRollout: result.activeRollout,
    });
  }
