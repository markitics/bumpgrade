...
export const agentReadContracts: AgentReadContract[] = [
  ...
  },
  {
    ...
    safeForAgents: [
      ...
      "Treat drag/drop placement as owner-session UI on top of existing move modes, not as freeform canvas styling or direct agent writing",
  ...
  },
  {
    ...
    safeForAgents: [
      ...
      "Update private draft block title/body copy, link resource-delivery metadata, link webinar-event metadata, duplicate a private draft, or archive/unpublish a draft only after exact owner confirmation",
    ...
    writeBoundary:
      `This owner-session API records direct agent-safe private funnel draft writes after exact confirmation, idempotency, current draft revision, and audit correlation checks. It can update private draft block copy, link resource-delivery metadata, link webinar-event metadata, duplicate a private draft, and archive/unpublish a draft. It does not publish from an agent, link checkout metadata from an agent, purge rows from an agent, create unauthenticated public agent writes, mutate billing, expose arbitrary private R2 delivery, create signed URLs, run live webinar or fulfillment automation, or expose owner identity in responses. Issue #${agentFunnelDraftWriteIssue} tracks this slice.`,
  },
  {
  ...
  },
  {
    ...
    writeBoundary:
      "Seeded referral clicks can be captured with idempotency and destination-route validation, eligible clicks can be attached to sandbox checkout intents as evidence, trusted checkout attribution can create review-only commission ledger evidence, owner sessions can review, hold, or reverse that evidence, public-safe partner reports can be read, read-only payout preparation can be inspected, and owner sessions can record payout preparation, fraud review, fraud enforcement, partner notification readiness, partner notification send preflight, and notification provider readiness evidence; cookie assignment, buyer attribution finalization, payable commission writes, direct agent review writes, payout actions, tax collection, private partner portals, partner notification sends, provider-send enablement, provider configuration, provider secret storage, provider calls, send payload creation, and queue dispatch require future confirmed-write APIs.",
  },
  {
...
export const agentSourceEvidenceRoutes: AgentSourceEvidenceRoute[] = [
  ...
  },
  {
    ...
    volatileClaims:
      "The public funnel contract exposes template and block-template records, webinar/resource page-shape records, owner-gated template-create, checkout-link, checkout-unlink, resource-delivery-link, public funnel resource delivery token, webinar-event-link, archived-draft purge, within-step block-reorder, cross-step block-move, editable draft, publish capability metadata, direct agent-safe private resource-delivery and webinar-event linking metadata, public sandbox checkout-start rendering metadata, entitlement-safe resource access references, external webinar registration/replay references, and aggregate owner product delivery-gate counts; it does not expose unpublished private draft copy, direct agent template creation, direct agent checkout linking or unlinking, direct agent-created delivery tokens, direct agent purge, non-archived purge, direct agent block reordering, direct agent cross-step block moves, live billing, live webinar scheduling, attendance tracking, replay hosting, arbitrary uploaded private asset delivery, signed URLs, live fulfillment automation, unpublishing, or unconfirmed agent edits.",
  },
  {
