...
export const agentReadContracts: AgentReadContract[] = [
  ...
  },
  {
    ...
    safeForAgents: [
      ...
      "Inspect input kinds, platform-specific source checklists, preflight signal labels, redacted sourceChecklistReview maps, exportFileAnalysis fields, duplicate-review fields, rollback routes, and saved private plan parts",
    ...
    writeBoundary:
      "Importer source-data is read-only. Platform-specific source checklists describe what a prospect can bring before private plan creation. Public preflight review routes return redacted import maps, sourceChecklistReview status, and exportFileAnalysis structure without persisting records or echoing pasted source material, raw rows, raw file text, export file names, customer rows, credentials, sessions, or payment data. Private-draft importer APIs require a verified publisher session, exact confirmation, idempotency, and redacted responses; source URL and export file name duplicate review can reuse a matching private import plan. Private rollback routes archive importer-created plans and preserve saved plan content, steps, and audit history, while public publishing, live checkout, subscriber sends, domains, fulfillment, account transfer, payment credential migration, and customer passwords remain outside this contract.",
  },
  {
...
export const agentMcpPlan: AgentMcpPlan[] = [
  ...
  },
  {
    ...
    purpose: "Expose importer platform IDs, competitor mappings, input kinds, platform-specific source checklists, preflight signal labels, redacted sourceChecklistReview maps, exportFileAnalysis fields, saved private plan parts, duplicate-review statuses, rollback routes, limitations, and safety gates.",
    safetyBoundary:
      "Read-only; public export parsing returns redacted structure only. No raw export storage, private record creation, public publishing, checkout migration, subscriber send, domain, fulfillment, or account-to-account transfer may be performed by this resource.",
  },
  {
