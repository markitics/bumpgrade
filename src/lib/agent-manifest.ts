...
export const agentReadContracts: AgentReadContract[] = [
  ...
  },
  {
    ...
    writeBoundary:
      "Owner-session seed/create/template-create/duplicate/update/reorder/block-edit/block-style/block-add/block-remove/block-reorder/block-cross-step-move/checkout-link/checkout-unlink/resource-delivery-link/webinar-event-link/archive/purge draft writes, including webinar/resource template-to-draft creation, granular block title/body editing with preserved block metadata, curated block visual style presets with preserved block metadata, block add/remove from the reusable block library with checkout-linked block protection, within-step block reordering that preserves checkout/resource/webinar metadata, drag/drop block placement that reuses the same move endpoint modes with fresh revisions, cross-step block moves that preserve block metadata while changing step membership, checkout unlinking with preserved block identity and copy, resource delivery linking to public-safe product/access catalog assets, webinar event/replay linking to public-safe external URLs, private draft preview, exact-confirmed public publishing, exact-confirmed archive/unpublish, and exact-confirmed archived-draft purge with tombstone evidence exist at /admin/funnels. The owner-session /api/agent/funnels/draft-writes JSON API now lets agents perform private block copy edits, curated visual style presets, reusable block add/remove, checkout-offer linking, checkout unlinking, resource-delivery linking, webinar-event linking, within-step block reordering, cross-step block moves, private draft duplication, public publishing, archive/unpublish, and archived-draft purge after exact confirmation, idempotency, current draft revision, and audit correlation checks. The owner-session /api/agent/funnels/resource-delivery-tokens API can create a funnel-scoped resource delivery token for a published linked resource block after exact confirmation, idempotency, current published revision, checkout intent, entitlement, and audit correlation checks, with replays redacted because raw tokens are not stored. Agent style writes store only curated style IDs and preserve block metadata. Direct agent block removal refuses checkout-linked blocks and keeps at least one block per step. Owner-confirmed direct agent publishing creates a public route mutation with archive-draft rollback and no billing mutation. Direct agent archived-draft purge records a tombstone before deleting already archived draft and step rows without deleting audit rows, product assets, R2 objects, buyer records, or billing state. Published linked checkout blocks can render the existing sandbox checkout start surface, published resource-linked blocks can render entitlement-safe resource access references and mint funnel-scoped private download tokens only when the submitted checkout intent and entitlement match the linked product and file asset, and published webinar-linked blocks can render external registration/replay references. Owner product delivery-gate writes are separate owner-authenticated product APIs that expose only aggregate funnel source-data. Direct agent template creation, unauthenticated public agent-created delivery tokens, non-archived or bulk purge, unauthenticated public agent publishing, live billing, live webinar scheduling, attendance tracking, replay hosting, arbitrary uploaded private asset delivery, signed URLs, live fulfillment automation, and full absolute-position canvas editing require future confirmed-write APIs.",
  },
  {
    ...
    safeForAgents: [
      ...
      "Treat drag/drop placement as owner-session UI on top of existing move modes, not as full absolute-position canvas editing or direct agent writing",
  ...
  },
  {
    ...
    writeBoundary:
      "This public API can mint a one-use Bumpgrade download route for a published D1 funnel resource/delivery block after checkout intent, entitlement, product, and file asset scope are revalidated. The owner-session agent route can create the same scoped token after exact confirmation, idempotency, audit correlation, and current published-revision checks. Neither route exposes private R2 keys, signed URLs, buyer data, raw funnel rows, raw entitlement rows, arbitrary uploaded assets, live fulfillment mutation, or unauthenticated public agent writes.",
  },
  {
...
export const agentSourceEvidenceRoutes: AgentSourceEvidenceRoute[] = [
  ...
  },
  {
    ...
    volatileClaims:
      "The affiliate/referral contract includes seeded click capture, checkout attribution evidence, review-only commission ledger evidence, owner review/reversal action boundaries, aggregate counts, public-safe partner reports, public-safe partner portal status pages, read-only payout preparation, owner-confirmed payout preparation records, owner-reviewed fraud review records, owner-confirmed fraud enforcement records, owner-reviewed partner notification readiness records, owner-reviewed partner notification send preflight records, and owner-reviewed notification provider readiness records; it is not cookie assignment, buyer attribution finalization, payable commission state, private fraud signal exposure, tax collection, partner notification sends, provider-send enablement, provider configuration, provider secret storage, provider calls, send payload creation, queue dispatch, authenticated private partner portal access, direct agent review automation, or Stripe payout capability.",
  },
  {
...
export const agentMcpPlan: AgentMcpPlan[] = [
  ...
  },
  {
    ...
    purpose: "Expose importer platform IDs, competitor mappings, input kinds, platform-specific source checklists, export match templates, preflight signal labels, redacted sourceChecklistReview maps, exportFileAnalysis fields, platformExportMatches, private structured import-record contracts, safe extractedFields plans, subscriberImportDepth metadata, subscriberImportPreflight actions, subscriberImportCreation actions, private record review routes, review-action routes, extracted-field edit actions, saved private plan parts, duplicate-review statuses, rollback routes, limitations, and safety gates.",
    safetyBoundary:
      "Read-only; public export parsing returns redacted structure only. Public source-data exposes the importRecords, extractedFields, subscriberImportDepth, subscriberImportPreflight, and subscriberImportCreation contracts but not private record content, raw values, or private subscriber emails. No raw public export storage, public publishing, checkout migration, global audience send-list creation, sequence enrollment, subscriber send, private export, domain, fulfillment, or account-to-account transfer may be performed by this resource.",
  },
  {
  ...
  },
  {
    ...
    purpose: "Expose seeded funnel, published D1 funnels, ordered steps, blocks, reusable templates including webinar/resource page shapes, block templates, owner-gated draft duplication capability, owner-gated block-edit capability, owner-gated visual block style capability, owner-gated block add/remove capability, owner-gated block reorder capability, owner-gated cross-step block move capability, owner-gated archive/unpublish capability, owner-gated archived-draft purge capability, checkout-link capability, checkout-unlink capability, resource-delivery-link capability, public and owner-session agent funnel resource delivery token capability, webinar-event-link capability, public funnel checkout-start capability, revision IDs, owner-gated draft capability, and write-safety boundaries.",
    safetyBoundary:
      "Public resource stays read-only; published linked checkout blocks can render the sandbox checkout start surface, published resource-linked blocks can render entitlement-safe product access references and mint funnel-scoped private download tokens only after checkout intent and entitlement scope match, published webinar-linked blocks can render external event/replay references, and owner-session draft create/seed/template-create/duplicate/update/reorder/block-edit/block-style/block-add/block-remove/block-reorder/block-cross-step-move/checkout-link/checkout-unlink/resource-delivery-link/webinar-event-link/archive/purge, drag/drop block placement through existing move modes, webinar/resource template-to-draft, private preview, exact-confirmed publish, exact-confirmed archive/unpublish, and exact-confirmed archived-draft purge exist in admin UI. The owner-session /api/agent/funnels/draft-writes JSON API now allows direct agent-safe block copy edits, curated visual style presets, reusable block add/remove, checkout-offer linking, checkout unlinking, resource-delivery linking, webinar-event linking, block reordering, cross-step block moves, private draft duplication, public publishing, archive/unpublish, and archived-draft purge after exact confirmation, idempotency, current revision, and audit correlation checks. The owner-session /api/agent/funnels/resource-delivery-tokens API can create a funnel-scoped resource delivery token for a published linked resource block after exact confirmation, idempotency, current published revision, checkout intent, entitlement, and audit correlation checks, with replays redacted because raw tokens are not stored. Agent style writes store only curated style IDs and preserve block metadata. Direct agent publishing creates a public route mutation with archive-draft rollback and no billing mutation. Direct agent archived purge records a tombstone first and does not delete audit rows, product assets, R2 objects, buyer records, or billing state. Direct agent template creation, unauthenticated public agent-created delivery tokens, non-archived or bulk purge, unauthenticated public agent publishing, live billing, live webinar scheduling, attendance tracking, replay hosting, arbitrary uploaded private asset delivery, signed URLs, live fulfillment automation, and full absolute-position canvas editing require future confirmed-write contracts.",
  },
  {
  ...
  },
  {
    ...
    safetyBoundary:
      "Requires owner identity, exact confirmation, idempotency key, current archived draft revision, a durable funnel_purge_events tombstone, and redaction. It may delete only archived draft and step rows; audit rows, product assets, R2 objects, buyer records, billing state, non-archived drafts, and bulk retention actions remain out of scope.",
  },
  {
  ...
  },
  {
    ...
    purpose:
      "Expose seeded affiliate programs, partner records, referral links, public-safe partner reports, public-safe partner portal status pages, read-only payout preparation, owner-confirmed payout preparation records, owner-reviewed fraud review records, owner-confirmed fraud enforcement records, owner-reviewed partner notification readiness records, owner-reviewed partner notification send preflight records, owner-reviewed notification provider readiness records, aggregate click counts, checkout attribution evidence, attribution rules, commission fixtures, payout review, and fraud flags.",
    safetyBoundary:
      "Seeded referral click capture, checkout attribution evidence, review-only commission evidence, owner review actions, public-safe partner reports, read-only payout preparation, owner-confirmed payout preparation records, owner-reviewed fraud review records, owner-confirmed fraud enforcement records, owner-reviewed partner notification readiness records, owner-reviewed partner notification send preflight records, and owner-reviewed notification provider readiness records are live; buyer attribution finalization, payable commission writes, tax handling, payout account access, partner notification sends, provider-send enablement, provider configuration, provider secret storage, provider calls, send payload creation, queue dispatch, and Stripe payouts require confirmed-write contracts.",
  },
  {
