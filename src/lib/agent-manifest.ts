...
export const agentReadContracts: AgentReadContract[] = [
  ...
  },
  {
    ...
    writeBoundary:
      "Public visitors can submit the seeded opt-in form with explicit consent and can record unsubscribe/suppression evidence without exposing list membership; verified owners can inspect private subscriber rows, create private CRM notes, view broadcast readiness, preview safety, queue readiness, delivery-batch dry runs, queue-message dry runs, dispatch preflight dry runs, dispatch attempt receipts, sender-domain readiness, provider-event readiness, provider rate-limit readiness, and provider response readiness, and record dry-run schedule intents, delivery batches, queue-message evidence, dispatch preflight evidence, and dispatch attempt receipts in /admin/audience; imports, real email delivery, private exports, direct agent subscriber writes, private DNS/provider setup, provider webhooks, Cloudflare Queue dispatch, queue payload bodies, recipient payloads, provider responses, and provider message IDs require future confirmed-write APIs.",
  },
  {
