...
export const agentReadContracts: AgentReadContract[] = [
  ...
  },
  {
    ...
    writeBoundary:
      "The POST endpoint can seed, create, create from templates including webinar/resource page shapes, duplicate, update, reorder, block-edit, block-add, block-remove, checkout-link, checkout-unlink, resource-delivery-link, publish, and archive/unpublish private draft steps for an authenticated owner. Block edits update title/body copy only and preserve block IDs, block kinds, and checkout-link/resource-link metadata. Block add/remove uses reusable block-library items and refuses checkout-linked block removal until checkout metadata is unlinked through the dedicated confirmed action. Checkout unlinking and resource delivery linking preserve block ID, kind, title, body, step order, and audit evidence. Archived drafts become read-only owner evidence; private preview is owner-gated; destructive deletion, direct agent checkout unlinking, direct agent resource delivery linking, direct agent archive/unpublish, live webinar scheduling, arbitrary private R2 delivery, signed URLs, live fulfillment automation, direct agent template creation, direct agent duplication, and direct agent edits are not live.",
  },
  {
...
export const agentSourceEvidenceRoutes: AgentSourceEvidenceRoute[] = [
  ...
  },
  {
    ...
    volatileClaims:
      "The public funnel contract exposes template and block-template records, webinar/resource page-shape records, owner-gated template-create, checkout-link, checkout-unlink, resource-delivery-link, editable draft, publish capability metadata, public sandbox checkout-start rendering metadata, entitlement-safe resource access references, and aggregate owner product delivery-gate counts; it does not expose unpublished private draft copy, direct agent template creation, direct agent checkout linking or unlinking, direct agent resource delivery linking, live billing, live webinar scheduling, arbitrary private R2 delivery, signed URLs, live fulfillment automation, unpublishing, or unconfirmed agent edits.",
  },
  {
...
export const agentMcpPlan: AgentMcpPlan[] = [
  ...
  },
  {
    ...
    purpose: "Expose seeded funnel, published D1 funnels, ordered steps, blocks, reusable templates including webinar/resource page shapes, block templates, owner-gated draft duplication capability, owner-gated block-edit capability, owner-gated block add/remove capability, owner-gated archive/unpublish capability, checkout-link capability, checkout-unlink capability, resource-delivery-link capability, public funnel checkout-start capability, revision IDs, owner-gated draft capability, and write-safety boundaries.",
    safetyBoundary:
      "Public resource stays read-only; published linked checkout blocks can render the sandbox checkout start surface, published resource-linked blocks can render entitlement-safe product access references, and owner-session draft create/seed/template-create/duplicate/update/reorder/block-edit/block-add/block-remove/checkout-link/checkout-unlink/resource-delivery-link/archive, webinar/resource template-to-draft, private preview, exact-confirmed publish, and exact-confirmed archive/unpublish exist in admin UI. Direct agent template creation, direct agent block editing, direct agent block add/remove, direct agent checkout unlinking, direct agent checkout-link, direct agent resource delivery linking, direct agent duplication, direct agent archive/unpublish, live billing, live webinar scheduling, arbitrary private R2 delivery, signed URLs, live fulfillment automation, destructive delete, and direct agent-edit tools require confirmed-write contracts.",
  },
  {
