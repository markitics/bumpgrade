...
export const agentReadContracts: AgentReadContract[] = [
  ...
  },
  {
    ...
    kind: "json",
    auth: "public",
    sourceOfTruth: "src/lib/product-access.ts + src/lib/product-entitlement-inspection.ts + src/lib/customer-product-entitlements.ts",
    stableIds: [
      "productId",
      "assetId",
      "accessRuleId",
    ...
    safeForAgents: [
      ...
      "Inspect customer-safe entitlement lookup boundaries",
      "Inspect entitlement and fulfillment boundaries",
    ],
    writeBoundary:
      "Trusted paid sandbox webhooks can grant idempotent entitlement rows for seeded checkout line items; verified owners can inspect private entitlement rows in /admin/products; customers can read redacted entitlement state by checkout intent evidence; product, asset, signed URL, revocation, live fulfillment, subscription access, and private content writes require future authenticated confirmed-write APIs.",
  },
  {
    id: "read-customer-product-entitlements",
    title: "Customer product entitlements",
    route: "/api/products/entitlements",
    kind: "api",
    auth: "public",
    sourceOfTruth: "D1 tables checkout_intents, product_entitlements, product_fulfillment_tasks, commerce_products, and commerce_prices through src/lib/customer-product-entitlements.ts",
    stableIds: ["checkoutIntentId", "productId", "entitlementTemplateId", "accessRuleId", "customerProductEntitlementLookupId"],
    safeForAgents: [
      "Read product entitlement status for a caller-provided checkout intent ID",
      "Inspect queued fulfillment summaries without private asset delivery",
      "Confirm customer lookup redacts buyer email, buyer hash, raw Stripe IDs, source event IDs, metadata JSON, private R2 keys, and signed URLs",
    ],
    writeBoundary:
      "This route is read-only and keyed by Bumpgrade checkout intent evidence. Signed downloads, protected lessons, revocation, refunds, customer portal actions, private asset delivery, and direct agent entitlement writes require future authenticated confirmed-write APIs.",
  },
  {
...
export const agentSourceEvidenceRoutes: AgentSourceEvidenceRoute[] = [
  ...
  },
  {
    ...
    route: "/products/source-data",
    resolves:
      "Seeded product catalog, assets, access rules, entitlement templates, sandbox webhook grant mappings, aggregate owner-entitlement inspection counts, customer entitlement lookup boundary, redaction flags, preview route, revision ID, and confirmed-write boundary.",
    stableIds: [
      "productId",
      "assetId",
      "accessRuleId",
    ...
    volatileClaims:
      "The product/access contract includes sandbox webhook-backed entitlement row grants, owner-only entitlement inspection, and customer-safe lookup by checkout intent evidence; it is not private asset delivery, signed URL access, revocation, or live fulfillment automation.",
  },
  {
...
export const agentMcpPlan: AgentMcpPlan[] = [
  ...
  },
  {
    ...
    purpose: "Expose seeded products, assets, access rules, entitlement templates, revision IDs, aggregate owner-entitlement inspection counts, customer lookup boundaries, and fulfillment boundaries.",
    safetyBoundary: "Read-only; customer lookups require caller-provided checkout intent evidence, and private asset access, private entitlement rows, entitlement writes, subscription access changes, and fulfillment actions require owner auth or confirmed-write contracts.",
  },
  {
