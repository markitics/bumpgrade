...
import { audienceSequenceDeliveryReadinessIssue } from "@/lib/audience-sequence-delivery";
import {
...
export const agentReadContracts: AgentReadContract[] = [
  ...
  },
  {
    ...
    auth: "public",
    sourceOfTruth:
      "src/lib/audience-automation.ts + src/lib/audience-subscribers.ts + src/lib/audience-broadcasts.ts + src/lib/audience-imports.ts + src/lib/audience-exports.ts",
    stableIds: [
      "subscriberId",
  ...
  },
  {
    ...
    stableIds: ["subscriberId", "subscriberSegmentId", "subscriberTagId", "emailSequenceId", "consentRecordId", "suppressionEntryId", "timelineEntryId", "audienceImportIntentId", "audienceImportPreflightId", "audienceExportReadinessId", "audienceSequenceDeliveryReadinessId", "ownerUserId"],
    safeForAgents: [
      ...
      "Inspect owner-visible import intent, import preflight, aggregate export readiness, and aggregate sequence delivery readiness records without importing, exporting, or sending contacts",
      "Confirm public source-data redacts email, name, suppression hashes, reasons, private note bodies, actor emails, raw IP, raw user agent, body templates, unsubscribe URLs, Queue payloads, provider IDs, send URLs, export URLs, and private metadata",
    ],
    writeBoundary:
      "This owner page can create private CRM notes through the owner note API, record non-destructive import intents through the import intent API, record aggregate import preflights through the import preflight API, and inspect aggregate export and sequence delivery readiness; real imports, sends, sequence delivery, broadcasts, private exports, export file creation, CRM automation, and direct agent subscriber writes require future confirmed-write APIs.",
  },
  {
...
export const agentSourceEvidenceRoutes: AgentSourceEvidenceRoute[] = [
  ...
  },
  {
    ...
    volatileClaims:
      `The audience automation contract includes consent-backed opt-in capture, aggregate owner-inspection evidence, unsubscribe/suppression evidence, unsubscribe-paused sequence enrollment aggregates, private owner-note counts, broadcast readiness, dry-run schedule intent counts, preview/footer safety, queue readiness, delivery-batch dry runs, queue-message dry runs, dispatch preflight dry runs, dispatch attempt receipts, sender-domain readiness, provider-event readiness, provider rate-limit readiness, provider response readiness, send-payload readiness, Queue producer readiness, Queue consumer readiness, owner-confirmed import intents, owner-confirmed import preflights, aggregate export readiness evidence from issue #${audienceExportReadinessIssue}, and aggregate sequence delivery readiness evidence from issue #${audienceSequenceDeliveryReadinessIssue}; it is not contact import, raw import row storage, raw email storage, subscriber creation from imports, live email sending, sequence delivery, private export, export file creation, export URL creation, private DNS/provider setup, provider webhook processing, Cloudflare Queue dispatch, Queue producer execution, Queue consumer execution, queue payload body creation or reading, ack/retry/dead-letter row creation, recipient payload creation, body-template exposure, unsubscribe URL creation, send URL creation, provider ID storage, raw payload body storage, provider response creation, provider message creation, personalized body generation, or direct public agent subscriber write capability.`,
  },
  {
