...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    featureStatus: "pending",
    issueNumbers: [17, 85, 103, 137, 167, 169, 171, 173, 175],
    ...
    happyPath: [
      ...
      "Find the seeded audience workspace, revision ID, opt-in form, lead magnet, tag IDs, sequence IDs, automation IDs, aggregate subscriber inspection counts, aggregate suppression counts, aggregate CRM timeline counts, broadcast readiness counts, dry-run schedule intent counts, preview/footer safety records, and write boundaries.",
      ...
      "Open /admin/audience as a verified owner to inspect private subscriber rows, active tags, consent counts, draft sequence enrollment state, suppression totals, private CRM timeline notes, broadcast readiness, dry-run schedule intents, and preview safety.",
    ...
    edgeCases: [
      "The seeded audience automation workspace can capture explicit-consent opt-ins, record unsubscribe/suppression evidence, store private owner notes, calculate broadcast readiness, record dry-run schedule intent metadata, and expose preview safety metadata, but it is not a general contact import, send queue, provider-send path, or full CRM database.",
    ...
    agentAccess:
      "Agents can read /audience/source-data, aggregate subscriber/suppression/timeline/broadcast readiness/schedule intent/preview safety counts, and the opt-in/unsubscribe/note/schedule-intent write boundaries. Owner sessions can inspect private rows, create private notes, view broadcast readiness and preview safety, and record dry-run schedule intents in /admin/audience. Direct agent subscriber writes, imports, real sends, private exports, or CRM automation require future authenticated confirmed-write APIs with consent, suppression, sender-domain safety, and audit correlation.",
    validation: [
      "Playwright covers /audience/source-data, aggregate subscriber/suppression/timeline/broadcast readiness/schedule intent/preview safety redaction, /audience/indie-launch-waitlist, valid opt-in, unsubscribe, unknown-email suppression, owner /admin/audience inspection, owner CRM note creation, owner schedule-intent creation, unauthorized note/schedule-intent rejection, validation failures, duplicate idempotency, sitemap discovery, and agent manifest discovery.",
      "Issues #85, #103, #137, #167, #169, #171, #173, and #175 record the audience automation source-data contract, first live opt-in capture path, owner subscriber inspection path, unsubscribe/suppression path, owner CRM timeline note path, broadcast readiness path, dry-run broadcast schedule intent path, and broadcast preview safety path.",
    ],
    sortOrder: 50,
    updatedAt: null,
  },
