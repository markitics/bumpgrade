...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    userGoal:
      "Inspect seeded analytics definitions, capture privacy-safe test events, record browser-side funnel page views with deterministic variant evidence, assign deterministic variants, and read aggregate conversion report rows before cookies, contact-level reporting, or automated decisions exist.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Find event IDs, page-view beacon boundary, aggregate variant event counts, metric IDs, aggregate event counts, aggregate conversion report rows, experiment IDs, variant IDs, assignment rule, assignment API, and write boundary.",
      "Open /funnels/indie-launch-sandbox and let the session-idempotent page-view beacon assign a variant and record a seeded event with that variant ID.",
    ...
    edgeCases: [
      "Public source-data exposes aggregate counts, aggregate variant counts, and conversion rows only, not raw event or assignment rows.",
    ...
    agentAccess:
      "Agents can read /analytics/source-data, event capture boundaries, page-view beacon boundaries, aggregate variant evidence, assignment boundaries, and aggregate conversion report rows. Direct agent analytics writes, custom events, campaign attribution mutation, experiment routing, and automated decisions require future authenticated confirmed-write APIs with privacy review, idempotency, stale-state checks, audit correlation, redaction, retention limits, and sample-size caveats.",
    validation: [
      "Playwright covers /analytics/source-data, /analytics/indie-launch-dashboard, event ingestion, page-view beacon capture with variant evidence, bot suppression, assignment ingestion, conversion reporting from captured events, duplicate idempotency, deterministic assignment, validation failures, opt-in event recording, sitemap discovery, and agent manifest discovery.",
      "Issues #87, #105, #107, #119, #121, and #123 record the analytics source-data scaffold, first privacy-safe event capture path, first deterministic assignment path, first aggregate conversion report, first browser-side funnel page-view beacon, and first variant-linked page-view evidence.",
    ],
    sortOrder: 51,
    updatedAt: null,
  },
  {
    ...
    happyPath: [
      "Capture seeded analytics events with idempotency or visit the public funnel preview to assign a seeded variant and emit a session-idempotent page-view event with variant evidence.",
      "Fetch /analytics/source-data.",
    ...
    agentAccess:
      "Agents can read aggregate funnel conversion rows and cite metric IDs, event IDs, aggregate variant evidence, and issues #119/#121/#123 evidence. Direct analytics writes, traffic routing, and automated experiment decisions require future confirmed-write APIs.",
    validation: [
      "Playwright seeds captured events, visits the funnel page-view beacon, replays duplicate idempotency, verifies aggregate variant counts, verifies conversion counts and rates, and checks public source-data excludes raw/private rows.",
      "Issues #119, #121, and #123 record the aggregate conversion report, browser-side funnel page-view instrumentation, and variant-linked page-view evidence.",
    ],
    sortOrder: 55,
    updatedAt: null,
  },
  {
    ...
    validation: [
      "Playwright covers valid event ingestion, bot/crawler suppression, duplicate idempotency, validation failures, and aggregate-only source-data.",
      "Issues #105 and #121 record the first live analytics event capture path and the first bot-suppressed browser beacon path.",
    ],
    sortOrder: 53,
    updatedAt: null,
  },
