...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    agentAccess:
      "Agents can read /products/source-data, aggregate entitlement inspection counts, the preview route, and the customer-safe checkout intent lookup contract. Owner sessions can inspect private buyer entitlement rows in /admin/products. Trusted paid sandbox webhooks can grant seeded entitlement rows; direct product/access writes require actor identity, exact confirmation, idempotency, stale-state checks, audit correlation, redaction, and trusted checkout or subscription evidence in a later API.",
    validation: [
      "Playwright covers /products/source-data, aggregate entitlement inspection redaction, /products/indie-launch-library, customer /products/entitlements lookup, owner /admin/products inspection, sitemap discovery, and agent manifest read-contract discovery.",
  ...
  },
  {
    ...
    agentAccess:
      "Agents can read /products/source-data, /api/products/entitlements, and /commerce/source-data to understand the entitlement grant contract, customer-safe lookup, and aggregate inspection counts. Owner sessions can inspect private buyer entitlement rows in /admin/products. Agents cannot grant, revoke, expose private buyer data, or issue signed download URLs without future authenticated confirmed-write APIs.",
    validation: [
      "Playwright covers source-data output, aggregate redaction, a paid test checkout webhook that creates entitlement rows, customer-safe lookup, owner /admin/products inspection, and duplicate webhook idempotency.",
