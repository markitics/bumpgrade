...
const journeyProofByFeatureId: Record<string, AdminUserJourneyProof> = {
  "feature-funnel-builder": {
    ...
    notes: [
      "Issue #14 is the shipped funnel MVP proof. Issue #417 now includes owner-confirmed checkout unlinking; live billing stays in issue #219; physical deletion, drag-and-drop editing, private resource delivery, webinar integrations, and direct public agent writes stay in issue #417.",
      "Issue #215 adds owner-confirmed private draft duplication without copying checkout-link metadata; issue #341 adds owner-confirmed archive/unpublish without deleting evidence; issue #417 adds owner-confirmed checkout unlinking; issue #430 adds owner-session block title/body editing while preserving block metadata; issue #432 adds owner-session reusable block add/remove while refusing checkout-linked block removal.",
    ],
  },
...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    happyPath: [
      ...
      "Use the write boundary to avoid claiming live billing, physical deletion, direct agent checkout unlinking, direct agent archive/unpublish, direct agent checkout-link, direct agent block editing, direct agent block add/remove, or direct agent-write capability.",
    ],
    edgeCases: [
      "The seeded funnel is read-only and not an authenticated builder UI.",
      "Owner-session template-to-draft creation, private draft duplication, granular block copy editing, reusable block add/remove, checkout-offer linking, checkout unlinking, and archive/unpublish lifecycle actions are available from /admin/funnels, and published linked checkout blocks can render the existing sandbox checkout start surface. Block copy edits preserve IDs, kinds, and checkout-link metadata. Block removal refuses checkout-linked blocks until the dedicated unlink action clears checkout metadata. Duplicates stay private and strip checkout-link metadata. Webinar/resource templates do not schedule webinars, host replays, deliver private files, or grant entitlements. Physical deletion, live billing, direct agent archive/unpublish, direct agent checkout unlinking, direct agent block editing, direct agent block add/remove, and direct agent edits require future confirmed-write APIs.",
      "Generated copy remains draft until a publisher confirms it.",
    ],
    agentAccess:
      "Agents can read /funnels/source-data, reusable template and block-template records, webinar/resource page-shape records, draft duplication capability metadata, granular block-edit capability metadata, block add/remove capability metadata, checkout-link capability metadata, public funnel checkout-start capability metadata, archive/unpublish lifecycle metadata, the seeded preview route, and published D1 funnel routes. Owner-session template-to-draft creation, private draft duplication, block copy editing, reusable block add/remove, checkout-offer linking, and archive/unpublish actions require owner auth, idempotency, and stale-state checks; direct agent writes require actor identity, confirmation, idempotency, stale-state checks, audit correlation, redaction, and rollback notes.",
    validation: [
      ...
      "Issue #417 records owner-confirmed checkout unlinking plus the remaining advanced funnel parity follow-up after MVP closeout.",
    ],
    sortOrder: 46,
    updatedAt: null,
  },
  {
    ...
    featureId: "feature-funnel-builder",
    featureStatus: "live",
    issueNumbers: [14, 79, 91, 93, 95, 135, 159, 161, 163, 165, 213, 215, 341, 409, 417, 430],
    ...
    edgeCases: [
      ...
      "Granular block editing changes existing block title/body copy only; it preserves block ID, kind, agent-editable flag, ordered step structure, and checkout-link metadata.",
      ...
      "Checkout-link deletion, physical deletion, drag-and-drop layout editing, live billing, direct agent block editing, direct agent block add/remove, direct agent duplication, and direct agent writes still require future confirmed-write APIs.",
    ...
    agentAccess:
      "Agents can read public /funnels/source-data, seeded funnel routes, reusable templates including webinar/resource page shapes, draft duplication capability metadata, block-edit capability metadata, block add/remove capability metadata, checkout-link capability metadata, public funnel checkout-start capability metadata, archive/unpublish lifecycle metadata, and published D1 funnel routes. Owner-session UI may create from templates, duplicate, edit steps, edit existing block copy, add reusable blocks, remove safe unlinked blocks, link checkout offers, preview, publish, archive, and unpublish private draft steps with actor identity, confirmation where required, idempotency, audit correlation, stale-state checks, and redaction; direct agent edit/duplicate/archive tools are still planned.",
    validation: [
      "Playwright covers the owner-gated /admin/funnels surface, webinar/resource template records, template-to-draft create path, draft duplicate path, granular block-edit path, block add/remove path, linked-checkout block removal refusal, checkout-link create path, idempotent replay, stale checkout-link, block-edit, and block-add rejection, seed/update/reorder/publish/archive POST paths, stale publish/archive rejection, archived draft read-only behavior, private draft preview, public D1 funnel route rendering, public linked-checkout start rendering, archive removal from /funnels/source-data, /funnels/source-data capability metadata, and agent manifest discovery.",
  ...
  },
  {
    ...
    userGoal: "Open the future Bumpgrade mobile app to check roadmap, work-log, for-Mark attention, commerce health, owner-session boundaries, and confirmed-action requirements without separate mobile-only semantics.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Review the shared owner-session panel and confirmed-action cards before any private mobile rows or live mobile mutations exist.",
      ...
      "Use issue #414 for private mobile rows, live confirmed-write APIs, device proof, push-notification boundaries, and eventual distribution readiness.",
      "Follow closed scaffold issues #13, #67, #68, #153, #155, and #157 for completed read-only evidence.",
    ],
    edgeCases: [
      "The iOS simulator target is not App Store distribution, push notifications, private mobile rows, or live confirmed-write support.",
      "The Android emulator target is not Play Store distribution, push notifications, private mobile rows, or live confirmed-write support.",
    ...
    validation: [
      "Issue #414 now renders the owner-session and confirmed-action contract in the app scaffolds while tracking private mobile rows, live confirmed-write APIs, device proof, push-notification boundaries, and eventual distribution readiness after scaffold closeout.",
