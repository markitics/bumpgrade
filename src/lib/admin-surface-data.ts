...
const journeyProofByFeatureId: Record<string, AdminUserJourneyProof> = {
  "feature-funnel-builder": {
    ...
    notes: ["Live billing, physical deletion, direct public agent archive/unpublish, and direct public agent edits remain outside this proof. Issue #215 adds owner-confirmed private draft duplication without copying checkout-link metadata; issue #341 adds owner-confirmed archive/unpublish without deleting evidence."],
  },
  ...
  "feature-analytics-testing": {
    status: "passed",
    lastTestedAt: "2026-05-22T12:00:00.000Z",
    ...
    summary:
      "Analytics preview, aggregate conversion reporting, source attribution, time windows, aggregate report export metadata, owner-confirmed experiment decision evidence, owner-reviewed content/consent readiness, owner-reviewed send-payload readiness, owner-reviewed queue-producer readiness, owner-reviewed queue-consumer readiness, owner-reviewed provider-call readiness, owner-reviewed delivery-attempt readiness, owner-reviewed delivery-result readiness, owner-reviewed delivery-status webhook readiness, owner-reviewed provider-polling readiness, owner-reviewed receipt-payload readiness, owner-reviewed delivery-receipt readiness, and owner-reviewed provider-status reconciliation readiness have proof surfaces.",
    ciLinks: [{ label: "GitHub Actions", url: "https://github.com/markitics/bumpgrade/actions", kind: "ci" }],
    screenshotLinks: [
...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    happyPath: [
      ...
      "Use the write boundary to avoid claiming live billing, physical deletion, direct agent archive/unpublish, direct agent checkout-link, or direct agent-write capability.",
    ],
    edgeCases: [
      "The seeded funnel is read-only and not an authenticated builder UI.",
      "Owner-session template-to-draft creation, private draft duplication, checkout-offer linking, and archive/unpublish lifecycle actions are available from /admin/funnels, and published linked checkout blocks can render the existing sandbox checkout start surface. Duplicates stay private and strip checkout-link metadata. Webinar/resource templates do not schedule webinars, host replays, deliver private files, or grant entitlements. Physical deletion, live billing, direct agent archive/unpublish, and direct agent edits require future confirmed-write APIs.",
      "Generated copy remains draft until a publisher confirms it.",
    ],
    agentAccess:
      "Agents can read /funnels/source-data, reusable template and block-template records, webinar/resource page-shape records, draft duplication capability metadata, checkout-link capability metadata, public funnel checkout-start capability metadata, archive/unpublish lifecycle metadata, the seeded preview route, and published D1 funnel routes. Owner-session template-to-draft creation, private draft duplication, checkout-offer linking, and archive/unpublish actions require confirmation and idempotency; direct agent writes require actor identity, confirmation, idempotency, stale-state checks, audit correlation, redaction, and rollback notes.",
    validation: [
  ...
  },
  {
    ...
    featureId: "feature-funnel-builder",
    featureStatus: "launch-preview",
    issueNumbers: [14, 79, 91, 93, 95, 135, 159, 161, 163, 165, 213, 215, 341],
    primaryUser: "Publisher or owner preparing the first launch funnel",
    userGoal: "Create, seed, template-start, or duplicate an owner-gated draft funnel, including webinar and resource page shapes, tune the ordered steps, attach the seeded sandbox checkout offer to a checkout block, preview it privately, publish it to a public route that can start the linked sandbox checkout after exact confirmation, then archive or unpublish without deleting evidence.",
    sourceEvidence: [
    ...
    edgeCases: [
      ...
      "Archiving changes status to archived, clears preview_route, and preserves draft, step, block, checkout-link, and audit records; it does not physically delete data.",
      "Checkout-link deletion, physical deletion, drag-and-drop layout editing, live billing, direct agent duplication, and direct agent writes still require future confirmed-write APIs.",
    ...
    agentAccess:
      "Agents can read public /funnels/source-data, seeded funnel routes, reusable templates including webinar/resource page shapes, draft duplication capability metadata, checkout-link capability metadata, public funnel checkout-start capability metadata, archive/unpublish lifecycle metadata, and published D1 funnel routes. Owner-session UI may create from templates, duplicate, edit, link checkout offers, preview, publish, archive, and unpublish private draft steps with actor identity, confirmation, idempotency, audit correlation, stale-state checks, and redaction; direct agent edit/duplicate/archive tools are still planned.",
    validation: [
      "Playwright covers the owner-gated /admin/funnels surface, webinar/resource template records, template-to-draft create path, draft duplicate path, checkout-link create path, idempotent replay, stale checkout-link rejection, seed/update/reorder/publish/archive POST paths, stale publish/archive rejection, private draft preview, public D1 funnel route rendering, public linked-checkout start rendering, archive removal from /funnels/source-data, /funnels/source-data capability metadata, and agent manifest discovery.",
  ...
  },
  {
    ...
    userGoal:
      "Inspect products, create draft product records, link owner-created products to test offer/funnel IDs, create synthetic test checkout/access grants, inspect assets, access rules, entitlement templates, sandbox entitlement grant mappings, subscription-backed membership access, owner entitlement rows, owner-confirmed revocation intents, and protected fixture delivery checks before real private content storage is enabled.",
    sourceEvidence: [
    ...
    edgeCases: [
      ...
      "Public /products/source-data exposes aggregate entitlement, owner-created product, owner-created product test grant, owner-upload, and revocation-intent counts plus redaction flags, not buyer emails, owner emails, checkout IDs, entitlement IDs, target entitlement IDs, raw Stripe IDs, hashes, private reason notes, metadata JSON, R2 keys, signed URLs, or upload bodies.",
    ...
    agentAccess:
      "Agents can read /products/source-data, aggregate entitlement inspection counts, aggregate owner-created product and test grant counts, the preview route, the customer-safe checkout intent lookup contract, the short-lived private R2-backed download-token boundary with redemption revalidation, the owner-authenticated private asset upload intent boundary, owner-confirmed non-destructive revocation intent evidence, protected content readiness, protected fixture delivery checks, and subscription-backed membership access state. Owner sessions can create draft products, link owner-created products to test offer/funnel IDs, create synthetic paid test checkout/access grants, inspect private buyer entitlement rows, revocation intent records, and protected content readiness in /admin/products, create small private asset upload records, and record non-destructive revocation intents only through exact-confirmed, idempotent, stale-state-checked writes. Trusted paid sandbox webhooks can grant seeded entitlement rows and trusted Stripe Billing webhooks can sync membership access; direct customer delivery of arbitrary uploads, destructive revocation, real protected media delivery, live offer/funnel publishing, Customer Portal actions, and subscription mutations require later APIs.",
    validation: [
      "Playwright covers /products/source-data, owner draft product creation, owner-created product test grant creation, aggregate entitlement inspection redaction, /products/indie-launch-library, customer /products/entitlements lookup, private R2-backed token delivery, current checkout-state revalidation, replay rejection, protected fixture delivery, protected fixture stale-state rejection, subscription-backed membership access activation/inactivation, owner private asset upload intent creation, owner-confirmed non-destructive revocation intent creation, idempotent replay, stale-state rejection, unauthorized rejection, owner /admin/products inspection, revocation intent readiness, protected content readiness, sitemap discovery, and agent manifest read-contract discovery.",
  ...
  },
  {
    ...
    userGoal:
      "Confirm that a paid sandbox checkout webhook can grant product entitlements, trusted Stripe Billing state can sync membership access, owner-created draft products can create test offer/access grant evidence, queued fulfillment evidence stays public-safe, owner-confirmed revocation intent records stay non-destructive, and protected fixtures return only for active eligible checkout-linked entitlements.",
    sourceEvidence: [
      "https://bumpgrade.com/products/source-data",
      "https://bumpgrade.com/products/entitlements",
      "https://bumpgrade.com/api/products/entitlements",
    ...
    edgeCases: [
      ...
      "Owner-created product test grants do not create Stripe Checkout Sessions, live charges, published offer copy, or public buyer records.",
      "Duplicate webhook events do not create duplicate entitlements.",
      "Public /products/source-data exposes aggregate counts and redaction flags, not raw buyer rows, owner identity, checkout IDs, entitlement IDs, or idempotency keys.",
    ...
    agentAccess:
      "Agents can read /products/source-data, /api/products/entitlements, /api/products/download-tokens, /api/products/protected-content, and /commerce/source-data to understand the owner product creation contract, owner-created product test offer/access grant contract, entitlement grant contract, customer-safe lookup, short-lived private R2-backed fixture delivery with redemption revalidation, owner-upload intent boundary, owner-confirmed non-destructive revocation intent boundary, protected content readiness, protected fixture delivery checks, subscription-backed membership access, and aggregate inspection counts. Owner sessions can create draft products, create owner-created product test offer/access grants, inspect private buyer entitlement rows, revocation intent records, and protected content readiness in /admin/products, create small private asset upload records, and record revocation intents via exact-confirmed, idempotent, stale-state-checked writes. Agents cannot grant live access, destructively revoke, expose private buyer data, deliver arbitrary protected bodies, deliver arbitrary uploads to customers, mutate subscriptions, publish live offer/funnel changes, create Customer Portal sessions, or issue signed object URLs without future authenticated confirmed-write APIs.",
    validation: [
      "Playwright covers source-data output, owner draft product creation, owner-created product test grant creation, aggregate redaction, a paid test checkout webhook that creates entitlement rows, customer-safe lookup, private R2-backed token delivery, current checkout-state revalidation, replay rejection, protected fixture delivery and stale-state rejection, subscription-backed membership access activation/inactivation, owner private asset upload intent creation, owner-confirmed non-destructive revocation intent creation, idempotent replay, stale-state rejection, unauthorized rejection, owner /admin/products inspection, revocation intent readiness, protected content readiness, and duplicate webhook idempotency.",
  ...
  },
  {
    ...
    featureStatus: "launch-preview",
    issueNumbers: [18, 87, 105, 107, 119, 121, 123, 125, 127, 129, 261, 263, 265, 267, 269, 271, 284, 286, 288, 290, 292, 294, 297, 299, 301, 303, 305, 307, 309, 311],
  ...
  },
  {
    ...
    happyPath: [
      ...
      "Find the seeded affiliate program, revision ID, partner IDs, partner report IDs, payout preparation IDs, payout preparation record IDs, fraud review record IDs, partner notification readiness record IDs, partner notification send preflight record IDs, partner notification provider readiness record IDs, referral link IDs, attribution rule IDs, commission rule IDs, fixture ledger IDs, payout batch ID, review flags, click capture API, checkout attribution contract, review-only commission ledger contract, owner review action contract, partner report contract, payout preparation contract, owner payout preparation record contract, owner fraud review record contract, owner notification readiness record contract, owner notification send preflight record contract, owner notification provider readiness record contract, and write boundary.",
      ...
      "Read fraudReviewRecords, partnerNotificationReadinessRecords, partnerNotificationSendPreflightRecords, and partnerNotificationProviderReadinessRecords to inspect aggregate owner-reviewed fraud review, partner notification readiness, send preflight, and provider readiness evidence without private fraud signals, recipient data, message bodies, send payloads, provider configuration, provider secrets, sender credentials, provider IDs, queue rows, or raw rows.",
  ...
  },
  {
    ...
    userGoal:
      "Inspect read-only payout preparation rows, checklist blockers, partner-report links, owner-confirmed payout preparation records, owner-reviewed fraud review records, owner-reviewed partner notification readiness records, owner-reviewed partner notification send preflight records, and owner-reviewed notification provider readiness records before any Stripe payout, tax, payout account, partner send, provider-send configuration, provider secret storage, or fraud enforcement capability exists.",
    sourceEvidence: [
      "https://bumpgrade.com/affiliates/source-data",
      "https://bumpgrade.com/affiliates/indie-launch-partners",
      "https://bumpgrade.com/api/admin/affiliates/payout-preparation-records",
    ...
    happyPath: [
      "Fetch /affiliates/source-data.",
      "Find payoutPreparationContract, payoutPreparationSummary, payoutPreparationRecords, fraudReviewRecords, partnerNotificationReadinessRecords, partnerNotificationSendPreflightRecords, and partnerNotificationProviderReadinessRecords.",
      ...
      "Treat the preparation, fraud review, notification readiness, notification send preflight, and notification provider readiness rows as evidence only; do not call them payable commission state, partner statements, notification sends, provider-send configuration, provider secret storage, or fraud enforcement.",
    ],
    edgeCases: [
      ...
      "Public preparation rows, owner-confirmed payout records, owner-reviewed fraud records, notification readiness records, notification send preflight records, and notification provider readiness records never expose recipient emails, message bodies, send payloads, provider configuration, provider secrets, sender credentials, provider message IDs, queue rows, buyer data, raw ledger rows, raw click rows, raw checkout rows, raw actor identity, private fraud signals, partner payout accounts, tax forms, or Stripe payout identifiers.",
      "Stripe payouts, transfer creation, private partner portals, payout account storage, tax collection, fraud enforcement, payable commission finalization, partner notification sends, provider-send enablement, provider configuration, provider secret storage, provider calls, queue dispatch, and direct agent writes require future confirmed-write APIs.",
    ],
    agentAccess:
      "Agents can read public-safe payout preparation, fraud review, notification readiness, notification send preflight, and notification provider readiness evidence from /affiliates/source-data and cite payoutPreparationId, payoutPreparationRecordId, reviewFlagId, fraudReviewRecordId, partnerNotificationReadinessRecordId, partnerNotificationSendPreflightRecordId, and partnerNotificationProviderReadinessRecordId values. Owner sessions can record notification provider readiness evidence through /api/admin/affiliates/notification-provider-readiness. Payout execution, tax handling, payout accounts, partner sends, provider-send enablement, provider configuration, provider secret storage, send payload creation, fraud enforcement, and direct agent affiliate writes require future authenticated confirmed-write APIs.",
    validation: [
      "Playwright covers payoutPreparationContract, payoutPreparationSummary, owner-confirmed payout preparation records, owner-reviewed fraud review records, owner-reviewed partner notification readiness records, owner-reviewed partner notification send preflight records, owner-reviewed notification provider readiness records, preview rendering, admin journey exposure, agent manifest stable IDs, and redaction boundaries.",
      "Issues #195, #273, #275, #277, #279, and #281 record the read-only payout preparation slice, owner-confirmed payout preparation records, owner-reviewed fraud review records, owner-reviewed partner notification readiness records, owner-reviewed partner notification send preflight records, and owner-reviewed notification provider readiness records.",
    ],
    sortOrder: 59,
    updatedAt: null,
  },
