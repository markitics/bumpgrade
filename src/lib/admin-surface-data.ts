...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    userGoal:
      "Inspect affiliate programs, partner records, referral links, attribution windows, commission rules, review-only ledger evidence, payout review states, fraud flags, aggregate click counts, and checkout attribution evidence before payouts exist.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Find the seeded affiliate program, revision ID, partner IDs, referral link IDs, attribution rule IDs, commission rule IDs, fixture ledger IDs, payout batch ID, review flags, click capture API, checkout attribution contract, review-only commission ledger contract, and write boundary.",
      ...
      "Use /commerce/source-data and /offers/source-data to distinguish checkout attribution evidence and review-only ledger evidence from payable commissions.",
    ],
    edgeCases: [
      "Public source-data exposes aggregate click, checkout attribution, and commission ledger counts only, not raw click, checkout, buyer, or payout rows.",
      "Unsupported referral link IDs, codes, destination routes, referral click IDs, checkout intent IDs, and missing idempotency keys return public-safe validation errors.",
    ...
    agentAccess:
      "Agents can read /affiliates/source-data, /commerce/source-data, preview routes, click capture boundaries, checkout attribution boundaries, and review-only commission ledger boundaries. Payout, fraud, tax, partner notification, owner review, reversal, and direct agent affiliate writes require future authenticated confirmed-write APIs with actor identity, explicit confirmation, idempotency, stale-state checks, audit correlation, redaction, refund-window checks, payout review, and private payout data boundaries.",
    validation: [
      "Playwright covers /affiliates/source-data, /affiliates/indie-launch-partners, click ingestion, checkout attribution, review-only commission ledger creation, duplicate idempotency, validation failures, aggregate-only source-data, sitemap discovery, and agent manifest discovery.",
      "Issues #89, #109, #111, and #113 record the affiliate/referral source-data scaffold, privacy-safe click capture, checkout attribution evidence, and first review-only ledger evidence path.",
    ],
    sortOrder: 52,
    updatedAt: null,
  },
  ...
  },
  {
    ...
    userGoal:
      "Understand what Bumpgrade must know before it can create a Stripe checkout session safely, including optional referral-click evidence that remains separate from commissions.",
    sourceEvidence: [
    ...
    agentAccess:
      "Agents may read the architecture, product, price, checkout-intent, referral attribution, and redacted webhook records once exposed. Agents must not create checkout sessions without confirmed-write rules, idempotency, audit correlation, and stale-state checks, and must not create commissions or payout state.",
    validation: [
