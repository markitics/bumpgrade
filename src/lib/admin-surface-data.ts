...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    userGoal: "Inspect opt-in forms, tags, lead magnets, sequences, broadcasts, automation rules, owner subscriber rows, aggregate suppression evidence, private CRM timeline note counts, and the live consent/unsubscribe/note boundary before email sends exist.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Find the seeded audience workspace, revision ID, opt-in form, lead magnet, tag IDs, sequence IDs, automation IDs, aggregate subscriber inspection counts, aggregate suppression counts, aggregate CRM timeline counts, and write boundaries.",
      ...
      "Open /admin/audience as a verified owner to inspect private subscriber rows, active tags, consent counts, draft sequence enrollment state, suppression totals, and private CRM timeline notes.",
    ...
    edgeCases: [
      "The seeded audience automation workspace can capture explicit-consent opt-ins, record unsubscribe/suppression evidence, and store private owner notes, but it is not a general contact import or full CRM database.",
      "Public /audience/source-data exposes aggregate counts and redaction flags, not subscriber emails, names, raw IPs, raw user agents, suppression hashes, note bodies, actor emails, reasons, or private metadata.",
      "The unsubscribe API returns the submitted normalized email to the submitter but does not reveal list membership.",
      "Subscriber imports, direct agent contact writes, email sends, broadcasts, private exports, and CRM automation require future confirmed-write APIs.",
      "Codex project email in issue #10 is separate from publisher/customer email workflows.",
    ],
    agentAccess:
      "Agents can read /audience/source-data, aggregate subscriber/suppression/timeline counts, and the opt-in/unsubscribe/note write boundaries. Owner sessions can inspect private rows and create private notes in /admin/audience. Direct agent subscriber writes, imports, sends, broadcasts, private exports, or CRM automation require future authenticated confirmed-write APIs with consent, suppression, and sender-domain safety.",
    validation: [
      "Playwright covers /audience/source-data, aggregate subscriber/suppression/timeline redaction, /audience/indie-launch-waitlist, valid opt-in, unsubscribe, unknown-email suppression, owner /admin/audience inspection, owner CRM note creation, unauthorized note rejection, validation failures, duplicate idempotency, sitemap discovery, and agent manifest discovery.",
      "Issues #85, #103, #137, #167, and #169 record the audience automation source-data contract, first live opt-in capture path, owner subscriber inspection path, unsubscribe/suppression path, and owner CRM timeline note path.",
    ],
    sortOrder: 50,
    updatedAt: null,
  },
