...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    userGoal: "Inspect an ordered opt-in, sales, and thank-you funnel before visual editing and checkout-linking writes exist.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Use the write boundary to avoid claiming checkout-link, unpublish/delete, or direct agent-write capability.",
    ],
    edgeCases: [
      "The seeded funnel is read-only and not an authenticated builder UI.",
      "Checkout linking, deletion, unpublishing, and direct agent edits require future confirmed-write APIs.",
      "Generated copy remains draft until a publisher confirms it.",
    ],
    agentAccess:
      "Agents can read /funnels/source-data, the seeded preview route, and published D1 funnel routes. Funnel writes require actor identity, confirmation, idempotency, stale-state checks, audit correlation, redaction, and rollback notes.",
    validation: [
  ...
  },
  {
    ...
    featureId: "feature-funnel-builder",
    featureStatus: "pending",
    issueNumbers: [14, 79, 91, 93, 95, 135],
    primaryUser: "Publisher or owner preparing the first launch funnel",
    userGoal: "Create or seed a D1-backed draft funnel, tune the ordered opt-in, sales, and thank-you steps, preview it privately, then publish it to a public route after exact confirmation.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Open the public /funnels/{slug} route and confirm the published sequence is crawlable.",
    ...
    edgeCases: [
      ...
      "Checkout linking, deletion, unpublishing, drag-and-drop layout editing, and direct agent writes still require future confirmed-write APIs.",
      "/funnels/source-data lists published D1 funnels but does not expose raw owner session or unpublished private draft data.",
    ],
    agentAccess:
      "Agents can read public /funnels/source-data, seeded funnel routes, and published D1 funnel routes. Owner-session UI may create, edit, preview, and publish private draft steps with actor identity, idempotency, audit correlation, stale-state checks, and redaction; direct agent edit tools are still planned.",
    validation: [
      "Playwright covers the owner-gated /admin/funnels surface, seed/update/reorder/publish POST paths, stale publish rejection, private draft preview, public D1 funnel route rendering, /funnels/source-data capability metadata, and agent manifest discovery.",
  ...
  },
  {
    ...
    primaryUser: "Publisher, customer, or agent planning fulfillment",
    userGoal: "Inspect products, assets, access rules, entitlement templates, sandbox entitlement grant mappings, owner entitlement rows, and customer-safe lookup state before private fulfillment delivery is enabled.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Find seeded product types, asset IDs, access rules, entitlement templates, sandbox grant mappings, aggregate entitlement inspection counts, customer lookup contract, revision ID, and write boundary.",
      ...
      "Open /products/entitlements/{checkoutIntentId} or /api/products/entitlements with checkout intent evidence to inspect customer-safe entitlement status and next steps.",
    ...
    edgeCases: [
      ...
      "Public /products/source-data exposes aggregate entitlement counts, the customer lookup contract, and redaction flags, not buyer emails, raw Stripe IDs, hashes, metadata JSON, R2 keys, or signed URLs.",
      "Customer lookup requires a Bumpgrade checkout intent ID and still excludes buyer identity, hashes, raw Stripe IDs, source event IDs, metadata JSON, private R2 keys, and signed URLs.",
      "Private R2 keys, signed URLs, protected lessons, revocation, and live fulfillment require future confirmed-write APIs.",
      "Subscription access requires trusted billing state before membership entitlements can be granted.",
    ],
    agentAccess:
      "Agents can read /products/source-data, aggregate entitlement inspection counts, customer-safe entitlement lookup boundaries, and the preview route. Owner sessions can inspect private buyer entitlement rows in /admin/products. Customer lookups require caller-provided checkout intent evidence. Trusted paid sandbox webhooks can grant seeded entitlement rows; direct product/access writes require actor identity, exact confirmation, idempotency, stale-state checks, audit correlation, redaction, and trusted checkout or subscription evidence in a later API.",
    validation: [
      "Playwright covers /products/source-data, aggregate entitlement inspection redaction, customer-safe entitlement lookup, /products/indie-launch-library, owner /admin/products inspection, sitemap discovery, and agent manifest read-contract discovery.",
      ...
      "Issue #141 records the customer product entitlement lookup path.",
    ],
    sortOrder: 49,
    updatedAt: null,
  },
  {
    ...
    happyPath: [
      ...
      "Open /products/entitlements/{checkoutIntentId} to inspect customer-safe entitlement status, fulfillment summary, and next steps.",
    ...
    edgeCases: [
      ...
      "Customer lookup exposes product and fulfillment state for a caller-provided checkout intent ID but not buyer email, hashes, raw Stripe IDs, source event IDs, metadata JSON, R2 keys, or signed URLs.",
    ...
    agentAccess:
      "Agents can read /products/source-data, /api/products/entitlements, and /commerce/source-data to understand the entitlement grant contract, customer-safe lookup, and aggregate inspection counts. Owner sessions can inspect private buyer entitlement rows in /admin/products. Agents cannot grant, revoke, inspect private buyer access, or issue signed download URLs without future authenticated confirmed-write APIs.",
    validation: [
      "Playwright covers source-data output, aggregate redaction, a paid test checkout webhook that creates entitlement rows, customer-safe entitlement lookup, owner /admin/products inspection, and duplicate webhook idempotency.",
  ...
  },
  {
    ...
    userGoal: "Inspect opt-in forms, tags, lead magnets, sequences, broadcasts, automation rules, owner subscriber rows, and the live consent-backed capture boundary before email sends exist.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Find the seeded audience workspace, revision ID, opt-in form, lead magnet, tag IDs, sequence IDs, automation IDs, aggregate subscriber inspection counts, and write boundary.",
      ...
      "Open /admin/audience as a verified owner to inspect private subscriber rows, active tags, consent counts, and draft sequence enrollment state.",
    ...
    edgeCases: [
      ...
      "Public /audience/source-data exposes aggregate counts and redaction flags, not subscriber emails, names, raw IPs, raw user agents, or private metadata.",
      "Subscriber imports, direct agent contact writes, email sends, broadcasts, unsubscribe changes, private exports, and CRM notes require future confirmed-write APIs.",
      "Codex project email in issue #10 is separate from publisher/customer email workflows.",
    ],
    agentAccess:
      "Agents can read /audience/source-data, aggregate subscriber inspection counts, and the opt-in write boundary. Owner sessions can inspect private rows in /admin/audience. Direct agent subscriber writes, imports, sends, unsubscribes, broadcasts, private exports, or CRM notes require future authenticated confirmed-write APIs with consent, suppression, and sender-domain safety.",
    validation: [
      "Playwright covers /audience/source-data, aggregate subscriber inspection redaction, /audience/indie-launch-waitlist, valid opt-in, owner /admin/audience inspection, validation failures, duplicate idempotency, sitemap discovery, and agent manifest discovery.",
      "Issues #85, #103, and #137 record the audience automation source-data contract, first live opt-in capture path, and owner subscriber inspection path.",
    ],
    sortOrder: 50,
    updatedAt: null,
  },
