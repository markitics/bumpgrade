...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    issueNumbers: [14, 79, 159, 161, 163],
    primaryUser: "Publisher or agent planning the first funnel",
    userGoal:
      "Inspect an ordered opt-in, sales, and thank-you funnel plus reusable templates, block records, and owner-session checkout-link capability before visual editing or direct agent template creation exists.",
    sourceEvidence: [
  ...
  },
  {
    ...
    featureId: "feature-funnel-builder",
    featureStatus: "pending",
    issueNumbers: [14, 79, 91, 93, 95, 135, 159, 161, 163],
    primaryUser: "Publisher or owner preparing the first launch funnel",
    userGoal: "Create, seed, or template-start a D1-backed draft funnel, tune the ordered steps, attach the seeded sandbox checkout offer to a checkout block, preview it privately, then publish it to a public route after exact confirmation.",
    sourceEvidence: [
    ...
    edgeCases: [
      ...
      "Checkout-offer linking writes public-safe metadata into private draft step blocks and does not start a checkout session or enable live billing.",
      "Publishing requires exact confirmation and a fresh revision ID.",
      "Checkout-link deletion, unpublishing, drag-and-drop layout editing, and direct agent writes still require future confirmed-write APIs.",
      "/funnels/source-data lists published D1 funnels but does not expose raw owner session or unpublished private draft data.",
    ],
    agentAccess:
      "Agents can read public /funnels/source-data, seeded funnel routes, reusable templates, checkout-link capability metadata, and published D1 funnel routes. Owner-session UI may create from templates, edit, link checkout offers, preview, and publish private draft steps with actor identity, confirmation, idempotency, audit correlation, stale-state checks, and redaction; direct agent edit tools are still planned.",
    validation: [
      "Playwright covers the owner-gated /admin/funnels surface, template-to-draft create path, checkout-link create path, idempotent replay, stale checkout-link rejection, seed/update/reorder/publish POST paths, stale publish rejection, private draft preview, public D1 funnel route rendering, /funnels/source-data capability metadata, and agent manifest discovery.",
