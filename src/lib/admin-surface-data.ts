...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    userGoal: "Inspect opt-in forms, tags, lead magnets, sequences, broadcasts, automation rules, owner subscriber rows, aggregate suppression evidence, private CRM timeline note counts, broadcast readiness, and the live consent/unsubscribe/note boundary before email sends exist.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Find the seeded audience workspace, revision ID, opt-in form, lead magnet, tag IDs, sequence IDs, automation IDs, aggregate subscriber inspection counts, aggregate suppression counts, aggregate CRM timeline counts, broadcast readiness counts, and write boundaries.",
      ...
      "Open /admin/audience as a verified owner to inspect private subscriber rows, active tags, consent counts, draft sequence enrollment state, suppression totals, private CRM timeline notes, and broadcast readiness.",
    ...
    edgeCases: [
      "The seeded audience automation workspace can capture explicit-consent opt-ins, record unsubscribe/suppression evidence, store private owner notes, and calculate broadcast readiness, but it is not a general contact import, send queue, or full CRM database.",
      ...
      "Subscriber imports, direct agent contact writes, email sends, broadcast scheduling, private exports, and CRM automation require future confirmed-write APIs.",
      "Codex project email in issue #10 is separate from publisher/customer email workflows.",
    ],
    agentAccess:
      "Agents can read /audience/source-data, aggregate subscriber/suppression/timeline/broadcast readiness counts, and the opt-in/unsubscribe/note write boundaries. Owner sessions can inspect private rows, create private notes, and view broadcast readiness in /admin/audience. Direct agent subscriber writes, imports, sends, broadcast scheduling, private exports, or CRM automation require future authenticated confirmed-write APIs with consent, suppression, sender-domain safety, and audit correlation.",
    validation: [
      "Playwright covers /audience/source-data, aggregate subscriber/suppression/timeline/broadcast readiness redaction, /audience/indie-launch-waitlist, valid opt-in, unsubscribe, unknown-email suppression, owner /admin/audience inspection, owner CRM note creation, unauthorized note rejection, validation failures, duplicate idempotency, sitemap discovery, and agent manifest discovery.",
      "Issues #85, #103, #137, #167, #169, and #171 record the audience automation source-data contract, first live opt-in capture path, owner subscriber inspection path, unsubscribe/suppression path, owner CRM timeline note path, and broadcast readiness path.",
    ],
    sortOrder: 50,
    updatedAt: null,
  },
