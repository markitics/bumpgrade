...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    happyPath: [
      ...
      "Find seeded product types, asset IDs, access rules, entitlement templates, sandbox grant mappings, aggregate entitlement inspection counts, customer lookup contract, private R2-backed delivery contract, revision ID, and write boundary.",
      ...
      "Create and consume a short-lived download token for an active file entitlement, streaming the seeded private R2-backed fixture through Bumpgrade without exposing private R2 keys or signed object URLs.",
    ...
    edgeCases: [
      ...
      "Public /products/source-data exposes aggregate entitlement counts and redaction flags, not buyer emails, raw Stripe IDs, hashes, metadata JSON, R2 keys, or signed URLs.",
      "Arbitrary private asset uploads, signed object URLs, protected lessons, revocation, and live fulfillment automation require future confirmed-write APIs.",
      "Subscription access requires trusted billing state before membership entitlements can be granted.",
    ],
    agentAccess:
      "Agents can read /products/source-data, aggregate entitlement inspection counts, the preview route, the customer-safe checkout intent lookup contract, and the short-lived private R2-backed download-token boundary. Owner sessions can inspect private buyer entitlement rows in /admin/products. Trusted paid sandbox webhooks can grant seeded entitlement rows; direct product/access writes require actor identity, exact confirmation, idempotency, stale-state checks, audit correlation, redaction, and trusted checkout or subscription evidence in a later API.",
    validation: [
      "Playwright covers /products/source-data, aggregate entitlement inspection redaction, /products/indie-launch-library, customer /products/entitlements lookup, private R2-backed token delivery and replay rejection, owner /admin/products inspection, sitemap discovery, and agent manifest read-contract discovery.",
  ...
  },
  {
    ...
    happyPath: [
      ...
      "Create and consume a one-use download token that streams the seeded private R2-backed fixture for a file entitlement.",
    ...
    edgeCases: [
      ...
      "Arbitrary private asset uploads remain disabled.",
    ...
    agentAccess:
      "Agents can read /products/source-data, /api/products/entitlements, /api/products/download-tokens, and /commerce/source-data to understand the entitlement grant contract, customer-safe lookup, short-lived private R2-backed fixture delivery, and aggregate inspection counts. Owner sessions can inspect private buyer entitlement rows in /admin/products. Agents cannot grant, revoke, expose private buyer data, upload arbitrary assets, or issue signed object URLs without future authenticated confirmed-write APIs.",
    validation: [
      "Playwright covers source-data output, aggregate redaction, a paid test checkout webhook that creates entitlement rows, customer-safe lookup, private R2-backed token delivery and replay rejection, owner /admin/products inspection, and duplicate webhook idempotency.",
