...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    edgeCases: [
      ...
      "Public /audience/source-data exposes aggregate counts and redaction flags, not subscriber emails, names, raw IPs, raw user agents, suppression hashes, note bodies, actor emails, recipient payloads, provider message IDs, provider responses, Cloudflare Queue message bodies, send queue payloads, delivery payloads, reasons, or private metadata.",
    ...
    validation: [
      ...
      "Issues #85, #103, #137, #167, #169, #171, #173, #175, #177, #183, #189, #191, and #197 record the audience automation source-data contract, first live opt-in capture path, owner subscriber inspection path, unsubscribe/suppression path, owner CRM timeline note path, broadcast readiness path, dry-run broadcast schedule intent path, broadcast preview safety path, queue readiness path, delivery-batch dry-run path, queue-message dry-run path, dispatch-preflight dry-run path, and dispatch-attempt receipt path.",
    ],
    sortOrder: 50,
    updatedAt: null,
  },
