...
const journeyProofByFeatureId: Record<string, AdminUserJourneyProof> = {
  "feature-funnel-builder": {
    ...
    summary: "Funnel previews, template library, draft admin, publishing, checkout linking, resource delivery links, block reordering, and webinar/resource shapes have screenshot evidence.",
    ...
    notes: [
      "Issue #14 is the shipped funnel MVP proof. Issue #417 now includes owner-confirmed checkout unlinking, resource delivery links, and owner-session within-step block reordering; live billing stays in issue #219; physical deletion, freeform drag-and-drop editing, arbitrary private R2 delivery, live fulfillment automation, webinar integrations, and direct public agent writes stay in issue #417.",
      "Issue #215 adds owner-confirmed private draft duplication without copying checkout-link or resource-link metadata; issue #341 adds owner-confirmed archive/unpublish without deleting evidence; issue #417 adds owner-confirmed checkout unlinking, resource delivery links, and within-step block reordering; issue #430 adds owner-session block title/body editing while preserving block metadata; issue #432 adds owner-session reusable block add/remove while refusing checkout-linked block removal.",
    ],
  },
...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    validation: [
      ...
      "Issue #417 records owner-confirmed checkout unlinking, owner-confirmed resource delivery links, owner-session within-step block reordering, and the remaining advanced funnel parity follow-up after MVP closeout.",
    ],
    sortOrder: 46,
    updatedAt: null,
  },
  {
    ...
    edgeCases: [
      ...
      "Checkout-link deletion, physical deletion, freeform drag-and-drop layout editing, cross-step block moves, live billing, direct agent resource delivery linking, arbitrary private R2 delivery, signed URLs, live fulfillment automation, direct agent block editing, direct agent block add/remove, direct agent block reordering, direct agent duplication, and direct agent writes still require future confirmed-write APIs.",
    ...
    agentAccess:
      "Agents can read public /funnels/source-data, seeded funnel routes, reusable templates including webinar/resource page shapes, draft duplication capability metadata, block-edit capability metadata, block add/remove capability metadata, block reorder capability metadata, checkout-link capability metadata, resource-delivery-link capability metadata, public funnel checkout-start capability metadata, archive/unpublish lifecycle metadata, and published D1 funnel routes. Owner-session UI may create from templates, duplicate, edit steps, edit existing block copy, add reusable blocks, remove safe unlinked blocks, reorder existing blocks within a step, link checkout offers, link resource delivery, preview, publish, archive, and unpublish private draft steps with actor identity, confirmation where required, idempotency, audit correlation, stale-state checks, and redaction; direct agent edit/duplicate/archive/resource-link/block-reorder tools are still planned.",
    validation: [
      "Playwright covers the owner-gated /admin/funnels surface, webinar/resource template records, template-to-draft create path, draft duplicate path, granular block-edit path, block add/remove path, block reorder path, linked-checkout block removal refusal, checkout-link create path, resource delivery link path, idempotent replay, stale checkout-link/resource-link, block-edit, block-add, and block-move rejection, seed/update/reorder/publish/archive POST paths, stale publish/archive rejection, archived draft read-only behavior, private draft preview, public D1 funnel route rendering, public linked-checkout start rendering, public resource-link rendering, archive removal from /funnels/source-data, /funnels/source-data capability metadata, and agent manifest discovery.",
