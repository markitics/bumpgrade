...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    userGoal:
      "Inspect affiliate programs, partner records, referral links, attribution windows, commission rules, payout review states, fraud flags, and aggregate click counts before buyer attribution or payouts exist.",
    sourceEvidence: [
    ...
    edgeCases: [
      "Public source-data exposes aggregate click counts only, not raw click rows.",
    ...
    agentAccess:
      "Agents can read /affiliates/source-data, the preview route, and click capture boundaries. Affiliate attribution, commission, payout, fraud, tax, partner notification, and direct agent affiliate writes require future authenticated confirmed-write APIs with actor identity, explicit confirmation, idempotency, stale-state checks, audit correlation, redaction, refund-window checks, payout review, and private payout data boundaries.",
    validation: [
      "Playwright covers /affiliates/source-data, /affiliates/indie-launch-partners, click ingestion, duplicate idempotency, validation failures, aggregate-only source-data, sitemap discovery, and agent manifest discovery.",
      "Issues #89 and #109 record the affiliate/referral source-data scaffold and first privacy-safe click capture path.",
    ],
    sortOrder: 52,
    updatedAt: null,
  },
