...
const journeyProofByFeatureId: Record<string, AdminUserJourneyProof> = {
  "feature-funnel-builder": {
    ...
    notes: [
      "Issue #14 is the shipped funnel MVP proof. Live billing stays in issue #219; physical deletion, drag-and-drop editing, block add/remove, private resource delivery, webinar integrations, and direct public agent writes stay in issue #417.",
      "Issue #215 adds owner-confirmed private draft duplication without copying checkout-link metadata; issue #341 adds owner-confirmed archive/unpublish without deleting evidence; issue #430 adds owner-session block title/body editing while preserving block metadata.",
    ],
  },
...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    happyPath: [
      ...
      "Use the write boundary to avoid claiming live billing, physical deletion, block add/remove, direct agent archive/unpublish, direct agent checkout-link, direct agent block editing, or direct agent-write capability.",
    ],
    edgeCases: [
      "The seeded funnel is read-only and not an authenticated builder UI.",
      "Owner-session template-to-draft creation, private draft duplication, granular block copy editing, checkout-offer linking, and archive/unpublish lifecycle actions are available from /admin/funnels, and published linked checkout blocks can render the existing sandbox checkout start surface. Block copy edits preserve IDs, kinds, and checkout-link metadata. Duplicates stay private and strip checkout-link metadata. Webinar/resource templates do not schedule webinars, host replays, deliver private files, or grant entitlements. Physical deletion, block add/remove, live billing, direct agent archive/unpublish, direct agent block editing, and direct agent edits require future confirmed-write APIs.",
      "Generated copy remains draft until a publisher confirms it.",
    ],
    agentAccess:
      "Agents can read /funnels/source-data, reusable template and block-template records, webinar/resource page-shape records, draft duplication capability metadata, granular block-edit capability metadata, checkout-link capability metadata, public funnel checkout-start capability metadata, archive/unpublish lifecycle metadata, the seeded preview route, and published D1 funnel routes. Owner-session template-to-draft creation, private draft duplication, block copy editing, checkout-offer linking, and archive/unpublish actions require owner auth, idempotency, and stale-state checks; direct agent writes require actor identity, confirmation, idempotency, stale-state checks, audit correlation, redaction, and rollback notes.",
    validation: [
  ...
  },
  {
    ...
    edgeCases: [
      ...
      "Checkout-link deletion, block add/remove, physical deletion, drag-and-drop layout editing, live billing, direct agent block editing, direct agent duplication, and direct agent writes still require future confirmed-write APIs.",
