...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    issueNumbers: [14, 79, 159, 161],
    primaryUser: "Publisher or agent planning the first funnel",
    userGoal:
      "Inspect an ordered opt-in, sales, and thank-you funnel plus reusable templates and block records before visual editing, direct agent template creation, and checkout-linking writes exist.",
    sourceEvidence: [
    ...
    edgeCases: [
      ...
      "Owner-session template-to-draft creation is available from /admin/funnels, but checkout linking, deletion, unpublishing, and direct agent edits require future confirmed-write APIs.",
      "Generated copy remains draft until a publisher confirms it.",
    ],
    agentAccess:
      "Agents can read /funnels/source-data, reusable template and block-template records, the seeded preview route, and published D1 funnel routes. Owner-session template-to-draft creation requires confirmation and idempotency; direct agent writes require actor identity, confirmation, idempotency, stale-state checks, audit correlation, redaction, and rollback notes.",
    validation: [
  ...
  },
  {
    ...
    featureId: "feature-funnel-builder",
    featureStatus: "pending",
    issueNumbers: [14, 79, 91, 93, 95, 135, 159, 161],
    primaryUser: "Publisher or owner preparing the first launch funnel",
    userGoal: "Create, seed, or template-start a D1-backed draft funnel, tune the ordered steps, preview it privately, then publish it to a public route after exact confirmation.",
    sourceEvidence: [
    ...
    agentAccess:
      "Agents can read public /funnels/source-data, seeded funnel routes, reusable templates, and published D1 funnel routes. Owner-session UI may create from templates, edit, preview, and publish private draft steps with actor identity, confirmation, idempotency, audit correlation, stale-state checks, and redaction; direct agent edit tools are still planned.",
    validation: [
      "Playwright covers the owner-gated /admin/funnels surface, template-to-draft create path, idempotent replay, seed/update/reorder/publish POST paths, stale publish rejection, private draft preview, public D1 funnel route rendering, /funnels/source-data capability metadata, and agent manifest discovery.",
