...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    userGoal:
      "Inspect affiliate programs, partner records, referral links, attribution windows, commission rules, payout review states, fraud flags, aggregate click counts, and checkout attribution evidence before commissions or payouts exist.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Find the seeded affiliate program, revision ID, partner IDs, referral link IDs, attribution rule IDs, commission rule IDs, ledger IDs, payout batch ID, review flags, click capture API, and write boundary.",
      ...
      "Confirm duplicate idempotency returns the same public-safe click without duplicating rows.",
      "Open /affiliates/indie-launch-partners to inspect the public preview.",
      "Use /commerce/source-data and /offers/source-data to distinguish checkout attribution evidence from payable commissions.",
    ],
    edgeCases: [
      "Public source-data exposes aggregate click and checkout attribution counts only, not raw click or checkout rows.",
      "Unsupported referral link IDs, codes, destination routes, and missing idempotency keys return public-safe validation errors.",
      "Cookie assignment, buyer attribution finalization, payout accounts, tax forms, fraud enforcement, Stripe payouts, and partner notifications require future confirmed-write APIs.",
      "Agents must not call fixture commission amounts payable or published affiliate terms.",
    ],
    agentAccess:
      "Agents can read /affiliates/source-data, /commerce/source-data, the preview route, click capture boundaries, and checkout attribution boundaries. Commission, payout, fraud, tax, partner notification, and direct agent affiliate writes require future authenticated confirmed-write APIs with actor identity, explicit confirmation, idempotency, stale-state checks, audit correlation, redaction, refund-window checks, payout review, and private payout data boundaries.",
    validation: [
      "Playwright covers /affiliates/source-data, /affiliates/indie-launch-partners, click ingestion, checkout attribution, duplicate idempotency, validation failures, aggregate-only source-data, sitemap discovery, and agent manifest discovery.",
      "Issues #89, #109, and #111 record the affiliate/referral source-data scaffold, first privacy-safe click capture path, and first checkout attribution evidence path.",
    ],
    sortOrder: 52,
    updatedAt: null,
  },
