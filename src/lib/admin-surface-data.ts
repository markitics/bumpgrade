...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    happyPath: [
      ...
      "Use the write boundary to avoid claiming live billing, non-archived purge, direct agent purge, direct agent block reordering, direct agent cross-step block moves, direct agent checkout unlinking, direct agent-created delivery tokens, direct agent public publishing, direct agent checkout-link, direct agent block add/remove, arbitrary uploaded private asset delivery, signed URLs, live fulfillment automation, live webinar scheduling, attendance tracking, replay hosting, or broad direct agent-write capability beyond the owner-session private draft write API.",
    ],
    edgeCases: [
      "The seeded funnel is read-only and not an authenticated builder UI.",
      "Owner-session template-to-draft creation, private draft duplication, granular block copy editing, reusable block add/remove, within-step block reordering, drag/drop block placement through the same move endpoints, cross-step block moves, checkout-offer linking, checkout unlinking, resource delivery linking, webinar event/replay linking, archive/unpublish lifecycle actions, and archived-draft purge are available from /admin/funnels, and published linked checkout blocks can render the existing sandbox checkout start surface. Owner-session agents can also call /api/agent/funnels/draft-writes for private block copy edits, resource-delivery linking, webinar-event linking, private draft duplication, and archive/unpublish after exact confirmation, idempotency, current revision checks, and audit correlation. Published linked resource blocks can mint scoped Bumpgrade download routes only when checkout intent and entitlement scope match the linked product and file asset. Block copy edits preserve IDs, kinds, checkout-link metadata, resource-link metadata, and webinar-link metadata. Direct agent resource-delivery and webinar-event links preserve existing block identity and do not mutate public routes. Block reordering preserves step membership and checkout/resource/webinar metadata. Cross-step block moves preserve block metadata while changing step membership and refuse to empty the source step. Block removal refuses checkout-linked blocks until the dedicated unlink action clears checkout metadata. Duplicates stay private and strip checkout-link/resource-link/webinar-link metadata. Archived-draft purge records tombstone evidence before deleting draft and step rows, without deleting prior audit rows, product assets, R2 objects, buyer records, or billing state. Webinar/resource templates do not schedule webinars, track attendance, host replays, deliver arbitrary uploaded private files, create signed URLs, automate fulfillment, or grant entitlements. Non-archived purge, bulk purge, live billing, freeform canvas styling, direct agent cross-step block moves, direct agent purge, direct agent checkout unlinking, direct agent-created delivery tokens, direct agent block add/remove, direct agent block reordering, direct agent public publishing, and broader direct agent edits require future confirmed-write APIs.",
  ...
  },
  {
    ...
    edgeCases: [
      ...
      "Checkout-link deletion, non-archived purge, bulk purge, freeform canvas layout styling, live billing, direct agent cross-step block moves, direct agent purge, direct agent-created delivery tokens, arbitrary uploaded private asset delivery, signed URLs, live fulfillment automation, live webinar scheduling, attendance tracking, replay hosting, direct agent block add/remove, direct agent block reordering, direct agent public publishing, and broad direct agent writes beyond private block copy edits, resource-delivery linking, webinar-event linking, private duplication, and archive/unpublish still require future confirmed-write APIs.",
