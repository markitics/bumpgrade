...
const journeyProofByFeatureId: Record<string, AdminUserJourneyProof> = {
  ...
  "feature-email-automation-crm": {
    ...
    method:
      "Opt-in, suppression, CRM note, sequence Queue consumer, broadcast readiness, queue, provider, and dispatch readiness route checks.",
    summary:
...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    validation: [
      ...
      "Issues #85, #103, #137, #167, #169, #171, #173, #175, #177, #183, #189, #191, #197, #199, #201, #203, #205, #207, #209, #211, #253, #259, #347, #351, #354, #358, #360, #362, #364, #366, and #368 record the audience automation source-data contract, first live opt-in capture path, owner subscriber inspection path, unsubscribe/suppression path, owner CRM timeline note path, broadcast readiness path, dry-run broadcast schedule intent path, broadcast preview safety path, queue readiness path, delivery-batch dry-run path, queue-message dry-run path, dispatch-preflight dry-run path, dispatch-attempt receipt path, sender-domain readiness path, provider-event readiness path, provider rate-limit readiness path, provider response readiness path, send-payload readiness path, Queue producer readiness path, Queue consumer readiness path, owner import-intent path, owner import-preflight path, aggregate export-readiness path, aggregate sequence-delivery-readiness path, dry-run sequence schedule-intent path, dry-run sequence delivery-batch path, dry-run sequence queue-message path, dry-run sequence dispatch-preflight path, dry-run sequence dispatch-attempt path, sequence Queue producer readiness path, and sequence Queue consumer readiness path.",
    ],
    sortOrder: 50,
    updatedAt: null,
  },
