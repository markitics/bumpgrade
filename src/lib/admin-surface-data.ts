...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    happyPath: [
      ...
      "Find seeded product types, asset IDs, access rules, entitlement templates, sandbox grant mappings, aggregate entitlement inspection counts, revision ID, and write boundary.",
    ...
    edgeCases: [
      ...
      "Private R2 keys, signed URLs, protected lessons, revocation, customer self-service entitlement inspection, and live fulfillment require future confirmed-write APIs.",
      "Subscription access requires trusted billing state before membership entitlements can be granted.",
    ],
    agentAccess:
      "Agents can read /products/source-data, aggregate entitlement inspection counts, and the preview route. Owner sessions can inspect private buyer entitlement rows in /admin/products. Trusted paid sandbox webhooks can grant seeded entitlement rows; direct product/access writes require actor identity, exact confirmation, idempotency, stale-state checks, audit correlation, redaction, and trusted checkout or subscription evidence in a later API.",
    validation: [
      "Playwright covers /products/source-data, aggregate entitlement inspection redaction, /products/indie-launch-library, owner /admin/products inspection, sitemap discovery, and agent manifest read-contract discovery.",
  ...
  },
  {
    ...
    agentAccess:
      "Agents can read /products/source-data and /commerce/source-data to understand the entitlement grant contract and aggregate inspection counts. Owner sessions can inspect private buyer entitlement rows in /admin/products. Agents cannot grant, revoke, inspect private buyer access, or issue signed download URLs without future authenticated confirmed-write APIs.",
    validation: [
      "Playwright covers source-data output, aggregate redaction, a paid test checkout webhook that creates entitlement rows, owner /admin/products inspection, and duplicate webhook idempotency.",
