...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    userGoal:
      "Inspect products, assets, access rules, entitlement templates, sandbox entitlement grant mappings, owner entitlement rows, and revocation intent readiness before destructive access removal is enabled.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Inspect protected content readiness in /admin/products without delivering protected bodies.",
    ...
    edgeCases: [
      ...
      "Protected content records are readiness metadata only until future delivery APIs enforce entitlement, subscription, stale-state, audit, and redaction checks.",
      "Signed object URLs, protected lessons, destructive revocation, customer delivery of arbitrary uploads, and live fulfillment automation require future APIs.",
      "Subscription access requires trusted billing state before membership entitlements can be granted.",
    ],
    agentAccess:
      "Agents can read /products/source-data, aggregate entitlement inspection counts, the preview route, the customer-safe checkout intent lookup contract, the short-lived private R2-backed download-token boundary with redemption revalidation, the owner-authenticated private asset upload intent boundary, non-destructive revocation intent readiness, and protected content readiness. Owner sessions can inspect private buyer entitlement rows, revocation intent records, and protected content readiness in /admin/products and can create small private asset upload records only through exact-confirmed, idempotent, revision-checked writes. Trusted paid sandbox webhooks can grant seeded entitlement rows; direct customer delivery of arbitrary uploads, destructive revocation, and protected content delivery require later APIs.",
    validation: [
  ...
  },
  {
    ...
    userGoal:
      "Confirm that a paid sandbox checkout webhook can grant product entitlements, queue fulfillment evidence, and expose revocation intent readiness without removing access or exposing private assets.",
    sourceEvidence: [
      "https://bumpgrade.com/products/source-data",
      "https://bumpgrade.com/products/entitlements",
      "https://bumpgrade.com/api/products/entitlements",
    ...
    edgeCases: [
      ...
      "Protected content records are non-delivery readiness metadata until future entitlement, subscription, stale-state, audit, and redaction checks exist.",
      "Signed downloads, protected content, customer portal, refunds, destructive revocation, and direct customer delivery of arbitrary uploads require later confirmed-write APIs.",
      "Raw Stripe identifiers and buyer email remain server-private.",
    ],
    agentAccess:
      "Agents can read /products/source-data, /api/products/entitlements, /api/products/download-tokens, and /commerce/source-data to understand the entitlement grant contract, customer-safe lookup, short-lived private R2-backed fixture delivery with redemption revalidation, owner-upload intent boundary, revocation intent readiness, protected content readiness, and aggregate inspection counts. Owner sessions can inspect private buyer entitlement rows, revocation intent records, and protected content readiness in /admin/products and create small private asset upload records via exact-confirmed, idempotent, revision-checked writes. Agents cannot grant, destructively revoke, expose private buyer data, deliver protected bodies, deliver arbitrary uploads to customers, or issue signed object URLs without future authenticated confirmed-write APIs.",
    validation: [
  ...
  },
  {
    ...
    happyPath: [
      ...
      "Open /admin/audience as a verified owner to inspect private subscriber rows, active tags, consent counts, draft sequence enrollment state, suppression totals, private CRM timeline notes, broadcast readiness, dry-run schedule intents, preview safety, and queue readiness.",
    ...
    edgeCases: [
      "The seeded audience automation workspace can capture explicit-consent opt-ins, record unsubscribe/suppression evidence, store private owner notes, calculate broadcast readiness, record dry-run schedule intent metadata, expose preview safety metadata, and expose queue readiness metadata, but it is not a general contact import, live queue producer, provider-send path, or full CRM database.",
      "Public /audience/source-data exposes aggregate counts and redaction flags, not subscriber emails, names, raw IPs, raw user agents, suppression hashes, note bodies, actor emails, recipient payloads, provider message IDs, send queue payloads, reasons, or private metadata.",
    ...
    validation: [
      "Playwright covers /audience/source-data, aggregate subscriber/suppression/timeline/broadcast readiness/schedule intent/preview safety/queue readiness redaction, /audience/indie-launch-waitlist, valid opt-in, unsubscribe, unknown-email suppression, owner /admin/audience inspection, owner CRM note creation, owner schedule-intent creation, unauthorized note/schedule-intent rejection, validation failures, duplicate idempotency, sitemap discovery, and agent manifest discovery.",
      "Issues #85, #103, #137, #167, #169, #171, #173, #175, and #177 record the audience automation source-data contract, first live opt-in capture path, owner subscriber inspection path, unsubscribe/suppression path, owner CRM timeline note path, broadcast readiness path, dry-run broadcast schedule intent path, broadcast preview safety path, and queue readiness path.",
    ],
    sortOrder: 50,
    updatedAt: null,
  },
