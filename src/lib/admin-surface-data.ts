...
const journeyProofByFeatureId: Record<string, AdminUserJourneyProof> = {
  ...
  "feature-affiliates-referrals": {
    status: "passed",
    lastTestedAt: "2026-05-21T09:54:00.000Z",
    environment: "Production screenshot and affiliate/referral source-data evidence.",
    method:
      "Referral click, checkout attribution, commission ledger, review action, partner report, payout-prep, owner payout-preparation record, and owner fraud-review record route checks.",
    summary:
      "Referral and affiliate evidence surfaces are proven; payout and fraud review records are owner-confirmed while payout mutation and fraud enforcement remain future-gated.",
    ciLinks: [{ label: "GitHub Actions", url: "https://github.com/markitics/bumpgrade/actions", kind: "ci" }],
    screenshotLinks: [
    ...
    notes: ["Payable commissions, fraud enforcement, Stripe payouts, payout accounts, tax records, private fraud signals, and partner notifications remain outside the current proof."],
  },
...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    featureStatus: "launch-preview",
    issueNumbers: [19, 89, 109, 111, 113, 115, 193, 195, 273, 275],
    ...
    happyPath: [
      ...
      "Find the seeded affiliate program, revision ID, partner IDs, partner report IDs, payout preparation IDs, payout preparation record IDs, fraud review record IDs, referral link IDs, attribution rule IDs, commission rule IDs, fixture ledger IDs, payout batch ID, review flags, click capture API, checkout attribution contract, review-only commission ledger contract, owner review action contract, partner report contract, payout preparation contract, owner payout preparation record contract, owner fraud review record contract, and write boundary.",
      ...
      "Read fraudReviewRecords to inspect aggregate owner-reviewed fraud review evidence without private fraud signals or raw rows.",
      ...
      "As an owner, POST payout preparation and fraud review records through /admin/affiliates with exact confirmation, idempotency, current revision checks, payout batch status checks, and public-safe redaction.",
    ...
    edgeCases: [
      "Public source-data exposes aggregate click, checkout attribution, commission ledger, owner action, partner report, payout preparation, payout preparation record, and fraud review record counts only, not raw click, checkout, buyer, actor, reason, private fraud signal, tax, payout, partner notification, or Stripe rows.",
      ...
      "Agents must not call review-only commission evidence, partner report totals, payout preparation rows, payout preparation records, or fraud review records payable, fraud enforcement, or published affiliate terms.",
    ],
    agentAccess:
      "Agents can read /affiliates/source-data, /commerce/source-data, preview routes, click capture boundaries, checkout attribution boundaries, review-only commission ledger boundaries, owner review action boundaries, public-safe partner reports, read-only payout preparation, owner-confirmed payout preparation records, and owner-reviewed fraud review records. Payout execution, fraud enforcement, tax, partner notification, private partner portal access, partner payout account storage, buyer attribution finalization, and direct agent affiliate writes require future authenticated confirmed-write APIs with actor identity, explicit confirmation, idempotency, stale-state checks, audit correlation, redaction, refund-window checks, payout review, private fraud review, and private payout data boundaries.",
    validation: [
  ...
  },
  {
    ...
    userGoal:
      "Inspect read-only payout preparation rows, checklist blockers, partner-report links, owner-confirmed payout preparation records, and owner-reviewed fraud review records before any Stripe payout, tax, payout account, partner notification, or fraud enforcement capability exists.",
    sourceEvidence: [
      "https://bumpgrade.com/affiliates/source-data",
      "https://bumpgrade.com/affiliates/indie-launch-partners",
      "https://bumpgrade.com/api/admin/affiliates/payout-preparation-records",
    ...
    happyPath: [
      "Fetch /affiliates/source-data.",
      "Find payoutPreparationContract, payoutPreparationSummary, payoutPreparationRecords, and fraudReviewRecords.",
      "Inspect each payoutPreparationId, linked payoutBatchId, partnerReportIds, eligible ledger IDs, blocked ledger IDs, reversed ledger IDs, readiness checklist, fraud review flag, and public-safe aggregate totals.",
      ...
      "Treat the preparation and fraud review rows as evidence only; do not call them payable commission state, partner statements, or fraud enforcement.",
    ],
    edgeCases: [
      "Refund-window, owner-hold, and self-referral evidence keep preparation blocked until future private review contracts exist.",
      "Public preparation rows, owner-confirmed payout records, and owner-reviewed fraud records never expose buyer data, raw ledger rows, raw click rows, raw checkout rows, raw actor identity, private fraud signals, partner payout accounts, tax forms, Stripe payout identifiers, or partner notification payloads.",
      "Stripe payouts, transfer creation, private partner portals, payout account storage, tax collection, fraud enforcement, payable commission finalization, and direct agent writes require future confirmed-write APIs.",
    ],
    agentAccess:
    ...
    validation: [
      "Playwright covers payoutPreparationContract, payoutPreparationSummary, owner-confirmed payout preparation records, owner-reviewed fraud review records, preview rendering, admin journey exposure, agent manifest stable IDs, and redaction boundaries.",
      "Issues #195, #273, and #275 record the read-only payout preparation slice, owner-confirmed payout preparation records, and owner-reviewed fraud review records.",
    ],
    sortOrder: 59,
    updatedAt: null,
  },
