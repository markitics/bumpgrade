...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    featureStatus: "pending",
    issueNumbers: [18, 87, 105, 107, 119, 121],
    primaryUser: "Publisher or agent optimizing a launch funnel",
    userGoal:
      "Inspect seeded analytics definitions, capture privacy-safe test events, record browser-side funnel page views, assign deterministic variants, and read aggregate conversion report rows before cookies, contact-level reporting, or automated decisions exist.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Find event IDs, page-view beacon boundary, metric IDs, aggregate event counts, aggregate conversion report rows, experiment IDs, variant IDs, assignment rule, assignment API, and write boundary.",
      "Open /funnels/indie-launch-sandbox and let the session-idempotent page-view beacon record a seeded event.",
    ...
    agentAccess:
      "Agents can read /analytics/source-data, event capture boundaries, page-view beacon boundaries, assignment boundaries, and aggregate conversion report rows. Direct agent analytics writes, custom events, campaign attribution mutation, experiment routing, and automated decisions require future authenticated confirmed-write APIs with privacy review, idempotency, stale-state checks, audit correlation, redaction, retention limits, and sample-size caveats.",
    validation: [
      "Playwright covers /analytics/source-data, /analytics/indie-launch-dashboard, event ingestion, page-view beacon capture, bot suppression, assignment ingestion, conversion reporting from captured events, duplicate idempotency, deterministic assignment, validation failures, opt-in event recording, sitemap discovery, and agent manifest discovery.",
      "Issues #87, #105, #107, #119, and #121 record the analytics source-data scaffold, first privacy-safe event capture path, first deterministic assignment path, first aggregate conversion report, and first browser-side funnel page-view beacon.",
    ],
    sortOrder: 51,
    updatedAt: null,
  },
  {
    ...
    happyPath: [
      "Capture seeded analytics events with idempotency or visit the public funnel preview to emit a session-idempotent page-view event.",
      "Fetch /analytics/source-data.",
    ...
    agentAccess:
      "Agents can read aggregate funnel conversion rows and cite metric IDs, event IDs, and issues #119/#121 evidence. Direct analytics writes, traffic routing, and automated experiment decisions require future confirmed-write APIs.",
    validation: [
      "Playwright seeds captured events, visits the funnel page-view beacon, replays duplicate idempotency, verifies conversion counts and rates, and checks public source-data excludes raw/private rows.",
      "Issues #119 and #121 record the aggregate conversion report and browser-side funnel page-view instrumentation.",
    ],
    sortOrder: 55,
    updatedAt: null,
  },
