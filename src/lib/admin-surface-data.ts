...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    userGoal:
      "Inspect products, assets, access rules, entitlement templates, sandbox entitlement grant mappings, subscription-backed membership access, owner entitlement rows, revocation intent readiness, and protected fixture delivery checks before real private content storage is enabled.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Find seeded product types, asset IDs, access rules, entitlement templates, sandbox grant mappings, subscription membership access contract, aggregate entitlement inspection counts, customer lookup contract, private R2-backed delivery contract, owner-upload intent contract, revocation intent readiness, protected content readiness, protected fixture delivery contract, revision ID, and write boundary.",
      ...
      "Inspect revocation intent readiness in /admin/products without removing access.",
    ...
    edgeCases: [
      ...
      "Revocation intent records are non-destructive until future exact-confirmed APIs enforce stale-state, reason, notification, and audit checks.",
  ...
  },
  {
    ...
    userGoal:
      "Confirm that a paid sandbox checkout webhook can grant product entitlements, trusted Stripe Billing state can sync membership access, queued fulfillment evidence stays public-safe, revocation intent readiness is explicit, and protected fixtures return only for active eligible checkout-linked entitlements.",
    sourceEvidence: [
      "https://bumpgrade.com/products/source-data",
      "https://bumpgrade.com/products/entitlements",
      "https://bumpgrade.com/api/products/entitlements",
    ...
    happyPath: [
      ...
      "Open /admin/products as a verified owner to inspect the entitlement rows, queued fulfillment evidence, revocation intent readiness, and protected content readiness.",
    ],
    edgeCases: [
      ...
      "Revocation intents are non-destructive until future exact-confirmed APIs enforce stale-state, reason, notification, and audit checks.",
      "Protected fixture delivery rejects mismatched entitlements and stale checkout state.",
      "Canceled, unpaid, incomplete_expired, or deleted subscriptions pause membership access without exposing raw subscription/customer IDs, Customer Portal URLs, member posts, private files, or progress rows.",
    ...
    agentAccess:
      "Agents can read /products/source-data, /api/products/entitlements, /api/products/download-tokens, /api/products/protected-content, and /commerce/source-data to understand the entitlement grant contract, customer-safe lookup, short-lived private R2-backed fixture delivery with redemption revalidation, owner-upload intent boundary, revocation intent readiness, protected content readiness, protected fixture delivery checks, subscription-backed membership access, and aggregate inspection counts. Owner sessions can inspect private buyer entitlement rows, revocation intent records, and protected content readiness in /admin/products and create small private asset upload records via exact-confirmed, idempotent, revision-checked writes. Agents cannot grant, destructively revoke, expose private buyer data, deliver arbitrary protected bodies, deliver arbitrary uploads to customers, mutate subscriptions, create Customer Portal sessions, or issue signed object URLs without future authenticated confirmed-write APIs.",
    validation: [
      "Playwright covers source-data output, aggregate redaction, a paid test checkout webhook that creates entitlement rows, customer-safe lookup, private R2-backed token delivery, current checkout-state revalidation, replay rejection, protected fixture delivery and stale-state rejection, subscription-backed membership access activation/inactivation, owner private asset upload intent creation, idempotent replay, unauthorized rejection, owner /admin/products inspection, revocation intent readiness, protected content readiness, and duplicate webhook idempotency.",
  ...
  },
  {
    ...
    edgeCases: [
      "The seeded audience automation workspace can capture explicit-consent opt-ins, record unsubscribe/suppression evidence, store private owner notes, calculate broadcast readiness, record dry-run schedule intent metadata, expose preview safety metadata, expose queue readiness metadata, record delivery-batch dry runs, record aggregate dry-run queue-message evidence, and record dispatch preflight evidence, but it is not a general contact import, live Cloudflare Queue producer, provider-send path, or full CRM database.",
      "Public /audience/source-data exposes aggregate counts and redaction flags, not subscriber emails, names, raw IPs, raw user agents, suppression hashes, note bodies, actor emails, recipient payloads, provider message IDs, Cloudflare Queue message bodies, send queue payloads, delivery payloads, reasons, or private metadata.",
      "The unsubscribe API returns the submitted normalized email to the submitter but does not reveal list membership.",
      "Subscriber imports, direct agent contact writes, real email sends, private exports, and CRM automation require future confirmed-write APIs.",
      "Codex project email in issue #10 is separate from publisher/customer email workflows.",
    ],
    agentAccess:
      "Agents can read /audience/source-data, aggregate subscriber/suppression/timeline/broadcast readiness/schedule intent/preview safety/queue readiness/delivery-batch/queue-message/dispatch-preflight counts, and the opt-in/unsubscribe/note/schedule-intent/delivery-batch/queue-message/dispatch-preflight write boundaries. Owner sessions can inspect private rows, create private notes, view broadcast readiness, inspect preview safety and queue readiness, and record dry-run schedule intents, delivery-batch dry runs, queue-message dry runs, and dispatch preflight dry runs in /admin/audience. Direct agent subscriber writes, imports, real sends, private exports, or CRM automation require future authenticated confirmed-write APIs with consent, suppression, sender-domain safety, queue safety, provider limits, and audit correlation.",
    validation: [
      "Playwright covers /audience/source-data, aggregate subscriber/suppression/timeline/broadcast readiness/schedule intent/preview safety/queue readiness/delivery-batch/queue-message/dispatch-preflight redaction, /audience/indie-launch-waitlist, valid opt-in, unsubscribe, unknown-email suppression, owner /admin/audience inspection, owner CRM note creation, owner schedule-intent creation, owner delivery-batch creation, owner queue-message creation, owner dispatch-preflight creation, unauthorized note/schedule-intent/delivery-batch/queue-message/dispatch-preflight rejection, validation failures, duplicate idempotency, sitemap discovery, and agent manifest discovery.",
      "Issues #85, #103, #137, #167, #169, #171, #173, #175, #177, #183, #189, and #191 record the audience automation source-data contract, first live opt-in capture path, owner subscriber inspection path, unsubscribe/suppression path, owner CRM timeline note path, broadcast readiness path, dry-run broadcast schedule intent path, broadcast preview safety path, queue readiness path, delivery-batch dry-run path, queue-message dry-run path, and dispatch-preflight dry-run path.",
    ],
    sortOrder: 50,
    updatedAt: null,
  },
  ...
  },
  {
    ...
    userGoal:
      "Inspect affiliate programs, partner records, referral links, attribution windows, commission rules, public-safe partner reports, review-only ledger evidence, owner review/reversal state, payout review states, fraud flags, aggregate click counts, and checkout attribution evidence before payouts exist.",
    sourceEvidence: [
    ...
    happyPath: [
      ...
      "Find the seeded affiliate program, revision ID, partner IDs, partner report IDs, referral link IDs, attribution rule IDs, commission rule IDs, fixture ledger IDs, payout batch ID, review flags, click capture API, checkout attribution contract, review-only commission ledger contract, owner review action contract, partner report contract, and write boundary.",
    ...
    edgeCases: [
      "Public source-data exposes aggregate click, checkout attribution, commission ledger, owner action, and partner report counts only, not raw click, checkout, buyer, actor, reason, tax, payout, or Stripe rows.",
      ...
      "Agents must not call review-only commission evidence or partner report totals payable or published affiliate terms.",
    ],
    agentAccess:
    ...
    validation: [
      ...
      "Issues #89, #109, #111, #113, #115, and #193 record the affiliate/referral source-data scaffold, privacy-safe click capture, checkout attribution evidence, first review-only ledger evidence path, owner review/reversal action boundary, and public-safe partner report contract.",
    ],
    sortOrder: 52,
    updatedAt: null,
  },
