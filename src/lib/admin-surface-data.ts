...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    happyPath: [
      ...
      "Create a short-lived sandbox download token for an active file entitlement without exposing private R2 keys or signed object URLs.",
    ...
    edgeCases: [
      ...
      "Private R2-backed delivery, signed object URLs, protected lessons, revocation, and live fulfillment require future confirmed-write APIs.",
      "Subscription access requires trusted billing state before membership entitlements can be granted.",
    ],
    agentAccess:
      "Agents can read /products/source-data, aggregate entitlement inspection counts, the preview route, the customer-safe checkout intent lookup contract, and the short-lived sandbox download-token boundary. Owner sessions can inspect private buyer entitlement rows in /admin/products. Trusted paid sandbox webhooks can grant seeded entitlement rows; direct product/access writes require actor identity, exact confirmation, idempotency, stale-state checks, audit correlation, redaction, and trusted checkout or subscription evidence in a later API.",
    validation: [
      "Playwright covers /products/source-data, aggregate entitlement inspection redaction, /products/indie-launch-library, customer /products/entitlements lookup, sandbox download-token creation and replay rejection, owner /admin/products inspection, sitemap discovery, and agent manifest read-contract discovery.",
      ...
      "Issue #143 records the short-lived sandbox download-token path.",
    ],
    sortOrder: 49,
    updatedAt: null,
  },
  {
    ...
    happyPath: [
      ...
      "Create and consume a one-use sandbox download token for a file entitlement.",
    ...
    edgeCases: [
      ...
      "Private R2-backed asset delivery remains disabled.",
    ...
    agentAccess:
      "Agents can read /products/source-data, /api/products/entitlements, /api/products/download-tokens, and /commerce/source-data to understand the entitlement grant contract, customer-safe lookup, short-lived sandbox delivery, and aggregate inspection counts. Owner sessions can inspect private buyer entitlement rows in /admin/products. Agents cannot grant, revoke, expose private buyer data, or issue private R2-backed downloads without future authenticated confirmed-write APIs.",
    validation: [
      "Playwright covers source-data output, aggregate redaction, a paid test checkout webhook that creates entitlement rows, customer-safe lookup, sandbox download-token creation and replay rejection, owner /admin/products inspection, and duplicate webhook idempotency.",
