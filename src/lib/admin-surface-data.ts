...
const fallbackUserJourneys: AdminUserJourney[] = [
  ...
  },
  {
    ...
    validation: [
      ...
      "Issues #85, #103, #137, #167, #169, #171, #173, #175, #177, #183, #189, #191, #197, #199, #201, #203, #205, #207, #209, #211, #253, #259, and #347 record the audience automation source-data contract, first live opt-in capture path, owner subscriber inspection path, unsubscribe/suppression path, owner CRM timeline note path, broadcast readiness path, dry-run broadcast schedule intent path, broadcast preview safety path, queue readiness path, delivery-batch dry-run path, queue-message dry-run path, dispatch-preflight dry-run path, dispatch-attempt receipt path, sender-domain readiness path, provider-event readiness path, provider rate-limit readiness path, provider response readiness path, send-payload readiness path, Queue producer readiness path, Queue consumer readiness path, owner import-intent path, owner import-preflight path, and aggregate export-readiness path.",
    ],
    sortOrder: 50,
    updatedAt: null,
  },
