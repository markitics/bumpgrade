...
export type MobileDirectorReviewApi = {
  id: string;
  issue: number;
  status: "owner-mobile-director-review-ready";
  route: string;
  authBoundary: "owner-session";
  purpose: string;
...
export type MobileCommerceReviewApi = {
  id: string;
  issue: number;
  status: "owner-mobile-commerce-review-ready";
  route: string;
  authBoundary: "owner-session";
  purpose: string;
  reviewBoundary: string;
...
export type MobileConfirmedAction = {
  ...
  title: string;
  status:
    | "mobile-ui-contract-ready"
    | "owner-intent-api-ready"
    | "owner-director-review-api-ready"
    | "owner-commerce-review-api-ready"
    | "future-api-required";
  surface: string;
...
export type MobileAdminContract = {
  ...
  directorReviewApi: MobileDirectorReviewApi;
  commerceReviewApi: MobileCommerceReviewApi;
...
export const mobileAdminUpdatedAt = "2026-05-25";
...
export const mobileAdminContract: MobileAdminContract = {
  ...
  directorReviewApi: {
    id: "mobile-director-review-api",
    issue: 414,
    status: "owner-mobile-director-review-ready",
    route: "/api/mobile-admin/director-reviews",
    authBoundary: "owner-session",
    purpose:
      "Let a verified owner acknowledge a Director workstream from Mobile Admin after reviewing the current CEO-style status brief.",
    reviewBoundary:
    ...
    publicSourceDataSummary:
      "/mobile-admin/source-data and /mobile-admin/dashboard/source-data may expose the Director review API route, status, counts, latest reviewed workstream labels, and redaction flags; owner-only GET /api/mobile-admin/director-reviews is required before stale-state tokens are returned.",
    requiredInputs: [
      "workstreamId",
  ...
  commerceReviewApi: {
    ...
    authBoundary: "owner-session",
    purpose:
      "Let a verified owner acknowledge current commerce health from Mobile Admin after reviewing the public-safe commerce source-data contract.",
    reviewBoundary:
    ...
    publicSourceDataSummary:
      "/mobile-admin/source-data and /mobile-admin/dashboard/source-data may expose the commerce review API route, status, counts, latest reviewed commerce target labels, and redaction flags; owner-only GET /api/mobile-admin/commerce-reviews is required before stale-state tokens are returned.",
    requiredInputs: [
      "reviewTargetId",
  ...
  confirmedActions: [
    ...
    },
    {
      ...
      issue: 414,
      title: "Review commerce health",
      status: "owner-commerce-review-api-ready",
      surface: "Commerce health summary",
      confirmationText: "ACKNOWLEDGE MOBILE COMMERCE HEALTH",
      requiredInputs: [
        "actorUserId",
        "role",
        "reviewTargetId",
      ...
      safetyRules: [
        "Require exact confirmation before recording owner commerce-health review state.",
        "Check the current /commerce/source-data revision before accepting stale mobile acknowledgements.",
        "Return redacted result metadata only; never return buyer identity, Stripe identifiers, entitlement rows, signed URLs, or R2 object keys to public source-data.",
      ],
      mutationBoundary:
  ...
  jobs: [
    ...
    },
    {
      ...
      sourceRoutes: [
        "/mobile-admin/dashboard/source-data",
        "/api/mobile-admin/commerce-reviews",
        "/commerce/source-data",
        "/api/commerce/checkout",
      ],
      writeBoundary:
        "Mobile can record owner-confirmed commerce-health review state through /api/mobile-admin/commerce-reviews, but no live checkout, refund, subscription, price, fulfillment, or entitlement mutation from mobile is live until higher-risk confirmed-write APIs ship.",
    },
  ],
  apiDependencies: [
    {
    ...
    },
    {
      ...
      authBoundary: "owner-session",
      stableIds: ["directorWorkstreamId", "mobileDirectorReviewId", "idempotencyKey", "auditCorrelationId", "staleStateToken"],
    },
    {
      ...
      authBoundary: "owner-session",
      stableIds: ["commerceReviewTargetId", "mobileCommerceReviewId", "idempotencyKey", "auditCorrelationId", "staleStateToken"],
    },
    {
  ...
  confirmedWriteRules: [
    ...
    "The first domain-specific Mobile Admin confirmed-write slice can acknowledge Director workstreams through /api/mobile-admin/director-reviews, but it mutates only low-risk owner review state.",
    "The first Mobile Admin commerce review slice can acknowledge commerce source-data health through /api/mobile-admin/commerce-reviews, but it cannot create checkouts, refunds, subscription changes, price changes, fulfillment changes, entitlement changes, push notifications, distribution state, private-row changes, or public agent writes.",
    "The first mobile app slices can record audit-only action intents through /api/mobile-admin/actions, but higher-risk production mutations remain disabled until additional domain-specific confirmed-write APIs exist.",
