...
function emptySummary(source: AudienceInspectionSummary["source"], loadError: string | null): AudienceInspectionSummary {
  return {
    ...
    writeBoundary:
      "Issue #137 exposes owner-gated subscriber inspection and public aggregate source-data. Issue #167 records unsubscribe/suppression evidence and marks known subscribers unsubscribed without public list-membership leakage. Issue #169 lets owners create private CRM timeline notes while public source-data exposes aggregate counts only. Issue #171 calculates suppression-aware broadcast readiness without creating send queues. Issue #173 records owner-confirmed dry-run schedule intents without recipient payloads or provider message IDs. Issue #175 exposes preview and unsubscribe-footer safety without personalized bodies. Imports, real sends, private subscriber exports, and direct agent writes still require future confirmed-write APIs with consent and suppression checks.",
  };
}
