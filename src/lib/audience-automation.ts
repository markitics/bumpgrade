import {
  ...
  audienceBroadcastProviderResponseReadinessUpdatedAt,
...
export const audienceAutomationUpdatedAt = audienceBroadcastProviderResponseReadinessUpdatedAt;
...
export const audienceAutomationWorkspace: AudienceAutomationWorkspace = {
  ...
  summary:
    "An audience and automation scaffold with live consent-backed opt-in capture, public-safe unsubscribe/suppression evidence, owner-only CRM timeline notes, suppression-aware broadcast draft readiness, preview/footer safety, queue readiness, delivery-batch dry runs, dry-run queue-message evidence, dispatch preflight evidence, dispatch attempt receipts, sender-domain readiness gates, provider-event readiness gates, provider rate-limit readiness gates, and provider response readiness gates for the seeded waitlist, plus draft lead magnets, tags, sequences, broadcasts, and automation rules before email sends exist.",
  segments: [
    {
  ...
  broadcastDrafts: [
    {
      ...
      approvalBoundary:
        "Issue #205 can inspect provider response readiness for this draft, but real sending still requires verified sender alignment, provider event handling, provider rate-limit handling, queue producers/consumers, and audit correlation.",
    },
  ],
...
export const audienceAutomationSourceData = {
  ...
  caveat:
    "This contract proves audience, opt-in, email sequence, automation read/preview semantics, consent-backed subscriber capture, public-safe unsubscribe/suppression evidence, owner-only CRM timeline note evidence, suppression-aware broadcast readiness, owner-confirmed dry-run schedule intent evidence, broadcast preview/footer safety evidence, delivery queue readiness evidence, delivery-batch dry-run evidence, dry-run queue-message evidence, dispatch preflight evidence, dispatch attempt receipts, sender-domain readiness gates, provider-event readiness gates, provider rate-limit readiness gates, provider response readiness gates, and aggregate owner-inspection evidence. It does not import contacts, send email, dispatch Cloudflare Queue messages, create queue payload bodies, create recipient payloads, expose private DNS credentials, expose raw DNS records, expose provider secrets, expose provider limit secrets, expose raw provider payloads, expose raw provider response bodies, publicly expose private contact data, automate CRM actions, or provide direct confirmed-write public agent APIs.",
};
