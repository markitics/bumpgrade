...
export const featureCatalog: FeatureRecord[] = [
  ...
  },
  {
    ...
    expectedCapabilities: [
      ...
      "Customer-safe entitlement lookup by checkout intent evidence from issue #141.",
    ...
    evidence: [
      ...
      "Issue #141 adds `/api/products/entitlements` and `/products/entitlements/{checkoutIntentId}` as customer-safe entitlement lookup surfaces.",
    ],
    agentContract:
      "Agents may read public product metadata, aggregate entitlement counts, customer-safe entitlement lookup boundaries, and entitlement grant boundaries; private customer access, signed delivery, revocation, and billing data must stay behind authenticated contracts.",
  },
  {
