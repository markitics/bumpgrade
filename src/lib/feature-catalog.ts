...
export const featureCatalog: FeatureRecord[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      "Issue #417 adds owner-confirmed checkout unlinking and owner-confirmed resource delivery links to product/access catalog assets while tracking advanced drag-and-drop editing, arbitrary private R2 delivery, live fulfillment automation, physical deletion policy, webinar integrations, and direct agent-safe write tools after the MVP closeout.",
    ],
    agentContract:
      "Agents may read reusable template, block-template, webinar/resource template, draft duplication, granular block-edit, block add/remove, archive/unpublish lifecycle, checkout-link, checkout-unlink, resource-delivery-link, product delivery-gate, and public funnel checkout-start capability records from `/funnels/source-data`; owner sessions can seed, create from templates, duplicate, update, reorder, edit existing block title/body copy, add reusable blocks, remove safe unlinked blocks, link checkout offers, unlink checkout metadata, link resource/delivery blocks to product access assets, preview, publish, archive, and unpublish private draft funnel steps with confirmation where required and stale-state checks; duplicated drafts stay private and strip checkout-link/resource-link metadata, block edits preserve existing block and checkout-link/resource-link metadata, checkout unlinking preserves the block while clearing metadata, block removal refuses checkout-linked blocks until they are unlinked, resource delivery links expose entitlement-safe product/asset metadata without private files or signed URLs, archived drafts preserve evidence and leave public source-data when unpublished, and published linked checkout blocks can render the sandbox checkout start surface. Issue #417 keeps direct agent writes, drag-and-drop editing, physical deletion, live webinar integrations, arbitrary private R2 delivery, signed URLs, and live fulfillment automation pending until actor identity, confirmation, idempotency, stale-state checks, audit correlation, redaction, and rollback notes are designed together.",
  },
  {
