...
export const featureCatalog: FeatureRecord[] = [
  ...
  },
  {
    ...
    issue: 467,
    summary:
      "Public import center, dedicated platform import paths, platform-specific source guides, redacted preflight review maps with export-file structure parsing and platform export matching, `/imports/source-data`, verified-publisher private draft creation, owner-only subscriber review, and checkout rebuild readiness for moving competitor context into Bumpgrade launch workspaces.",
    ...
    expectedCapabilities: [
      ...
      "Private import records include safe extracted field plans for owner review before any importer cleanup or editing step.",
      ...
      "Verified publishers can create private importer subscriber records from a confirmed fresh upload or paste after preflight, without sequence enrollments, sends, private exports, or go-live effects.",
      ...
      "Verified publishers can mark imported checkout-offer and product-catalog records ready for Bumpgrade checkout rebuild, needing payment-copy cleanup, or parked for later without creating checkout intents, Stripe sessions, credentials, public checkout routes, account transfer, domains, fulfillment, sends, or go-live effects.",
      ...
      "Verified publishers can edit safe extracted field labels, review status, and prompts without storing raw extracted values or enabling buyer-facing actions.",
      ...
      "Input kinds, platform-specific source checklists, export-match templates, export-file analysis fields, saved private plan parts, safety gates, unsupported fields, and source evidence IDs.",
  ...
  },
  {
    ...
    expectedCapabilities: [
      ...
      "Owner-gated D1 draft funnel seed/create foundation from issue #91.",
      ...
      "Owner-session visual style controls and bounded canvas layout controls for existing funnel blocks from issue #417.",
      "Owner-session agent-created resource delivery tokens for published linked resource blocks from issue #417.",
      "Redacted resource-delivery receipt evidence after successful private download redemption from issue #417.",
    ...
    evidence: [
      "Tracked by issue #14.",
      "Issue #79 adds `/funnels/source-data` and `/funnels/indie-launch-sandbox` as the first read-only contract and preview foundation.",
      ...
      "Issue #417 adds owner-confirmed checkout unlinking, owner-confirmed resource delivery links to product/access catalog assets, funnel-scoped private download-token delivery from published linked resource blocks, owner-session agent-created resource delivery tokens for published linked resource blocks, redacted resource-delivery receipt evidence after successful private download redemption, owner-confirmed webinar event/replay links to public-safe external URLs, owner-confirmed archived-draft purge with tombstone evidence, owner-confirmed bulk archived-draft purge with one tombstone per draft, owner-session within-step block reordering, owner-session drag/drop block placement through existing move endpoints, owner-session cross-step block moves, owner-session visual style controls and bounded canvas layout controls for existing blocks, and owner-session direct agent-safe draft writes including visual style presets, bounded canvas layouts, reusable block add/remove, checkout linking/unlinking, resource-delivery and webinar-event linking, block movement, public publishing, duplication, archive/unpublish, archived-draft purge, and bulk archived-draft purge while tracking arbitrary uploaded private asset delivery, live fulfillment automation, full webinar integrations, unauthenticated public agent-created delivery tokens, and unauthenticated public agent publishing after the MVP closeout.",
    ],
    agentContract:
      "Agents may read reusable template, block-template, webinar/resource template, draft duplication, granular block-edit, block add/remove, block visual-style, block canvas-layout, block-reorder, cross-step block-move, archive/unpublish lifecycle, archived-draft purge, bulk archived-draft purge, checkout-link, checkout-unlink, resource-delivery-link, public funnel resource-delivery-token, owner-session agent resource-delivery-token, resource-delivery receipt, webinar-event-link, product delivery-gate, public funnel checkout-start, and direct agent-safe draft-write capability records from `/funnels/source-data`; owner sessions can seed, create from templates, duplicate, update, reorder steps, edit existing block title/body copy, apply curated visual style presets to existing blocks, set bounded canvas layout for existing blocks, add reusable blocks, remove safe unlinked blocks, reorder existing blocks within the same step, drag/drop existing blocks by replaying the same move endpoints with fresh revisions, move existing blocks across steps, link checkout offers, unlink checkout metadata, link resource/delivery blocks to product access assets, link webinar blocks to external registration/replay URLs, preview, publish, archive, unpublish, purge already archived private draft funnel steps, and bulk purge selected archived private drafts with confirmation where required and stale-state checks; owner-session agents can call `/api/agent/funnels/draft-writes` to update private draft block copy, apply curated visual style presets, set bounded canvas layout, add reusable blocks, remove eligible unlinked blocks, link checkout metadata, unlink checkout metadata, link resource-delivery metadata, link webinar-event metadata, reorder blocks, move blocks across steps, duplicate a private draft, publish a draft, archive/unpublish a draft, purge an already archived draft, or bulk purge selected archived drafts after exact confirmation, idempotency, current draft revision, and audit correlation checks; owner-session agents can call `/api/agent/funnels/resource-delivery-tokens` to create a short-lived funnel-scoped private download token for a published linked resource block after exact confirmation, idempotency, current published revision, checkout intent, entitlement, and audit correlation checks, with idempotent replays returning redacted audit metadata instead of raw tokens; successful redemption through the existing product download route records redacted resource-delivery receipt evidence with safe funnel, block, product, asset, source, and hashed checkout/entitlement references only; duplicated drafts stay private and strip checkout-link/resource-link/webinar-link metadata, block edits preserve existing block and checkout-link/resource-link/webinar-link metadata, block visual styles store only a curated style ID, block canvas layouts store only bounded x, y, width, height, and zIndex numbers, both preserve block IDs, kinds, copy, checkout-link/resource-link/webinar-link metadata, direct agent checkout/resource-delivery/webinar-event/block-placement writes preserve existing block identity, direct agent style/layout writes preserve linked metadata and never accept arbitrary CSS or scripts, direct agent block add/remove uses the reusable block library, refuses checkout-linked removals, and keeps at least one block per step, owner-confirmed direct agent publishing creates a public route mutation without billing and can be rolled back with archive-draft, direct agent archived-draft purge and bulk archived-draft purge record tombstones before deleting only archived draft/step rows, within-step block reordering preserves block IDs, kinds, copy, checkout-link/resource-link/webinar-link metadata, and step membership, cross-step block moves preserve block IDs, kinds, copy, checkout-link/resource-link/webinar-link metadata, and audit evidence while changing step membership, checkout unlinking preserves the block while clearing metadata, block removal refuses checkout-linked blocks until they are unlinked, resource delivery links expose entitlement-safe product/asset metadata without private files or signed URLs, webinar event/replay links expose public-safe external references without provider secrets or attendee records, archived drafts preserve evidence and leave public source-data when unpublished, archived-draft purge records a tombstone before deleting draft and step rows while preserving prior audit rows and excluding product assets, R2 objects, buyer records, and billing state, published linked checkout blocks can render the sandbox checkout start surface, published resource-linked blocks can render entitlement-safe product access references and mint short-lived Bumpgrade download tokens only after checkout intent and entitlement match the linked product and file asset, and published webinar-linked blocks can render external registration/replay references. Issue #417 keeps unauthenticated public agent publishing, non-archived purge, full webinar integrations, arbitrary uploaded private asset delivery, signed URLs, unauthenticated public agent-created delivery tokens, and live fulfillment automation pending until stronger rollback and provider safety notes are designed together.",
  },
  {
    ...
    evidence: [
      ...
      "Issue #81 adds `/offers/source-data` and `/offers/indie-launch-stack` as the first read-only checkout-offer contract and preview foundation.",
  ...
  },
  {
    ...
    evidence: [
      "Tracked by issue #16.",
      "Issue #83 adds `/products/source-data` and `/products/indie-launch-library` as the first read-only product/access contract and preview foundation.",
  ...
  },
  {
    ...
    evidence: [
      "Tracked by issue #17.",
      "Issue #85 adds `/audience/source-data` and `/audience/indie-launch-waitlist` as the first read-only audience automation contract and preview foundation.",
  ...
  },
  {
    ...
    issue: 18,
    summary:
      "Privacy-safe analytics events, deterministic experiment assignments, funnel conversion reporting, fixed-window attribution, owner decision and winner rollout evidence, aggregate exports, and notification readiness proof.",
    ...
    expectedCapabilities: [
      ...
      "Public-safe source/campaign routing rules plus seeded sandbox funnel copy routing with a baseline holdout from issue #422.",
      "Owner-confirmed winner rollout and rollback routing for unmatched experiment traffic from issue #422.",
      "Issue #422 groups the remaining custom analytics schemas beyond the current seeded boundary, notification execution, and direct agent-safe write parity as one post-MVP execution bucket.",
    ],
    evidence: [
      "Tracked by issue #18.",
  ...
  },
  {
    ...
    evidence: [
      ...
      "Issue #424 tracks live payout execution, partner notification sends, private payout/tax data, authenticated private partner portals, payable statement creation, and direct agent-safe affiliate/referral writes as one pending post-MVP execution bucket.",
    ],
    agentContract:
      "Agents may read aggregate referral click counts, checkout attribution evidence, review-only commission ledger evidence, owner review action counts, public-safe partner reports, public-safe partner portal status pages, public-safe partner statement snapshots, read-only payout preparation checklists, owner-confirmed payout preparation records, owner-reviewed fraud review records, owner-confirmed fraud enforcement records, owner-reviewed partner notification readiness records, owner-reviewed partner notification send preflight records, owner-reviewed notification provider readiness records, and write boundaries; issue #424 owns buyer attribution finalization, payable statements, payable commissions, payout execution, payout reversals, private payout/tax data, authenticated private partner portals, partner sends, provider-send enablement, provider configuration, provider secret storage, provider calls, send payload creation, queue dispatch, and direct agent-safe affiliate/referral writes behind future confirmation, idempotency, stale-state, audit, redaction, refund-window, provider-safety, and rollback/dispute checks.",
  },
  {
  ...
  },
  {
    ...
    expectedCapabilities: [
      ...
      "Cloudflare Cron cleanup for expired anonymous recovery using the same preserve-claimed-records boundary.",
    ...
    evidence: [
      ...
      "Issue #466 adds logged-out structured playground save, browser-recovery save limits, owner-gated and scheduled cleanup, verified account claim, private launch draft creation, and private claim records while keeping go-live actions paid-gated.",
  ...
  },
  {
    ...
    issue: 414,
    summary:
      "Read-only live mobile dashboard hydration, redacted Director workstream digesting, iOS/Android foundations, owner-session private-row inspection, low-risk private-row actions, owner-confirmed Director workstream review actions, owner-confirmed commerce-health review actions, push/distribution readiness boundaries, and the shared confirmed-action contract are proven; installable private mobile admin parity is still tracked as the current follow-up.",
    ...
    expectedCapabilities: [
      ...
      "iOS, Android, and Expo foundations render the live dashboard panel from issue #155.",
      "iOS, Android, and Expo foundations live-read the dashboard with fixture fallback from issue #157.",
      "iOS, Android, and Expo foundations render the redacted Director workstream brief from the live dashboard payload.",
      "iOS, Android, and Expo foundations render the issue #414 owner-session and confirmed-action contract without enabling mobile mutations.",
    ...
    evidence: [
      ...
      "`/mobile-admin/ios/source-data` exposes the first iOS foundation, fixture, simulator target, smoke command, and screenshot path.",
      "`/mobile-admin/android/source-data` exposes the first Android foundation, fixture asset, emulator smoke command, and screenshot path.",
    ],
    agentContract:
  ...
  },
  {
    ...
    expectedCapabilities: [
      ...
      "Canonical self-serve pricing page with Experiment, Grow, Enterprise, White glove setup, logged-out playground, signed-in Free Build, and a non-indexed alternate usage-based pricing draft.",
      ...
      "Logged-out `/playground` route with browser-scoped structured builder recovery, save limits, owner-gated and scheduled cleanup, account claim path, private launch draft creation, and private claim-record creation after signup.",
    ...
    evidence: [
      ...
      "Issue #551 keeps `/pricing` canonical for buyers and keeps `/pricing-v2` as a non-indexed usage-pricing draft.",
      "`/users`, `/developers-and-agents`, `/resources`, `/pricing`, `/playground`, and non-indexed `/pricing-v2` are live destinations.",
      "`/playground/source-data` exposes logged-out structured builder recovery, save limits, account claim, owner-gated and scheduled cleanup, private draft creation, private claim-record creation, redaction, retention, and paid go-live gates.",
