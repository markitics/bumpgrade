...
export const featureCatalog: FeatureRecord[] = [
  ...
  },
  {
    ...
    evidence: [
      ...
      "Issue #430 lets verified owners edit existing draft block title/body copy after idempotency and a fresh revision check while preserving block IDs, kinds, ordered step structure, and checkout-link metadata.",
      ...
      "Issue #417 adds owner-confirmed checkout unlinking, owner-confirmed resource delivery links to product/access catalog assets, owner-session within-step block reordering, and owner-session cross-step block moves while tracking freeform drag-and-drop editing, arbitrary private R2 delivery, live fulfillment automation, physical deletion policy, webinar integrations, and direct agent-safe write tools after the MVP closeout.",
    ],
    agentContract:
      "Agents may read reusable template, block-template, webinar/resource template, draft duplication, granular block-edit, block add/remove, block-reorder, cross-step block-move, archive/unpublish lifecycle, checkout-link, checkout-unlink, resource-delivery-link, product delivery-gate, and public funnel checkout-start capability records from `/funnels/source-data`; owner sessions can seed, create from templates, duplicate, update, reorder steps, edit existing block title/body copy, add reusable blocks, remove safe unlinked blocks, reorder existing blocks within the same step, move existing blocks across steps, link checkout offers, unlink checkout metadata, link resource/delivery blocks to product access assets, preview, publish, archive, and unpublish private draft funnel steps with confirmation where required and stale-state checks; duplicated drafts stay private and strip checkout-link/resource-link metadata, block edits preserve existing block and checkout-link/resource-link metadata, within-step block reordering preserves block IDs, kinds, copy, checkout-link/resource-link metadata, and step membership, cross-step block moves preserve block IDs, kinds, copy, checkout-link/resource-link metadata, and audit evidence while changing step membership, checkout unlinking preserves the block while clearing metadata, block removal refuses checkout-linked blocks until they are unlinked, resource delivery links expose entitlement-safe product/asset metadata without private files or signed URLs, archived drafts preserve evidence and leave public source-data when unpublished, and published linked checkout blocks can render the sandbox checkout start surface. Issue #417 keeps direct agent writes, freeform drag-and-drop editing, physical deletion, live webinar integrations, arbitrary private R2 delivery, signed URLs, and live fulfillment automation pending until actor identity, confirmation, idempotency, stale-state checks, audit correlation, redaction, and rollback notes are designed together.",
  },
  {
