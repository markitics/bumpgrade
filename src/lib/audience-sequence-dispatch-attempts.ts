...
export const audienceSequenceDispatchAttemptIssue = 364;
export const audienceSequenceDispatchAttemptStatus = "sequence-dispatch-attempts-dry-run-ready";
export const audienceSequenceDispatchAttemptApiRoute =
  "/api/admin/audience/sequences/dispatch-attempts";
export const audienceSequenceDispatchAttemptConfirmationText =
  "Create dry-run Bumpgrade sequence dispatch attempt receipt";
...
export type AudienceSequenceDispatchAttemptPublic = {
  id: string;
  sequenceId: string;
...
export type AudienceSequenceDispatchAttemptSummary = {
  id: "audience-sequence-dispatch-attempt-contract";
  status: typeof audienceSequenceDispatchAttemptStatus;
  issue: typeof audienceSequenceDispatchAttemptIssue;
  parentIssue: 17;
  apiRoute: typeof audienceSequenceDispatchAttemptApiRoute;
  ...
  confirmation: {
    required: true;
    text: typeof audienceSequenceDispatchAttemptConfirmationText;
  };
  counts: {
  ...
  latestAttempts: AudienceSequenceDispatchAttemptPublic[];
  redaction: {
...
type CreateSequenceDispatchAttemptInput = {
...
type CreateSequenceDispatchAttemptResult =
  | {
      ok: true;
      status: "sequence_dispatch_attempt_recorded" | "sequence_dispatch_attempt_replayed";
      duplicate: boolean;
      attempt: AudienceSequenceDispatchAttemptPublic;
      redaction: AudienceSequenceDispatchAttemptSummary["redaction"];
    }
  | {
      ok: false;
      status:
...
function publicSequenceDispatchAttempt(
  row: SequenceDispatchAttemptRow,
  duplicate: boolean,
): AudienceSequenceDispatchAttemptPublic {
  return {
    id: row.id,
...
function emptySummary(
  source: AudienceSequenceDispatchAttemptSummary["source"],
  loadError: string | null,
): AudienceSequenceDispatchAttemptSummary {
  return {
    id: "audience-sequence-dispatch-attempt-contract",
...
export async function getAudienceSequenceDispatchAttemptSummary(): Promise<AudienceSequenceDispatchAttemptSummary> {
  try {
    ...
    const latest = await db
      ...
      .all<SequenceDispatchAttemptRow>();
    ...
    return {
      ...
      latestAttempts: (latest.results ?? []).map((row) => publicSequenceDispatchAttempt(row, false)),
    };
  } catch (error) {
    return emptySummary(
      "unavailable",
      error instanceof Error ? error.message : "Unable to load audience sequence dispatch attempts.",
    );
  }
}

export async function createAudienceSequenceDispatchAttempt(
  input: CreateSequenceDispatchAttemptInput,
): Promise<CreateSequenceDispatchAttemptResult> {
  ...
  if (
    ...
    return {
      ...
      message:
        "A dispatch preflight ID, sequence ID, expected workspace revision, expected sequence status, expected readiness count, and idempotency key are required.",
      redaction,
    };
  }

  if (input.confirmationText !== audienceSequenceDispatchAttemptConfirmationText) {
    return {
      ok: false,
      status: "confirmation_required",
      message: "Exact confirmation text is required before recording sequence dispatch attempt receipts.",
      redaction,
    };
  }
  ...
  if (existing) {
    return {
      ok: true,
      status: "sequence_dispatch_attempt_replayed",
      duplicate: true,
      attempt: publicSequenceDispatchAttempt(existing, true),
      redaction,
    };
  }
  ...
  if (
    !preflight ||
    preflight.status !== "dispatch_preflight_dry_run_recorded" ||
    preflight.sequence_id !== sequenceId
  ) {
    return {
      ok: false,
      status: "dispatch_preflight_not_found",
      message:
        "A current dry-run sequence dispatch preflight is required before a dispatch attempt receipt can be recorded.",
      redaction,
    };
  }
  ...
  if (
    ...
    numberValue(preflight.delivery_queue_rows_created) > 0 ||
    numberValue(preflight.cloudflare_queue_messages_created) > 0 ||
    numberValue(preflight.queue_payload_bodies_created) > 0 ||
    numberValue(preflight.recipient_payloads_created) > 0 ||
    numberValue(preflight.personalized_bodies_created) > 0 ||
    numberValue(preflight.unsubscribe_urls_created) > 0 ||
    numberValue(preflight.provider_responses_created) > 0 ||
    ...
    return {
      ...
      message:
        "Sequence dispatch preflight gates must stay dry-run without Queue dispatch, queue payload bodies, recipient payloads, unsubscribe URLs, provider sends, provider responses, or provider message IDs.",
      redaction,
    };
  }
  ...
  if (
    ...
    preflight.expected_workspace_revision_id !== expectedWorkspaceRevisionId
  ) {
    return {
      ok: false,
      status: "stale_workspace_revision",
      message: "The audience workspace changed before the sequence dispatch attempt receipt was recorded.",
      redaction,
      currentWorkspaceRevisionId: readiness.workspace.revisionId,
    };
  }

  if (sequence.status !== expectedSequenceStatus || preflight.expected_sequence_status !== expectedSequenceStatus) {
    return {
      ok: false,
      status: "stale_sequence_status",
      message: "The sequence status changed before the sequence dispatch attempt receipt was recorded.",
      redaction,
  ...
  if (
    ...
    numberValue(preflight.expected_ready_enrollment_count) !== expectedReadyEnrollmentCount
  ) {
    return {
      ok: false,
      status: "stale_readiness_count",
      message: "Sequence readiness changed before the sequence dispatch attempt receipt was recorded.",
      redaction,
      currentReadyEnrollmentCount: readiness.counts.readyEnrollments,
    };
  }

  const attemptId = `sequence-dispatch-attempt-${crypto.randomUUID()}`;
  await db
    .prepare(
      `INSERT INTO audience_sequence_dispatch_attempts (
        id, sequence_id, dispatch_preflight_id, delivery_queue_message_id, delivery_batch_id,
        schedule_intent_id, status, queue_name, queue_mode, queue_producer_mode,
        expected_workspace_revision_id, expected_sequence_status, expected_ready_enrollment_count,
        dry_run_message_count, held_enrollment_count, active_suppression_count,
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, 'dispatch_attempt_dry_run_recorded', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,
    ...
    .bind(
      ...
      numberValue(preflight.held_enrollment_count),
  ...
  if (!attempt) {
    return {
      ...
      message: "The sequence dispatch attempt receipt could not be saved.",
      redaction,
    };
  }
  ...
  return {
    ...
    duplicate: false,
    attempt: publicSequenceDispatchAttempt(attempt, false),
    redaction,
  };
}
