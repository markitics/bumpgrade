...
export const productTestCheckoutLinks = sqliteTable(
  "product_test_checkout_links",
  {
    id: text("id").primaryKey(),
    idempotencyKey: text("idempotency_key").notNull(),
    productId: text("product_id")
    ...
    amountCents: integer("amount_cents").notNull(),
    currency: text("currency").notNull().default("usd"),
    status: text("status").notNull().default("test_checkout_link_active"),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("product_test_checkout_links_idempotency_unique").on(table.idempotencyKey),
    productStatusIdx: index("product_test_checkout_links_product_status_idx").on(table.productId, table.status),
    publicPathUnique: uniqueIndex("product_test_checkout_links_public_path_unique").on(table.publicPath),
    createdIdx: index("product_test_checkout_links_created_idx").on(table.createdAt),
  }),
);

export const productTestCheckoutPurchases = sqliteTable(
  "product_test_checkout_purchases",
  {
    id: text("id").primaryKey(),
    idempotencyKey: text("idempotency_key").notNull(),
    ...
    status: text("status").notNull().default("test_checkout_paid"),
    confirmationTextSha256: text("confirmation_text_sha256").notNull(),
    billingMutationEnabled: integer("billing_mutation_enabled", { mode: "boolean" }).notNull().default(false),
    stripeCheckoutSessionCreated: integer("stripe_checkout_session_created", { mode: "boolean" }).notNull().default(false),
    liveChargeCreated: integer("live_charge_created", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("product_test_checkout_purchases_idempotency_unique").on(
      table.idempotencyKey,
    ),
    linkStatusIdx: index("product_test_checkout_purchases_link_status_idx").on(table.checkoutLinkId, table.status),
    productStatusIdx: index("product_test_checkout_purchases_product_status_idx").on(table.productId, table.status),
    checkoutIdx: index("product_test_checkout_purchases_checkout_idx").on(table.checkoutIntentId),
    createdIdx: index("product_test_checkout_purchases_created_idx").on(table.createdAt),
  }),
);
...
export const audienceSequenceDeliveryBatches = sqliteTable(
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("audience_sequence_delivery_batches_idempotency_unique").on(
      table.idempotencyKey,
    ),
    scheduleIntentIdx: index("audience_sequence_delivery_batches_schedule_intent_idx").on(
      table.scheduleIntentId,
    ),
    sequenceStatusIdx: index("audience_sequence_delivery_batches_sequence_status_idx").on(
      table.sequenceId,
      table.status,
    ),
  }),
...
export const audienceSequenceDeliveryQueueMessages = sqliteTable(
  "audience_sequence_delivery_queue_messages",
  {
    id: text("id").primaryKey(),
    sequenceId: text("sequence_id").notNull(),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("audience_sequence_delivery_queue_messages_idempotency_unique").on(
      table.idempotencyKey,
    ),
    deliveryBatchIdx: index("audience_sequence_delivery_queue_messages_batch_idx").on(
      table.deliveryBatchId,
    ),
    sequenceStatusIdx: index("audience_sequence_delivery_queue_messages_sequence_status_idx").on(
      table.sequenceId,
      table.status,
    ),
  }),
...
export const audienceSequenceDispatchPreflights = sqliteTable(
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("audience_sequence_dispatch_preflights_idempotency_unique").on(
      table.idempotencyKey,
    ),
    queueMessageIdx: index("audience_sequence_dispatch_preflights_queue_message_idx").on(
      table.deliveryQueueMessageId,
    ),
    sequenceStatusIdx: index("audience_sequence_dispatch_preflights_sequence_status_idx").on(
      table.sequenceId,
      table.status,
    ),
  }),
...
export const audienceSequenceQueueConsumerReadiness = sqliteTable(
  "audience_sequence_queue_consumer_readiness",
  {
    id: text("id").primaryKey(),
    sequenceId: text("sequence_id").notNull(),
    ...
    deliveryBatchId: text("delivery_batch_id").notNull(),
    scheduleIntentId: text("schedule_intent_id").notNull(),
    status: text("status").notNull().default("sequence_queue_consumer_readiness_recorded"),
    queueName: text("queue_name").notNull(),
    consumerBinding: text("consumer_binding").notNull(),
    consumerMode: text("consumer_mode").notNull(),
    consumerGateStatus: text("consumer_gate_status").notNull(),
    producerDependencyStatus: text("producer_dependency_status").notNull(),
    payloadReadPolicy: text("payload_read_policy").notNull(),
    ackPolicy: text("ack_policy").notNull(),
    retryPolicy: text("retry_policy").notNull(),
    deadLetterPolicy: text("dead_letter_policy").notNull(),
    idempotencyPolicy: text("idempotency_policy").notNull(),
    auditCorrelationPolicy: text("audit_correlation_policy").notNull(),
    providerHandoffPolicy: text("provider_handoff_policy").notNull(),
    backpressurePolicy: text("backpressure_policy").notNull(),
    expectedWorkspaceRevisionId: text("expected_workspace_revision_id").notNull(),
    expectedSequenceStatus: text("expected_sequence_status").notNull(),
    expectedReadyEnrollmentCount: integer("expected_ready_enrollment_count").notNull().default(0),
    expectedQueueProducerReadinessStatus: text("expected_queue_producer_readiness_status").notNull(),
    dryRunMessageCount: integer("dry_run_message_count").notNull().default(0),
    heldEnrollmentCount: integer("held_enrollment_count").notNull().default(0),
    activeSuppressionCount: integer("active_suppression_count").notNull().default(0),
    providerLimitPolicy: text("provider_limit_policy").notNull(),
    ...
    queueDeadLetterRecordsCreated: integer("queue_dead_letter_records_created", { mode: "boolean" })
      .notNull()
      .default(false),
    queuePayloadBodiesRead: integer("queue_payload_bodies_read", { mode: "boolean" })
      .notNull()
      .default(false),
    queuePayloadBodiesCreated: integer("queue_payload_bodies_created", { mode: "boolean" })
      .notNull()
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("audience_sequence_queue_consumer_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    queueProducerReadinessIdx: index("audience_sequence_queue_consumer_readiness_producer_idx").on(
      table.queueProducerReadinessId,
    ),
    dispatchAttemptIdx: index("audience_sequence_queue_consumer_readiness_attempt_idx").on(
      table.dispatchAttemptId,
    ),
    sequenceStatusIdx: index("audience_sequence_queue_consumer_readiness_sequence_status_idx").on(
      table.sequenceId,
      table.status,
    ),
  }),
...
export const audienceSequenceProviderCallReadiness = sqliteTable(
  "audience_sequence_provider_call_readiness",
  {
    id: text("id").primaryKey(),
    sequenceId: text("sequence_id").notNull(),
    ...
    deliveryBatchId: text("delivery_batch_id").notNull(),
    scheduleIntentId: text("schedule_intent_id").notNull(),
    status: text("status").notNull().default("sequence_provider_call_readiness_recorded"),
    queueName: text("queue_name").notNull(),
    providerName: text("provider_name").notNull(),
    providerMode: text("provider_mode").notNull(),
    providerCallGateStatus: text("provider_call_gate_status").notNull(),
    queueConsumerDependencyStatus: text("queue_consumer_dependency_status").notNull(),
    ...
    expectedSequenceStatus: text("expected_sequence_status").notNull(),
    expectedReadyEnrollmentCount: integer("expected_ready_enrollment_count").notNull().default(0),
    expectedQueueConsumerReadinessStatus: text("expected_queue_consumer_readiness_status").notNull(),
    dryRunMessageCount: integer("dry_run_message_count").notNull().default(0),
    heldEnrollmentCount: integer("held_enrollment_count").notNull().default(0),
    activeSuppressionCount: integer("active_suppression_count").notNull().default(0),
    providerLimitPolicy: text("provider_limit_policy").notNull(),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("audience_sequence_provider_call_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    queueConsumerReadinessIdx: index("audience_sequence_provider_call_readiness_consumer_idx").on(
      table.queueConsumerReadinessId,
    ),
    dispatchAttemptIdx: index("audience_sequence_provider_call_readiness_attempt_idx").on(
      table.dispatchAttemptId,
    ),
    sequenceStatusIdx: index("audience_sequence_provider_call_readiness_sequence_status_idx").on(
      table.sequenceId,
      table.status,
    ),
  }),
...
export const audienceSequenceDeliveryAttemptReadiness = sqliteTable(
  "audience_sequence_delivery_attempt_readiness",
  {
    ...
    deliveryBatchId: text("delivery_batch_id").notNull(),
    scheduleIntentId: text("schedule_intent_id").notNull(),
    status: text("status").notNull().default("sequence_delivery_attempt_readiness_recorded"),
    queueName: text("queue_name").notNull(),
    providerName: text("provider_name").notNull(),
    providerMode: text("provider_mode").notNull(),
    deliveryAttemptGateStatus: text("delivery_attempt_gate_status").notNull(),
    providerCallDependencyStatus: text("provider_call_dependency_status").notNull(),
    providerResponsePolicy: text("provider_response_policy").notNull(),
    providerMessageIdPolicy: text("provider_message_id_policy").notNull(),
    deliveryAttemptPolicy: text("delivery_attempt_policy").notNull(),
    idempotencyPolicy: text("idempotency_policy").notNull(),
    ...
    expectedSequenceStatus: text("expected_sequence_status").notNull(),
    expectedReadyEnrollmentCount: integer("expected_ready_enrollment_count").notNull().default(0),
    expectedProviderCallReadinessStatus: text("expected_provider_call_readiness_status").notNull(),
    dryRunMessageCount: integer("dry_run_message_count").notNull().default(0),
    heldEnrollmentCount: integer("held_enrollment_count").notNull().default(0),
    activeSuppressionCount: integer("active_suppression_count").notNull().default(0),
    providerLimitPolicy: text("provider_limit_policy").notNull(),
    ...
    unsubscribeFooterCheckStatus: text("unsubscribe_footer_check_status").notNull(),
    senderDomainGateStatus: text("sender_domain_gate_status").notNull(),
    deliveryAttemptEnabled: integer("delivery_attempt_enabled", { mode: "boolean" }).notNull().default(false),
    deliveryAttemptsCreated: integer("delivery_attempts_created", { mode: "boolean" })
      .notNull()
      .default(false),
    deliveryResultsCreated: integer("delivery_results_created", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("audience_sequence_delivery_attempt_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    providerCallReadinessIdx: index("audience_sequence_delivery_attempt_readiness_provider_call_idx").on(
      table.providerCallReadinessId,
    ),
    dispatchAttemptIdx: index("audience_sequence_delivery_attempt_readiness_attempt_idx").on(
      table.dispatchAttemptId,
    ),
    sequenceStatusIdx: index("audience_sequence_delivery_attempt_readiness_sequence_status_idx").on(
      table.sequenceId,
      table.status,
    ),
  }),
...
export const audienceSequenceDeliveryResultReadiness = sqliteTable(
  "audience_sequence_delivery_result_readiness",
  {
    id: text("id").primaryKey(),
    sequenceId: text("sequence_id").notNull(),
    ...
    deliveryBatchId: text("delivery_batch_id").notNull(),
    scheduleIntentId: text("schedule_intent_id").notNull(),
    status: text("status").notNull().default("sequence_delivery_result_readiness_recorded"),
    queueName: text("queue_name").notNull(),
    providerName: text("provider_name").notNull(),
    providerMode: text("provider_mode").notNull(),
    deliveryResultGateStatus: text("delivery_result_gate_status").notNull(),
    deliveryAttemptDependencyStatus: text("delivery_attempt_dependency_status").notNull(),
    providerResponsePolicy: text("provider_response_policy").notNull(),
    providerMessageIdPolicy: text("provider_message_id_policy").notNull(),
    deliveryAttemptPolicy: text("delivery_attempt_policy").notNull(),
    ...
    expectedSequenceStatus: text("expected_sequence_status").notNull(),
    expectedReadyEnrollmentCount: integer("expected_ready_enrollment_count").notNull().default(0),
    expectedDeliveryAttemptReadinessStatus: text("expected_delivery_attempt_readiness_status").notNull(),
    dryRunMessageCount: integer("dry_run_message_count").notNull().default(0),
    heldEnrollmentCount: integer("held_enrollment_count").notNull().default(0),
    activeSuppressionCount: integer("active_suppression_count").notNull().default(0),
    providerLimitPolicy: text("provider_limit_policy").notNull(),
    ...
    unsubscribeFooterCheckStatus: text("unsubscribe_footer_check_status").notNull(),
    senderDomainGateStatus: text("sender_domain_gate_status").notNull(),
    deliveryResultEnabled: integer("delivery_result_enabled", { mode: "boolean" }).notNull().default(false),
    deliveryAttemptsCreated: integer("delivery_attempts_created", { mode: "boolean" })
      .notNull()
      .default(false),
    deliveryResultsCreated: integer("delivery_results_created", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("audience_sequence_delivery_result_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    deliveryAttemptReadinessIdx: index("audience_sequence_delivery_result_readiness_attempt_readiness_idx").on(
      table.deliveryAttemptReadinessId,
    ),
    providerCallReadinessIdx: index("audience_sequence_delivery_result_readiness_provider_call_idx").on(
      table.providerCallReadinessId,
    ),
    dispatchAttemptIdx: index("audience_sequence_delivery_result_readiness_attempt_idx").on(table.dispatchAttemptId),
    sequenceStatusIdx: index("audience_sequence_delivery_result_readiness_sequence_status_idx").on(
      table.sequenceId,
      table.status,
    ),
  }),
...
export const audienceSequenceDeliveryStatusWebhookReadiness = sqliteTable(
  "audience_sequence_delivery_status_webhook_readiness",
  {
    id: text("id").primaryKey(),
    sequenceId: text("sequence_id").notNull(),
    ...
    deliveryBatchId: text("delivery_batch_id").notNull(),
    scheduleIntentId: text("schedule_intent_id").notNull(),
    status: text("status").notNull().default("sequence_delivery_status_webhook_readiness_recorded"),
    queueName: text("queue_name").notNull(),
    providerName: text("provider_name").notNull(),
    providerMode: text("provider_mode").notNull(),
    deliveryStatusWebhookGateStatus: text("delivery_status_webhook_gate_status").notNull(),
    deliveryResultDependencyStatus: text("delivery_result_dependency_status").notNull(),
    providerResponsePolicy: text("provider_response_policy").notNull(),
    providerMessageIdPolicy: text("provider_message_id_policy").notNull(),
    deliveryAttemptPolicy: text("delivery_attempt_policy").notNull(),
    deliveryResultPolicy: text("delivery_result_policy").notNull(),
    ...
    expectedSequenceStatus: text("expected_sequence_status").notNull(),
    expectedReadyEnrollmentCount: integer("expected_ready_enrollment_count").notNull().default(0),
    expectedDeliveryResultReadinessStatus: text("expected_delivery_result_readiness_status").notNull(),
    dryRunMessageCount: integer("dry_run_message_count").notNull().default(0),
    heldEnrollmentCount: integer("held_enrollment_count").notNull().default(0),
    activeSuppressionCount: integer("active_suppression_count").notNull().default(0),
    providerLimitPolicy: text("provider_limit_policy").notNull(),
    ...
    deliveryStatusWebhookPayloadsCreated: integer("delivery_status_webhook_payloads_created", { mode: "boolean" })
      .notNull()
      .default(false),
    providerPollingEnabled: integer("provider_polling_enabled", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("audience_sequence_delivery_status_webhook_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    deliveryResultReadinessIdx: index("audience_sequence_delivery_status_webhook_readiness_result_readiness_idx").on(
      table.deliveryResultReadinessId,
    ),
    deliveryAttemptReadinessIdx: index("audience_sequence_delivery_status_webhook_readiness_attempt_readiness_idx").on(
      table.deliveryAttemptReadinessId,
    ),
    providerCallReadinessIdx: index("audience_sequence_delivery_status_webhook_readiness_provider_call_idx").on(
      table.providerCallReadinessId,
    ),
    dispatchAttemptIdx: index("audience_sequence_delivery_status_webhook_readiness_attempt_idx").on(
      table.dispatchAttemptId,
    ),
    sequenceStatusIdx: index("audience_sequence_delivery_status_webhook_readiness_sequence_status_idx").on(
      table.sequenceId,
      table.status,
    ),
  }),
...
export const audienceSequenceProviderPollingReadiness = sqliteTable(
  "audience_sequence_provider_polling_readiness",
  {
    id: text("id").primaryKey(),
    sequenceId: text("sequence_id").notNull(),
    ...
    deliveryBatchId: text("delivery_batch_id").notNull(),
    scheduleIntentId: text("schedule_intent_id").notNull(),
    status: text("status").notNull().default("sequence_provider_polling_readiness_recorded"),
    queueName: text("queue_name").notNull(),
    providerName: text("provider_name").notNull(),
    providerMode: text("provider_mode").notNull(),
    providerPollingGateStatus: text("provider_polling_gate_status").notNull(),
    deliveryStatusWebhookDependencyStatus: text("delivery_status_webhook_dependency_status").notNull(),
    providerResponsePolicy: text("provider_response_policy").notNull(),
    providerMessageIdPolicy: text("provider_message_id_policy").notNull(),
    deliveryAttemptPolicy: text("delivery_attempt_policy").notNull(),
    deliveryResultPolicy: text("delivery_result_policy").notNull(),
    deliveryStatusWebhookPolicy: text("delivery_status_webhook_policy").notNull(),
    providerPollingPolicy: text("provider_polling_policy").notNull(),
    idempotencyPolicy: text("idempotency_policy").notNull(),
    auditCorrelationPolicy: text("audit_correlation_policy").notNull(),
    backpressurePolicy: text("backpressure_policy").notNull(),
    expectedWorkspaceRevisionId: text("expected_workspace_revision_id").notNull(),
    expectedSequenceStatus: text("expected_sequence_status").notNull(),
    expectedReadyEnrollmentCount: integer("expected_ready_enrollment_count").notNull().default(0),
    expectedDeliveryStatusWebhookReadinessStatus: text("expected_delivery_status_webhook_readiness_status").notNull(),
    dryRunMessageCount: integer("dry_run_message_count").notNull().default(0),
    heldEnrollmentCount: integer("held_enrollment_count").notNull().default(0),
    activeSuppressionCount: integer("active_suppression_count").notNull().default(0),
    providerLimitPolicy: text("provider_limit_policy").notNull(),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("audience_sequence_provider_polling_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    deliveryStatusWebhookReadinessIdx: index(
      "audience_sequence_provider_polling_readiness_status_webhook_readiness_idx",
    ).on(table.deliveryStatusWebhookReadinessId),
    deliveryResultReadinessIdx: index("audience_sequence_provider_polling_readiness_result_readiness_idx").on(
      table.deliveryResultReadinessId,
    ),
    deliveryAttemptReadinessIdx: index("audience_sequence_provider_polling_readiness_attempt_readiness_idx").on(
      table.deliveryAttemptReadinessId,
    ),
    providerCallReadinessIdx: index("audience_sequence_provider_polling_readiness_provider_call_idx").on(
      table.providerCallReadinessId,
    ),
    dispatchAttemptIdx: index("audience_sequence_provider_polling_readiness_attempt_idx").on(
      table.dispatchAttemptId,
    ),
    sequenceStatusIdx: index("audience_sequence_provider_polling_readiness_sequence_status_idx").on(
      table.sequenceId,
      table.status,
    ),
  }),
...
export const audienceSequenceReceiptPayloadReadiness = sqliteTable(
  "audience_sequence_receipt_payload_readiness",
  {
    ...
    deliveryBatchId: text("delivery_batch_id").notNull(),
    scheduleIntentId: text("schedule_intent_id").notNull(),
    status: text("status").notNull().default("sequence_receipt_payload_readiness_recorded"),
    queueName: text("queue_name").notNull(),
    providerName: text("provider_name").notNull(),
    providerMode: text("provider_mode").notNull(),
    receiptPayloadGateStatus: text("receipt_payload_gate_status").notNull(),
    providerPollingDependencyStatus: text("provider_polling_dependency_status").notNull(),
    providerResponsePolicy: text("provider_response_policy").notNull(),
    providerMessageIdPolicy: text("provider_message_id_policy").notNull(),
    deliveryAttemptPolicy: text("delivery_attempt_policy").notNull(),
    deliveryResultPolicy: text("delivery_result_policy").notNull(),
    ...
    expectedSequenceStatus: text("expected_sequence_status").notNull(),
    expectedReadyEnrollmentCount: integer("expected_ready_enrollment_count").notNull().default(0),
    expectedProviderPollingReadinessStatus: text("expected_provider_polling_readiness_status").notNull(),
    dryRunMessageCount: integer("dry_run_message_count").notNull().default(0),
    heldEnrollmentCount: integer("held_enrollment_count").notNull().default(0),
    activeSuppressionCount: integer("active_suppression_count").notNull().default(0),
    providerLimitPolicy: text("provider_limit_policy").notNull(),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("audience_sequence_receipt_payload_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    providerPollingReadinessIdx: index("audience_sequence_receipt_payload_readiness_provider_polling_idx").on(
      table.providerPollingReadinessId,
    ),
    deliveryStatusWebhookReadinessIdx: index(
      "audience_sequence_receipt_payload_readiness_status_webhook_readiness_idx",
    ).on(table.deliveryStatusWebhookReadinessId),
    deliveryResultReadinessIdx: index("audience_sequence_receipt_payload_readiness_result_readiness_idx").on(
      table.deliveryResultReadinessId,
    ),
    deliveryAttemptReadinessIdx: index("audience_sequence_receipt_payload_readiness_attempt_readiness_idx").on(
      table.deliveryAttemptReadinessId,
    ),
    providerCallReadinessIdx: index("audience_sequence_receipt_payload_readiness_provider_call_idx").on(
      table.providerCallReadinessId,
    ),
    dispatchAttemptIdx: index("audience_sequence_receipt_payload_readiness_attempt_idx").on(
      table.dispatchAttemptId,
    ),
    sequenceStatusIdx: index("audience_sequence_receipt_payload_readiness_sequence_status_idx").on(
      table.sequenceId,
      table.status,
    ),
  }),
...
export const analyticsNotificationDeliveryAttemptReadinessRecords = sqliteTable(
  "analytics_notification_delivery_attempt_readiness_records",
  {
    id: text("id").primaryKey(),
    dashboardId: text("dashboard_id").notNull(),
    readinessId: text("readiness_id").notNull(),
    ...
    providerCallReadinessId: text("provider_call_readiness_id").notNull(),
    recordKind: text("record_kind").notNull(),
    notificationDeliveryAttemptReadinessDisposition: text(
      "notification_delivery_attempt_readiness_disposition",
    ).notNull(),
    timeWindowKey: text("time_window_key").notNull(),
    expectedDashboardRevisionId: text("expected_dashboard_revision_id").notNull(),
    expectedReadinessStatus: text("expected_readiness_status").notNull(),
    ...
    expectedNotificationProviderCallReadinessStatus: text(
      "expected_notification_provider_call_readiness_status",
    ).notNull(),
    expectedOwnerReviewStatus: text("expected_owner_review_status").notNull(),
    expectedAlertThresholdCount: integer("expected_alert_threshold_count").notNull().default(0),
    expectedConversionSampleSize: integer("expected_conversion_sample_size").notNull().default(0),
    ...
    privateNoteSha256: text("private_note_sha256"),
    confirmationTextSha256: text("confirmation_text_sha256").notNull(),
    ownerDeliveryAttemptReadinessRecorded: integer("owner_delivery_attempt_readiness_recorded", {
      mode: "boolean",
    })
      .notNull()
      .default(false),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("analytics_notification_delivery_attempt_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    dashboardTimeIdx: index("analytics_notification_delivery_attempt_readiness_dashboard_time_idx").on(
      table.dashboardId,
      table.createdAt,
    ),
    readinessChannelIdx: index("analytics_notification_delivery_attempt_readiness_readiness_channel_idx").on(
      table.readinessId,
      table.channelId,
      table.createdAt,
    ),
    sendPayloadReadinessIdx: index("analytics_notification_delivery_attempt_readiness_send_payload_idx").on(
      table.sendPayloadReadinessId,
      table.createdAt,
    ),
    providerCallReadinessIdx: index("analytics_notification_delivery_attempt_readiness_provider_call_idx").on(
      table.providerCallReadinessId,
      table.createdAt,
    ),
  }),
);
export const analyticsNotificationDeliveryResultReadinessRecords = sqliteTable(
  "analytics_notification_delivery_result_readiness_records",
  {
    id: text("id").primaryKey(),
    dashboardId: text("dashboard_id").notNull(),
    readinessId: text("readiness_id").notNull(),
    ...
    deliveryAttemptReadinessId: text("delivery_attempt_readiness_id").notNull(),
    recordKind: text("record_kind").notNull(),
    notificationDeliveryResultReadinessDisposition: text(
      "notification_delivery_result_readiness_disposition",
    ).notNull(),
    timeWindowKey: text("time_window_key").notNull(),
    expectedDashboardRevisionId: text("expected_dashboard_revision_id").notNull(),
    expectedReadinessStatus: text("expected_readiness_status").notNull(),
    ...
    expectedNotificationDeliveryAttemptReadinessStatus: text(
      "expected_notification_delivery_attempt_readiness_status",
    ).notNull(),
    expectedOwnerReviewStatus: text("expected_owner_review_status").notNull(),
    expectedAlertThresholdCount: integer("expected_alert_threshold_count").notNull().default(0),
    expectedConversionSampleSize: integer("expected_conversion_sample_size").notNull().default(0),
    ...
    privateNoteSha256: text("private_note_sha256"),
    confirmationTextSha256: text("confirmation_text_sha256").notNull(),
    ownerDeliveryResultReadinessRecorded: integer("owner_delivery_result_readiness_recorded", {
      mode: "boolean",
    })
      .notNull()
      .default(false),
    ...
    queuePayloadIncluded: integer("queue_payload_included", { mode: "boolean" }).notNull().default(false),
    providerSendEnabled: integer("provider_send_enabled", { mode: "boolean" }).notNull().default(false),
    deliveryResultRecorded: integer("delivery_result_recorded", { mode: "boolean" }).notNull().default(false),
    providerConfigured: integer("provider_configured", { mode: "boolean" }).notNull().default(false),
    providerResponseCreated: integer("provider_response_created", { mode: "boolean" }).notNull().default(false),
    providerSecretIncluded: integer("provider_secret_included", { mode: "boolean" }).notNull().default(false),
    senderDomainConfigured: integer("sender_domain_configured", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("analytics_notification_delivery_result_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    dashboardTimeIdx: index("analytics_notification_delivery_result_readiness_dashboard_time_idx").on(
      table.dashboardId,
      table.createdAt,
    ),
    readinessChannelIdx: index("analytics_notification_delivery_result_readiness_readiness_channel_idx").on(
      table.readinessId,
      table.channelId,
      table.createdAt,
    ),
    sendPayloadReadinessIdx: index("analytics_notification_delivery_result_readiness_send_payload_idx").on(
      table.sendPayloadReadinessId,
      table.createdAt,
    ),
    deliveryAttemptReadinessIdx: index("analytics_notification_delivery_result_readiness_delivery_attempt_idx").on(
      table.deliveryAttemptReadinessId,
      table.createdAt,
    ),
  }),
);
export const analyticsNotificationDeliveryStatusWebhookReadinessRecords = sqliteTable(
  "analytics_notification_delivery_status_webhook_readiness_records",
  {
    id: text("id").primaryKey(),
    dashboardId: text("dashboard_id").notNull(),
    readinessId: text("readiness_id").notNull(),
    ...
    deliveryResultReadinessId: text("delivery_result_readiness_id").notNull(),
    recordKind: text("record_kind").notNull(),
    notificationDeliveryStatusWebhookReadinessDisposition: text(
      "notification_delivery_status_webhook_readiness_disposition",
    ).notNull(),
    timeWindowKey: text("time_window_key").notNull(),
    expectedDashboardRevisionId: text("expected_dashboard_revision_id").notNull(),
    expectedReadinessStatus: text("expected_readiness_status").notNull(),
    ...
    expectedNotificationDeliveryResultReadinessStatus: text(
      "expected_notification_delivery_result_readiness_status",
    ).notNull(),
    expectedOwnerReviewStatus: text("expected_owner_review_status").notNull(),
    expectedAlertThresholdCount: integer("expected_alert_threshold_count").notNull().default(0),
    expectedConversionSampleSize: integer("expected_conversion_sample_size").notNull().default(0),
    ...
    privateNoteSha256: text("private_note_sha256"),
    confirmationTextSha256: text("confirmation_text_sha256").notNull(),
    ownerDeliveryStatusWebhookReadinessRecorded: integer("owner_delivery_status_webhook_readiness_recorded", {
      mode: "boolean",
    })
      .notNull()
      .default(false),
    ...
    queuePayloadIncluded: integer("queue_payload_included", { mode: "boolean" }).notNull().default(false),
    providerSendEnabled: integer("provider_send_enabled", { mode: "boolean" }).notNull().default(false),
    deliveryStatusWebhookRecorded: integer("delivery_status_webhook_recorded", { mode: "boolean" })
      .notNull()
      .default(false),
    providerConfigured: integer("provider_configured", { mode: "boolean" }).notNull().default(false),
    providerResponseCreated: integer("provider_response_created", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("analytics_notification_delivery_status_webhook_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    dashboardTimeIdx: index("analytics_notification_delivery_status_webhook_readiness_dashboard_time_idx").on(
      table.dashboardId,
      table.createdAt,
    ),
    readinessChannelIdx: index("analytics_notification_delivery_status_webhook_readiness_readiness_channel_idx").on(
      table.readinessId,
      table.channelId,
      table.createdAt,
    ),
    sendPayloadReadinessIdx: index("analytics_notification_delivery_status_webhook_readiness_send_payload_idx").on(
      table.sendPayloadReadinessId,
      table.createdAt,
    ),
    deliveryResultReadinessIdx: index("analytics_notification_delivery_status_webhook_readiness_delivery_result_idx").on(
      table.deliveryResultReadinessId,
      table.createdAt,
    ),
  }),
...
export const analyticsNotificationProviderPollingReadinessRecords = sqliteTable(
  "analytics_notification_provider_polling_readiness_records",
  {
    id: text("id").primaryKey(),
    dashboardId: text("dashboard_id").notNull(),
    readinessId: text("readiness_id").notNull(),
    ...
    deliveryStatusWebhookReadinessId: text("delivery_status_webhook_readiness_id").notNull(),
    recordKind: text("record_kind").notNull(),
    notificationProviderPollingReadinessDisposition: text(
      "notification_provider_polling_readiness_disposition",
    ).notNull(),
    timeWindowKey: text("time_window_key").notNull(),
    expectedDashboardRevisionId: text("expected_dashboard_revision_id").notNull(),
    expectedReadinessStatus: text("expected_readiness_status").notNull(),
    ...
    expectedNotificationDeliveryStatusWebhookReadinessStatus: text(
      "expected_notification_delivery_status_webhook_readiness_status",
    ).notNull(),
    expectedOwnerReviewStatus: text("expected_owner_review_status").notNull(),
    expectedAlertThresholdCount: integer("expected_alert_threshold_count").notNull().default(0),
    expectedConversionSampleSize: integer("expected_conversion_sample_size").notNull().default(0),
    ...
    privateNoteSha256: text("private_note_sha256"),
    confirmationTextSha256: text("confirmation_text_sha256").notNull(),
    ownerProviderPollingReadinessRecorded: integer("owner_provider_polling_readiness_recorded", {
      mode: "boolean",
    })
      .notNull()
      .default(false),
    ...
    queuePayloadIncluded: integer("queue_payload_included", { mode: "boolean" }).notNull().default(false),
    providerSendEnabled: integer("provider_send_enabled", { mode: "boolean" }).notNull().default(false),
    providerPollingRecorded: integer("provider_polling_recorded", { mode: "boolean" }).notNull().default(false),
    providerConfigured: integer("provider_configured", { mode: "boolean" }).notNull().default(false),
    providerResponseCreated: integer("provider_response_created", { mode: "boolean" }).notNull().default(false),
    providerSecretIncluded: integer("provider_secret_included", { mode: "boolean" }).notNull().default(false),
    senderDomainConfigured: integer("sender_domain_configured", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("analytics_notification_provider_polling_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    dashboardTimeIdx: index("analytics_notification_provider_polling_readiness_dashboard_time_idx").on(
      table.dashboardId,
      table.createdAt,
    ),
    readinessChannelIdx: index("analytics_notification_provider_polling_readiness_readiness_channel_idx").on(
      table.readinessId,
      table.channelId,
      table.createdAt,
    ),
    sendPayloadReadinessIdx: index("analytics_notification_provider_polling_readiness_send_payload_idx").on(
      table.sendPayloadReadinessId,
      table.createdAt,
    ),
    deliveryStatusWebhookReadinessIdx: index("analytics_notification_provider_polling_readiness_status_webhook_idx").on(
      table.deliveryStatusWebhookReadinessId,
      table.createdAt,
    ),
  }),
);

export const analyticsNotificationReceiptPayloadReadinessRecords = sqliteTable(
  "analytics_notification_receipt_payload_readiness_records",
  {
    id: text("id").primaryKey(),
    dashboardId: text("dashboard_id").notNull(),
    readinessId: text("readiness_id").notNull(),
    ...
    providerPollingReadinessId: text("provider_polling_readiness_id").notNull(),
    recordKind: text("record_kind").notNull(),
    notificationReceiptPayloadReadinessDisposition: text(
      "notification_receipt_payload_readiness_disposition",
    ).notNull(),
    timeWindowKey: text("time_window_key").notNull(),
    expectedDashboardRevisionId: text("expected_dashboard_revision_id").notNull(),
    expectedReadinessStatus: text("expected_readiness_status").notNull(),
    ...
    expectedNotificationProviderPollingReadinessStatus: text(
      "expected_notification_provider_polling_readiness_status",
    ).notNull(),
    expectedOwnerReviewStatus: text("expected_owner_review_status").notNull(),
    expectedAlertThresholdCount: integer("expected_alert_threshold_count").notNull().default(0),
    expectedConversionSampleSize: integer("expected_conversion_sample_size").notNull().default(0),
    ...
    privateNoteSha256: text("private_note_sha256"),
    confirmationTextSha256: text("confirmation_text_sha256").notNull(),
    ownerReceiptPayloadReadinessRecorded: integer("owner_receipt_payload_readiness_recorded", {
      mode: "boolean",
    })
      .notNull()
      .default(false),
    ...
    queuePayloadIncluded: integer("queue_payload_included", { mode: "boolean" }).notNull().default(false),
    providerSendEnabled: integer("provider_send_enabled", { mode: "boolean" }).notNull().default(false),
    receiptPayloadRecorded: integer("receipt_payload_recorded", { mode: "boolean" }).notNull().default(false),
    providerConfigured: integer("provider_configured", { mode: "boolean" }).notNull().default(false),
    providerResponseCreated: integer("provider_response_created", { mode: "boolean" }).notNull().default(false),
    providerSecretIncluded: integer("provider_secret_included", { mode: "boolean" }).notNull().default(false),
    senderDomainConfigured: integer("sender_domain_configured", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("analytics_notification_receipt_payload_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    dashboardTimeIdx: index("analytics_notification_receipt_payload_readiness_dashboard_time_idx").on(
      table.dashboardId,
      table.createdAt,
    ),
    readinessChannelIdx: index("analytics_notification_receipt_payload_readiness_readiness_channel_idx").on(
      table.readinessId,
      table.channelId,
      table.createdAt,
    ),
    sendPayloadReadinessIdx: index("analytics_notification_receipt_payload_readiness_send_payload_idx").on(
      table.sendPayloadReadinessId,
      table.createdAt,
    ),
    providerPollingReadinessIdx: index("analytics_notification_receipt_payload_readiness_provider_polling_idx").on(
      table.providerPollingReadinessId,
      table.createdAt,
    ),
  }),
...
export const analyticsNotificationDeliveryReceiptReadinessRecords = sqliteTable(
  "analytics_notification_delivery_receipt_readiness_records",
  {
    id: text("id").primaryKey(),
    dashboardId: text("dashboard_id").notNull(),
    readinessId: text("readiness_id").notNull(),
    ...
    recordKind: text("record_kind").notNull(),
    notificationDeliveryReceiptReadinessDisposition: text(
      "notification_delivery_receipt_readiness_disposition",
    ).notNull(),
    timeWindowKey: text("time_window_key").notNull(),
    expectedDashboardRevisionId: text("expected_dashboard_revision_id").notNull(),
    expectedReadinessStatus: text("expected_readiness_status").notNull(),
    ...
    expectedNotificationReceiptPayloadReadinessStatus: text(
      "expected_notification_receipt_payload_readiness_status",
    ).notNull(),
    expectedOwnerReviewStatus: text("expected_owner_review_status").notNull(),
    expectedAlertThresholdCount: integer("expected_alert_threshold_count").notNull().default(0),
    expectedConversionSampleSize: integer("expected_conversion_sample_size").notNull().default(0),
    ...
    privateNoteSha256: text("private_note_sha256"),
    confirmationTextSha256: text("confirmation_text_sha256").notNull(),
    ownerDeliveryReceiptReadinessRecorded: integer("owner_delivery_receipt_readiness_recorded", {
      mode: "boolean",
    })
      .notNull()
      .default(false),
    ...
    queuePayloadIncluded: integer("queue_payload_included", { mode: "boolean" }).notNull().default(false),
    providerSendEnabled: integer("provider_send_enabled", { mode: "boolean" }).notNull().default(false),
    deliveryReceiptRecorded: integer("delivery_receipt_recorded", { mode: "boolean" }).notNull().default(false),
    providerConfigured: integer("provider_configured", { mode: "boolean" }).notNull().default(false),
    providerResponseCreated: integer("provider_response_created", { mode: "boolean" }).notNull().default(false),
    providerSecretIncluded: integer("provider_secret_included", { mode: "boolean" }).notNull().default(false),
    senderDomainConfigured: integer("sender_domain_configured", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("analytics_notification_delivery_receipt_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    dashboardTimeIdx: index("analytics_notification_delivery_receipt_readiness_dashboard_time_idx").on(
      table.dashboardId,
      table.createdAt,
    ),
    readinessChannelIdx: index("analytics_notification_delivery_receipt_readiness_readiness_channel_idx").on(
      table.readinessId,
      table.channelId,
      table.createdAt,
    ),
    sendPayloadReadinessIdx: index("analytics_notification_delivery_receipt_readiness_send_payload_idx").on(
      table.sendPayloadReadinessId,
      table.createdAt,
    ),
    receiptPayloadReadinessIdx: index("analytics_notification_delivery_receipt_readiness_receipt_payload_idx").on(
...
export const analyticsNotificationProviderStatusReconciliationReadinessRecords = sqliteTable(
  "analytics_notification_provider_status_reconciliation_readiness_records",
  {
    id: text("id").primaryKey(),
    dashboardId: text("dashboard_id").notNull(),
    readinessId: text("readiness_id").notNull(),
    ...
    providerDomainReadinessId: text("provider_domain_readiness_id").notNull(),
    sendPayloadReadinessId: text("send_payload_readiness_id").notNull(),
    deliveryReceiptReadinessId: text("delivery_receipt_readiness_id").notNull(),
    recordKind: text("record_kind").notNull(),
    notificationProviderStatusReconciliationReadinessDisposition: text(
      "notification_provider_status_reconciliation_readiness_disposition",
    ).notNull(),
    timeWindowKey: text("time_window_key").notNull(),
    expectedDashboardRevisionId: text("expected_dashboard_revision_id").notNull(),
    expectedReadinessStatus: text("expected_readiness_status").notNull(),
    ...
    expectedNotificationDeliveryReceiptReadinessStatus: text(
      "expected_notification_delivery_receipt_readiness_status",
    ).notNull(),
    expectedOwnerReviewStatus: text("expected_owner_review_status").notNull(),
    expectedAlertThresholdCount: integer("expected_alert_threshold_count").notNull().default(0),
    expectedConversionSampleSize: integer("expected_conversion_sample_size").notNull().default(0),
    ...
    privateNoteSha256: text("private_note_sha256"),
    confirmationTextSha256: text("confirmation_text_sha256").notNull(),
    ownerProviderStatusReconciliationReadinessRecorded: integer("owner_provider_status_reconciliation_readiness_recorded", {
      mode: "boolean",
    })
      .notNull()
      .default(false),
    ...
    queueProducerEnabled: integer("queue_producer_enabled", { mode: "boolean" }).notNull().default(false),
    queueConsumerEnabled: integer("queue_consumer_enabled", { mode: "boolean" }).notNull().default(false),
    providerStatusReconciliationEnabled: integer("provider_status_reconciliation_enabled", { mode: "boolean" })
      .notNull()
      .default(false),
    queueMessageCreated: integer("queue_message_created", { mode: "boolean" }).notNull().default(false),
    queueMessageConsumed: integer("queue_message_consumed", { mode: "boolean" }).notNull().default(false),
    ...
    queuePayloadIncluded: integer("queue_payload_included", { mode: "boolean" }).notNull().default(false),
    providerSendEnabled: integer("provider_send_enabled", { mode: "boolean" }).notNull().default(false),
    providerStatusReconciliationRecorded: integer("provider_status_reconciliation_recorded", { mode: "boolean" })
      .notNull()
      .default(false),
    providerConfigured: integer("provider_configured", { mode: "boolean" }).notNull().default(false),
    providerResponseCreated: integer("provider_response_created", { mode: "boolean" }).notNull().default(false),
  ...
  (table) => ({
    idempotencyUnique: uniqueIndex("analytics_notification_provider_status_reconciliation_readiness_idempotency_unique").on(
      table.idempotencyKey,
    ),
    dashboardTimeIdx: index("analytics_notification_provider_status_reconciliation_readiness_dashboard_time_idx").on(
      table.dashboardId,
      table.createdAt,
    ),
    readinessChannelIdx: index("analytics_notification_provider_status_reconciliation_readiness_readiness_channel_idx").on(
      table.readinessId,
      table.channelId,
      table.createdAt,
    ),
    sendPayloadReadinessIdx: index("analytics_notification_provider_status_reconciliation_readiness_send_payload_idx").on(
      table.sendPayloadReadinessId,
      table.createdAt,
    ),
    deliveryReceiptReadinessIdx: index("analytics_notification_provider_status_reconciliation_readiness_delivery_receipt_idx").on(
      table.deliveryReceiptReadinessId,
      table.createdAt,
    ),
  }),
);
