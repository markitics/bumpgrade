...
- `/funnels/source-data`: public-safe JSON with one seeded draft funnel, ordered
  ...
  ID, preview route, owner-session editable draft capability metadata,
  owner-session checkout-link capability metadata, published D1 funnel
  summaries, and write boundary.
...
template additionally requires exact template confirmation text. Linking the
