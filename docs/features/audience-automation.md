...
Issues #85, #103, #137, and #167 add the first audience automation contract,
the first consent-backed opt-in capture path, owner-gated subscriber inspection,
and public-safe unsubscribe/suppression evidence for issue #17.
...
- `/admin/audience`: owner-gated subscriber, tag, consent, and draft sequence
  enrollment inspection plus suppression totals.
...
- public-safe aggregate subscriber and suppression inspection counts and
  redaction flags.
...
email addresses, names, suppression hashes, unsubscribe reasons, raw
...
Owner sessions can inspect private contact rows in `/admin/audience`. Direct
agent subscriber writes, imports, email sends, broadcast scheduling, CRM notes,
private exports, or suppression-list administration require future authenticated
